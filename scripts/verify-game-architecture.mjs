...
assert.match(pathfindingSrc, /ENEMY_PATHFINDING_SCHEMA_VERSION = 2/, 'enemy pathfinding must use the v39.3.22m flow-field schema');
...
assert.match(clientRuntime, /HOST_IMPULSE_HARD_RECONCILE_D2/, 'guest host impulse correction must preserve hard drift classification for extreme hostile knockback only');
assert.match(clientRuntime, /HOST_RECONCILE_EXTRAPOLATE_MS/, 'guest reconcile diagnostics should keep direct peer RTT context');

assert.match(clientRuntime, /PREDICTION_BUFFER_LIMIT/, 'guest runtime must keep a bounded prediction input history');
assert.match(clientRuntime, /prunePredictionFrames/, 'guest runtime must prune prediction frames by host-acknowledged input sequence');
assert.match(clientRuntime, /replayPredictionFrames/, 'guest runtime must replay unacknowledged inputs after applying authoritative host pose');
assert.match(clientRuntime, /reconcileLocalPoseFromHost/, 'guest runtime must centralize rollback/replay reconciliation against host snapshots');
assert.match(clientRuntime, /app\.predictionFrames\.push/, 'guest runtime must record prediction frames for rollback replay');
assert.match(clientRuntime, /app\.inputSeq = \(app\.inputSeq \|\| 0\) \+ 1/, 'guest input sequence must advance every predicted movement frame, not only when packets are sent');
assert.match(clientRuntime, /clonePredictionInput\(inputState\)/, 'guest must send/replay sanitized prediction input snapshots');
assert.match(clientRuntime, /app\.reconcileStats/, 'guest runtime must expose reconciliation debug stats for HUD diagnostics');
assert.match(read('src/ui.js'), /REC \$\{reconcile\.ackedSeq/, 'HUD must show reconciliation ack/local sequence and drift diagnostics');
...
assert.match(combatRuntime, /payload\.aimX/, 'host combat must recompute guest shot angle from authoritative player position to aim target');
assert.match(rendererRuntime, /acceptedProjectileIds/, 'predicted projectiles must be reconciled against host-accepted projectile ids');
assert.match(rendererRuntime, /!acceptedIds\.has/, 'host-accepted shots must remove local-only predicted projectiles instead of hiding authoritative outcomes');

assert.match(hostRuntime, /updateHostWorld\(/, 'host runtime must own simulation ticking');
assert.match(hostRuntime, /HOST_SIM_FIXED_DT/, 'host runtime must use a fixed-step simulation delta instead of frame delta');
assert.match(hostRuntime, /fixedStepAccumulator/, 'host runtime must accumulate frame time for deterministic fixed-step catch-up');
assert.match(hostRuntime, /HOST_SIM_MAX_CATCHUP_STEPS/, 'host runtime must cap catch-up steps to avoid death spirals after throttling');
assert.match(hostRuntime, /writeHostSimStats/, 'host runtime must expose fixed-step/throttle diagnostics for HUD debugging');
assert.doesNotMatch(hostRuntime, /updateHostWorld\(app\.hostState, app\.hostInputs, dt\)/, 'host runtime must not feed variable frame dt into authoritative simulation');
assert.match(read('src/main.js'), /hostRuntime\.update\(rawDt, now, gameNow\)/, 'main loop must pass raw frame time into host fixed-step accumulator');
assert.match(read('src/ui.js'), /SIM \$\{hostSim\.steps/, 'HUD must show host fixed-step simulation diagnostics');
...
assert.match(snapshotBudget, /SNAPSHOT_HAZARD_RADIUS/, 'snapshot budget must define a hazard interest radius for per-peer priority snapshots');
assert.match(snapshotBudget, /applyInterestPriority/, 'snapshot budget must sort gameplay entities by per-peer interest before applying size caps');
assert.match(snapshotBudget, /projectilePriority/, 'snapshot budget must prioritize nearby/closing hostile projectiles before blind projectile caps');
assert.match(snapshotBudget, /enemyPriority/, 'snapshot budget must prioritize nearby enemies before blind enemy caps');
...
assert.match(read('src/app/hostRuntime.js'), /focusPlayerId: peerId/, 'host runtime must build focused per-peer snapshots instead of reusing one global packet');
assert.doesNotMatch(read('src/app/hostRuntime.js'), /cache\[mode\]/, 'host runtime must not cache one snapshot per mode for every peer; interest snapshots are per-peer');
...
assert.match(read('src/app/devControls.js'), /focusPlayerId: peerId/, 'dev snapshot broadcasts must keep per-peer interest priority');
