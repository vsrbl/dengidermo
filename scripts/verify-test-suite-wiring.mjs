...
assert.ok(rootScriptFiles.includes('verify-v39-0-6-shooter-ranged-attack.mjs'), 'current exact migration guard must live in scripts root');
...
assert.ok(!rootScriptFiles.some((name) => /^verify-v\d/.test(name) && name !== 'verify-v39-0-6-shooter-ranged-attack.mjs'), 'old exact-version checks must not remain in current scripts root');
...
for (const name of Object.keys(scripts)) {
  if (name === 'check:v39-0-6') continue;
...
assert.ok(checkAll.trim().endsWith('npm run check:v39-0-6'), 'check:all should end with the current migration guard');
