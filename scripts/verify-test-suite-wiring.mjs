...
assert.ok(rootScriptFiles.includes('verify-v39-0-4-bomber-enemy-and-visual-pass.mjs'), 'current exact migration guard must live in scripts root');
...
assert.ok(!rootScriptFiles.some((name) => /^verify-v\d/.test(name) && name !== 'verify-v39-0-4-bomber-enemy-and-visual-pass.mjs'), 'old exact-version checks must not remain in current scripts root');
...
for (const name of Object.keys(scripts)) {
  if (name === 'check:v39-0-4') continue;
...
assert.ok(checkAll.trim().endsWith('npm run check:v39-0-4'), 'check:all should end with the current migration guard');
