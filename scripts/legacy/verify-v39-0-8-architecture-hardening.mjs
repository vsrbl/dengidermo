...
assert.equal(VERSION, 'v39.0.8');
assert.equal(BUILD_ID, 'v39.0.8-20260520');

const pkg = readJson('package.json');
assert.equal(pkg.version, '39.0.8');
assert.equal(readJson('server/package.json').version, '39.0.8');
assert.equal(readJson('src/package.json').version, '39.0.8');
assert.equal(readJson('release.json').version, 'v39.0.8');
assert.equal(readJson('release.json').notes, 'v39.0.8 architecture hardening after armor: hostile projectile hook parity, scripted spawn retry semantics, armor source matrix, and renderer cleanup');
assert.equal(pkg.scripts['check:v39-0-8'], 'node scripts/verify-v39-0-8-architecture-hardening.mjs');
assert.ok(pkg.scripts['check:all'].includes('check:v39-0-8'), 'check:all must include current migration guard');
assert.ok(!pkg.scripts['check:all'].includes('check:v39-0-7'), 'check:all must not keep previous exact guard');
assert.ok(!exists('scripts/verify-v39-0-7-enemy-armor-system.mjs'), 'previous exact guard must be archived out of root scripts');
assert.ok(exists('scripts/legacy/verify-v39-0-7-enemy-armor-system.mjs'), 'previous exact guard should be archived');
...
assert.ok(projectileSrc.includes('moveProjectileWithRoomHooks'), 'player and hostile projectiles should share room projectile update modifier path');
...
assert.ok(projectileSrc.includes('updateHostileProjectile'), 'hostile projectile path must remain explicit and host-owned');
...
assert.ok(server.includes('const SERVER_VERSION = "v39.0.8"'), 'server version must match v39.0.8');
assert.ok(server.includes('const SERVER_BUILD_ID = "v39.0.8-20260520"'), 'server build must match v39.0.8 build');
assert.ok(server.includes('HEARTBEAT_INTERVAL_MS'), 'v38.14.6 heartbeat hardening must remain');

const index = read('index.html');
assert.ok(index.includes('src/main.v39-0-8.js?v=39.0.8'), 'index must use current cache-busted entry');
assert.ok(index.includes('V39.0.8 | BUILD 20260520'), 'HUD must expose v39.0.8');
assert.ok(exists('src/main.v39-0-8.js'), 'current versioned entry must exist');
assert.ok(!exists('src/main.v39-0-7.js'), 'previous versioned entry must not ship');
for (const name of ['session', 'clientRuntime', 'hostRuntime', 'upgradeClient', 'devControls', 'releaseIntegrity']) {
  assert.ok(exists(`src/app/${name}.v39-0-8.js`), `current versioned app module missing: ${name}`);
  assert.ok(!exists(`src/app/${name}.v39-0-7.js`), `previous versioned app module should not ship: ${name}`);
}
...
console.log('v39.0.8 architecture hardening checks passed');
