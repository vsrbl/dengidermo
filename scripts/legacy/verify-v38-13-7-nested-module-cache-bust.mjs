...
test('v38.13.8 is registered across frontend package server and cache queries', () => {
  ...
  assert.match(read('server/server.js'), /nncckkrr signaling v38\.13\.8/);
  assert.match(read('index.html'), /V38\.13\.8/);
  assert.match(read('index.html'), /style\.css\?v=38\.13\.8/);
  assert.match(read('index.html'), /config\.js\?v=38\.13\.8/);
...
test('entry graph uses versioned app module URLs to bypass cached v38.12-v38.13.6 modules', () => {
  const entry = read('src/main.v38-13-8.js');
  for (const name of ['session', 'hostRuntime', 'clientRuntime', 'upgradeClient', 'devControls']) {
    assert.ok(exists(`src/app/${name}.v38-13-8.js`), `${name}.v38-13-8.js missing`);
    assert.match(entry, new RegExp(`\\./app/${name}\\.v38-13-8\\.js`), `versioned entry must import ${name}.v38-13-8.js`);
...
test('START_WEAPON boundary is correct and cache compatibility alias exists', () => {
  ...
  const files = [
    'src/main.js',
    'src/main.v38-13-8.js',
    'src/app/session.js',
    'src/app/session.v38-13-8.js',
    'src/app/clientRuntime.js',
    'src/app/clientRuntime.v38-13-8.js'
  ];
  for (const rel of files) {
    const src = read(rel);
    assert.doesNotMatch(src, /START_WEAPON[^\n]*core\/constants\.js/, `${rel} must not import START_WEAPON from core/constants.js`);
  }
  assert.match(read('src/app/session.v38-13-8.js'), /import \{ START_WEAPON \} from "\.\.\/data\/weapons\.js";/);
  assert.match(read('src/app/clientRuntime.v38-13-8.js'), /import \{ START_WEAPON, WEAPONS \} from "\.\.\/data\/weapons\.js";/);
});
...
console.log(`All ${passed} v38.13.8 nested module cache-bust checks passed`);
