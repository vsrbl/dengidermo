...
await test('v38.13.8 versions are aligned across frontend package server and cache query strings', () => {
  assert.equal(VERSION, 'v38.13.8');
  assert.equal(pkg.version, '38.13.8');
  assert.equal(serverPkg.version, '38.13.8');
  assert.match(serverSrc, /SERVER_VERSION = \"v38\.13\.8\"/);
  assert.match(serverSrc, /SIGNALING_PROTOCOL_VERSION = 2/);
  assert.match(serverSrc, /type: \"hello\"/);
  assert.match(indexHtml, /main(?:\.v38-13-8)?\.js\?v=38\.13\.8/);
  assert.match(indexHtml, /config\.js\?v=38\.13\.8/);
  assert.match(indexHtml, /style\.css\?v=38\.13\.8/);
});
...
console.log(`All ${results.length} v38.13.8 visible connect and server-handshake checks passed`);
