...
test('version strings are aligned across frontend/package/server', () => {
  assert.equal(pkg.version, VERSION.replace(/^v/, ''));
  assert.equal(serverPkg.version, VERSION.replace(/^v/, ''));
  assert.match(serverSrc, new RegExp(VERSION.replaceAll('.', '\\.')), 'server banner is stale');
