...
test('baseline rooms still use open arena and do not change gameplay geometry', () => {
  for (const room of ROOM_SEQUENCE) {
    assert.equal(room.layout, 'open_arena', `${room.id} should not silently switch layouts in v38.13.3 foundation`);
  }
...
console.log(`All ${results.length} v38.13.3 room geometry checks passed`);
