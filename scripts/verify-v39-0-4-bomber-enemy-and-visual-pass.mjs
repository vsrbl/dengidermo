...
assert.equal(VERSION, 'v39.0.4');
assert.equal(BUILD_ID, 'v39.0.4-20260520');

const pkg = readJson('package.json');
assert.equal(pkg.version, '39.0.4');
assert.equal(readJson('server/package.json').version, '39.0.4');
assert.equal(readJson('src/package.json').version, '39.0.4');
assert.equal(readJson('release.json').version, 'v39.0.4');
assert.equal(readJson('release.json').notes, 'v39.0.4 bomber enemy + enemy visual pass: new bomber behavior and refreshed enemy silhouettes in static field content');
assert.equal(pkg.scripts['check:v39-0-4'], 'node scripts/verify-v39-0-4-bomber-enemy-and-visual-pass.mjs');
assert.ok(pkg.scripts['check:all'].includes('check:v39-0-4'), 'check:all must include current migration guard');
assert.ok(!pkg.scripts['check:all'].includes('check:v39-0-3'), 'check:all must not keep previous exact guard');
assert.ok(!exists('scripts/verify-v39-0-3-charger-enemy.mjs'), 'previous exact guard must be archived out of root scripts');
assert.ok(exists('scripts/legacy/verify-v39-0-3-charger-enemy.mjs'), 'previous exact guard should be archived');
...
assert.equal(ENEMIES.bomber.bomb.fuse, 0.72, 'bomber fuse must come from enemy data');
assert.equal(ENEMIES.bomber.bomb.explosionRadius, 88, 'bomber explosion radius must come from enemy data');
...
assert.deepEqual(staticField.enemyPool, ['runner', 'charger', 'bomber', 'shooter', 'tank'], 'bomber should be enabled only in the static-field controlled pool');
...
const state = createGameState('V39-BOMBER');
addPlayer(state, 'p1', 0);
for (let i = 0; i < 6; i += 1) beginRoomTransition(state, 'verify-v39-bomber', { offerUpgrades: false });
...
player.y = 800;
player.hp = 100;
const bomber = spawnEnemy(state, 'bomber', 1070, 800, { role: 'verify' });
...
assert.ok(state.effects.some((fx) => fx.type === 'bomberFuse'), 'fuse should emit visual warning through pushVisualEffect');
assert.equal(player.hp, 100, 'fuse must not damage player immediately');

state.time += 0.8;
updateEnemies(state, 0.8);
assert.ok(player.hp < 100, 'bomber explosion should damage player through the player damage pipeline');
assert.ok(!state.enemies[bomber.id], 'bomber should remove itself through enemy kill finalizer after exploding');
assert.ok(state.effects.some((fx) => fx.type === 'explosion' && fx.color === '#ff3048'), 'bomber explosion should emit red explosion visual');
...
assert.ok(enemyRenderSrc.includes('export const ENEMY_RENDERERS'), 'enemy renderer registry must exist');
assert.ok(enemyRenderSrc.includes('function drawCharger'), 'charger visual pass must live in enemy renderers');
assert.ok(enemyRenderSrc.includes('function drawBomber'), 'bomber visual pass must live in enemy renderers');

const server = read('server/server.js');
assert.ok(server.includes('const SERVER_VERSION = "v39.0.4"'), 'server version must match v39.0.4');
assert.ok(server.includes('const SERVER_BUILD_ID = "v39.0.4-20260520"'), 'server build must match v39.0.4 build');
...
assert.ok(index.includes('src/main.v39-0-4.js?v=39.0.4'), 'index must use current cache-busted entry');
assert.ok(index.includes('V39.0.4 | BUILD 20260520'), 'HUD must expose v39.0.4');
assert.ok(exists('src/main.v39-0-4.js'), 'current versioned entry must exist');
assert.ok(!exists('src/main.v39-0-3.js'), 'previous versioned entry must not ship');
for (const name of ['session', 'clientRuntime', 'hostRuntime', 'upgradeClient', 'devControls', 'releaseIntegrity']) {
  assert.ok(exists(`src/app/${name}.v39-0-4.js`), `current versioned app module missing: ${name}`);
  assert.ok(!exists(`src/app/${name}.v39-0-3.js`), `previous versioned app module should not ship: ${name}`);
}
...
console.log('v39.0.4 bomber enemy + visual pass checks passed');
