...
import { clearLocationRuntimeObjects } from '../src/game/runtimeReset.js';
...
test('kill combo dopamine moments are host-authoritative and economy rewards use playerEconomy', () => {
  ...
  assert.ok(state.events.some((event) => event.type === 'kill_combo' && event.nextRewardLabel), 'combo stack events should advertise the next reward so reward text stays visible at the bottom of the combo box');
  dealPlayerDamage(state, p, { amount: 999, sourceType: 'verify_death' });
  assert.ok(state.events.some((event) => event.type === 'kill_combo' && event.action === 'reset' && event.reason === 'death'), 'player death should reset the active kill combo');
  p.hp = p.maxHp;
  finishEnemyKill(state, disableArmor(spawnEnemy(state, 'grunt', p.x + 20, p.y)), { ownerId: p.id, kind: 'verifyProjectile' }, { sourceId: p.id, done: 1, killed: true });
  clearLocationRuntimeObjects(state);
  assert.ok(state.events.some((event) => event.type === 'kill_combo' && event.action === 'reset' && event.reason === 'room_end'), 'room transition/end reset should clear active kill combos through an authoritative reset event');
