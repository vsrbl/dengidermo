...
assert.ok(style.includes('font-size: clamp(12px, 1.25vw, 17px)') && style.includes('font-size: clamp(6px, 0.58vw, 8px)') && style.includes('min-height: 38px;') && style.includes('min-width: 66px;') && style.includes('align-content: center') && style.includes('align-self: center;') && style.includes('--combo-compact-top: clamp(154px'), 'combo counter must stay half-size, under the upgrade/install select area, and vertically centered inside the box after v39.3.22n');
