...
assert.ok(style.includes('font-size: clamp(13px, 1.34vw, 18px)') && style.includes('font-size: clamp(6px, 0.62vw, 8px)') && style.includes('min-height: 46px;') && style.includes('min-width: 78px;') && style.includes('padding: 5px 7px 6px;') && style.includes('align-content: center') && style.includes('align-self: center;') && style.includes('--combo-compact-top: clamp(184px'), 'combo counter must stay compact with small internal padding, under the upgrade/install select area, and vertically centered inside the box after v39.3.22p');
...
assert.ok(ui.includes('nextRewardLabel') && read('src/killComboFeed.js').includes('carryReward') && read('src/killComboFeed.js').includes('event.action === "reset"'), 'kill combo feed must persist reward text until combo fades and handle reset events from room end/death');
