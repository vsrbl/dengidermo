...
assert.equal(VERSION, 'v3.0.2');
assert.equal(BUILD_ID, 'locale_microcopy_driver_instant_block_vector');
...
for (const html of [index, fallbackPage]) {
  assert.match(html, /style\.css\?v=3\.0\.2/);
  assert.match(html, /main\.v2-1\.js\?v=3\.0\.2/);
}
...
assert.doesNotMatch(index + fallbackPage + main + hud + render + css, /game-3d|webgl3d|true-3d/i, 'v3.0.2 must stay on the stable 2D renderer');
...
console.log('v3.0.2 checks passed: exhaustive RU/EN chest paths, localized microcopy, and cooldown-free Driver block controls');
