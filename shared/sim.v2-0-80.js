...
} from './data.v2-0-80.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-80.js';
...
// v2.0.80: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function loopCostMul(run) {
  // v2.0.80: costs were far too flat. By loop 2 a player could sit on ~2k GLD
  // while WPN/ABL chests were still pocket change. Costs now scale faster than rewards.
  const depth = Math.max(0, run?.runDepth || 0);
  const loop = Math.max(0, Math.floor(depth / 4));
  const within = depth % 4;
  const base = Math.pow(2.35, loop) * (1 + within * 0.18);
  const late = Math.max(0, loop - 2);
  return Math.max(1, Math.min(260, base + late * late * 1.15));
}
...
function nextStaticRainLevel(run, players = null) {
  ...
  const debtEngine = (run.plan?.category === 'boss' || run.plan?.specialRoomId === 'chill_room') ? 0 : playerDebtEngineStacks(players);
  // While a seeding NATURAL STATIC RAIN room is still being played, show the real strike count.
  // Debt Engine itself is not a strike-counter cascade anymore; it contributes fixed stacks.
  const fromCurrentRoom = carry > 0 ? 0 : (run.staticRainCanSeedNext ? Math.max(0, run.roomStaticRainFalls || 0) : 0);
  return clampStaticRainLevel(debt + carry + debtEngine + fromCurrentRoom);
}
...
function roomObjectiveStatus(run, st = run?.roomStats || {}) {
  ...
  const time = roomSolvedTime(run, st);
  const solved = roomSolvedAt(run, st) > 0;
  // v2.0.80 rule: every room contract is finalised ONLY when the portal opens.
...
function rewardShellMarket(run, x, y) {
  // Shell Market modifier was removed in v2.0.80. Keep shell-break memory only.
...
function buildHeraldFloorPath(run, h, target) {
  ...
  const block = segmentBlockedByWalls(sx, sy, tx, ty, walls, 20);
  if (!block) return [[sx, sy], [tx, ty]];
  const pad = 58;
  const candidates = [
    [block.x - pad, block.y - pad], [block.x + block.w + pad, block.y - pad],
    [block.x + block.w + pad, block.y + block.h + pad], [block.x - pad, block.y + block.h + pad],
    [block.x - pad, ty], [block.x + block.w + pad, ty], [tx, block.y - pad], [tx, block.y + block.h + pad]
  ].map(([x, y]) => [Math.round(clamp(x, WALL_T + 35, run.plan.w - WALL_T - 35)), Math.round(clamp(y, WALL_T + 35, run.plan.h - WALL_T - 35))]);
  let best = null;
  for (const c of candidates) {
    const bad = segmentBlockedByWalls(sx, sy, c[0], c[1], walls, 18) || segmentBlockedByWalls(c[0], c[1], tx, ty, walls, 18);
    const score = Math.hypot(c[0] - sx, c[1] - sy) + Math.hypot(tx - c[0], ty - c[1]) + (bad ? 10000 : 0);
    if (!best || score < best.score) best = { c, score };
  }
  return best ? [[sx, sy], best.c, [tx, ty]] : [[sx, sy], [tx, ty]];
}
...
function movingWallDangerPenalty(run, x, y, half = 14) {
  ...
  for (const w of run.movingWalls) {
    ...
    if (insidePad) penalty += 1.2;
    const cx = w.x + w.w / 2, cy = w.y + w.h / 2;
    if (dist2(x, y, cx, cy) < (Math.max(w.w, w.h) + pad * 2) ** 2) penalty += 0.35;
  }
  return penalty;
}
...
function steerMove(run, e, dir, speedValue, dt, opts = {}) {
  ...
  for (const a of angles) {
    ...
    let score = moved * 1.3 - lookPenalty * 95 - spikePenalty * 120 - Math.abs(a) * 8;
...
function stepEnemySynergies(run, players, dt) {
  ...
  for (const d of dampers) {
    ...
    for (const e of run.enemies) {
      ...
      const near = dist2(e.x, e.y, d.x, d.y) < (def.fieldR + 260) ** 2;
      if (!near) continue;
      if (['shooter','prism','pulse','orbiter','echo'].includes(e.kind) && !e.escortAnchorId) {
        attachRotaryGuard(e, d, guarded, 5);
        guarded++;
      } else if (e.kind === 'herald') {
        e.preferredNestId = d.id;
        e.rallyT = Math.max(e.rallyT || 0, 1.2);
        guarded++;
      } else if (['runner','charger','tank'].includes(e.kind) && dist2(e.x, e.y, d.x, d.y) < (def.fieldR + 80) ** 2) {
...
export function startRoom(run, players) {
  ...
  const debtEngineStacks = (run.plan.category === 'boss' || run.plan.specialRoomId === 'chill_room') ? 0 : playerDebtEngineStacks(players);
  const incomingRainStacks = clampStaticRainLevel(debtStacks + carryStacks + debtEngineStacks);
  ...
  if ((run.plan.modifierIds || []).includes('casino_virus')) {
    run.casinoVirus = { spinsLeft: 3, nextSpin: 30, done: false, lastLabel: 'WAITING', activeRainStacks: 0, rainT: 0, spinSeq: 0, rainKind: '', lastSymbols: [] };
  }
  ...
  if (run.plan.category !== 'boss' && !isGreedRoom(run)) {
    ...
    // v2.0.80: skin rooms were too rare. Raise the base rate and add soft pity.
...
function director(run, players, dt) {
  ...
  if (casinoVirusActive) {
    // v2.0.80: normal director remains alive during the 3 virus spins, but is capped.
  ...
  for (const e of [...run.enemies]) {
    ...
    if (e.kind === 'shooter') {
      ...
      if ((e.summonWindT || 0) > 0) {
        ...
        if (e.fxT <= 0) {
          e.fxT = 0.055;
          run.fx.push({ t: 'herald_cast', id: e.id, target: target.id, x: Math.round(e.x), y: Math.round(e.y), x2: Math.round(target.x), y2: Math.round(target.y), points: buildHeraldFloorPath(run, e, target), p: Math.round(pfill * 100), dur: maxT });
        }
        if (e.summonWindT <= 0) {
        ...
        if (e.summonCd <= 0 && run.enemies.length < difficulty(run).maxActive && dT < 760) {
          ...
          e.fxT = 0;
          run.fx.push({ t: 'herald_cast', id: e.id, target: target.id, x: Math.round(e.x), y: Math.round(e.y), x2: Math.round(target.x), y2: Math.round(target.y), points: buildHeraldFloorPath(run, e, target), p: 0, dur: e.summonWindMax, start: 1 });
        }
      }
      e.dirX = toT.x; e.dirY = toT.y;
    } else if (e.kind === 'boss') {
...
function spawnCasinoVirusEvent(run, players) {
  ...
  if (roll < 0.26) {
    ...
    if (!(cv.activeRainStacks > 0)) cv.rainT = 0.18;
    cv.activeRainStacks = Math.max(cv.activeRainStacks || 0, 2);
    cv.rainKind = cv.activeRainStacks >= 5 ? 'big' : 'static';
    run.fx.push({ t: 'contract_done', label: 'CASINO VIRUS', body: 'ROOM STATIC RAIN STARTED' });
  } else if (roll < 0.66) {
    label = 'BIG STATIC RAIN';
    // BIG STATIC RAIN creates a full-room enhanced rain even when no previous virus rain exists.
    // If a smaller virus rain already exists, it is upgraded to the big version.
    if (!(cv.activeRainStacks > 0)) cv.rainT = 0.12;
    cv.activeRainStacks = Math.max(cv.activeRainStacks || 0, 5);
    cv.rainKind = 'big';
    run.fx.push({ t: 'contract_done', label: 'CASINO VIRUS', body: 'BIG ROOM STATIC RAIN STARTED' });
  } else if (roll < 0.80) {
  ...
  const symbols = casinoVirusSymbols(label);
  cv.lastLabel = label; cv.lastSymbols = symbols; cv.spinSeq = (cv.spinSeq || 0) + 1;
  run.fx.push({ t: 'casino_virus_spin', label, symbols, spinsLeft: cv.spinsLeft, nextSpin: Math.round(cv.nextSpin || 0), seq: cv.spinSeq, x: Math.round(center.x), y: Math.round(center.y), rainStacks: cv.activeRainStacks || 0, rainKind: cv.rainKind || '' });
...
function stepCasinoVirus(run, players, dt) {
  ...
  const cv = run.casinoVirus;
  if (cv.spinsLeft > 0) {
    cv.nextSpin = Math.max(0, (Number.isFinite(cv.nextSpin) ? cv.nextSpin : 30) - dt);
  ...
  if (run.plan.modifierIds.includes('anchor_gravity') && run.roomSockets?.length) {
    for (const s of run.roomSockets) {
      ...
      for (const p of players.values()) {
        ...
        const force = 78 * (1 - Math.sqrt(d) / s.r);
        const c = collideWalls(p.x + n.x * force * dt, p.y + n.y * force * dt, PLAYER_SIZE / 2, run.plan.walls, p.x, p.y);
        p.x = c.x; p.y = c.y; p.slowT = Math.max(p.slowT || 0, 0.10); p.slowMul = Math.min(p.slowMul || 1, 0.82);
      }
      for (const e of run.enemies) {
      ...
      for (const b of run.bullets) {
        ...
        if (d > (s.r + 120) ** 2 || d < 16) continue;
        const n = norm(s.x - b.x, s.y - b.y);
        const pull = b.from === 'p' ? 46 : 58;
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.80: VOID LASER is now a builder. Level I = one short segment.
