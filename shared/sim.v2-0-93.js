...
} from './data.v2-0-93.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-93.js';
...
const HERALD_PATH_FOLLOW_SPEED = 235; // v2.0.93: floor call-line follows dashes, but never teleports onto the player.
const clampStaticRainLevel = v => Math.max(0, Math.min(STATIC_RAIN_MAX_LEVEL, v | 0));


// v2.0.93: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function loopCostMul(run) {
  // v2.0.93: all prices scale about 1.5x faster than v2.0.93.
...
function rewardShellMarket(run, x, y) {
  // Shell Market modifier was removed in v2.0.93. Keep shell-break memory only.
...
export function startRoom(run, players) {
  ...
  if ((run.plan.modifierIds || []).includes('casino_virus')) {
    run.casinoVirus = { spinsLeft: 3, nextSpin: 6, done: false, lastLabel: 'WAITING', activeRainStacks: 0, rainT: 0, spinSeq: 0, rainKind: '', lastSymbols: [], pendingEvent: null };
  }
  ...
  if (run.plan.category !== 'boss' && !isGreedRoom(run)) {
    ...
    // v2.0.93: skin rooms were too rare. Raise the base rate and add soft pity.
...
function director(run, players, dt) {
  ...
  if (casinoVirusActive) {
    // v2.0.93: normal director remains alive during the 3 virus spins, but is capped.
...
function stepEnemies(run, players, dt) {
  ...
  for (const e of [...run.enemies]) {
    ...
    if (e.kind === 'shooter') {
      ...
      // v2.0.93: DMP is no longer a static turret. It slowly walks the nest toward the player,
      ...
      if ((e.summonWindT || 0) > 0) {
        ...
        // v2.0.93: the floor line is a pursuing signal, not a hitscan reticle.
...
function tryCleanupPortal(run) {
  ...
  if (hasMod(run, 'casino_virus')) {
    const cv = run.casinoVirus;
    if (cv && !cv.pendingEvent && (cv.spinsLeft || 0) <= 0 && liveEnemyCount(run) <= 0) {
      cv.done = true;
      openPortal(run);
    }
    return;
...
export function handleDevCommand(run, players, p, cmd = {}) {
  ...
  if (action === 'clear_enemies') {
    ...
    while (run.enemies.length && guard++ < 500) {
      ...
      for (const e of batch) if (run.enemies.includes(e)) killEnemy(run, players, e, p);
    }
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.93: VOID LASER is now a builder. Level I = one short segment.
