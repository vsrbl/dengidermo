...
} from './data.v2-0-72.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-72.js';
...
// v2.0.72: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function roomObjectiveStatus(run, st = run?.roomStats || {}) {
  ...
  const time = roomSolvedTime(run, st);
  const solved = roomSolvedAt(run, st) > 0;
  // v2.0.72 rule: every room contract is finalised ONLY when the portal opens.
...
function makeNextRoomPreview(run) {
  ...
  const plan = generateRoom(seed, depth, loop);
...
export function createRun(seedBase) {
  return {
    ...
    roomStats: null, roomObjective: null, roomObjectiveSettlement: null, roomObjectiveFrozenStats: null, nextRoomPreview: null, roomSockets: [], roomWires: [], prismLaneT: 0, pendingPrismLanes: [], pendingBloodTax: [], portalOpenedAt: 0, huntedExitOpenedAt: 0, huntedExitSpawnT: 0,
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss') {
    ...
    // v2.0.72: skin rooms were too rare. Raise the base rate and add soft pity.
...
export function resetRun(run, players) {
  ...
  run.staticRainFromPending = false;
  run.roomStaticRainFalls = 0;
  run.roomStats = null; run.roomObjective = null; run.roomObjectiveSettlement = null; run.roomObjectiveFrozenStats = null; run.nextRoomPreview = null; run.roomSockets = []; run.roomWires = []; run.pendingPrismLanes = []; run.pendingBloodTax = []; run.portalOpenedAt = 0; run.huntedExitOpenedAt = 0; run.huntedExitSpawnT = 0;
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.72: VOID LASER is now a builder. Level I = one short segment.
