...
} from './data.v2-0-43.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-43.js';
...
// v2.0.43: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function applyBulletElements(run, players, e, b, strength = 1) {
  if (!e || e.hp <= 0 || !b || !b.elem) return;
  const elem = String(b.elem || '');
  const owner = b.owner || null;
  const p = owner ? players.get(owner) : null;
  const fireStacks = p?.stats?.bulletFire || (elem.includes('fire') ? 1 : 0);
  const freezeStacks = p?.stats?.bulletFreeze || (elem.includes('freeze') ? 1 : 0);
  const poisonStacks = p?.stats?.bulletPoison || (elem.includes('poison') ? 1 : 0);
  const power = Math.max(0.5, Number(b.elemPower || 1)) * strength;
  ...
  if (elem.includes('freeze')) {
    const chill = 0.50 + freezeStacks * 0.08 * power;
    e.activeSlowT = Math.max(e.activeSlowT || 0, chill + ((e.poisonT || 0) > 0 ? 0.18 : 0));
    e.activeSlowMul = Math.min(e.activeSlowMul || 1, (e.poisonT || 0) > 0 ? 0.58 : 0.70);
    const chance = Math.min(0.42, 0.12 + freezeStacks * 0.055 + ((e.poisonT || 0) > 0 ? 0.06 : 0));
    if (Math.random() < chance * Math.min(1.15, power)) activeFreezeEnemy(run, e, 0.13 + freezeStacks * 0.025 + ((e.poisonT || 0) > 0 ? 0.04 : 0));
  }
...
function applyWeaponChain(run, players, startEnemy, b) {
  ...
  let from = { x: startEnemy.x, y: startEnemy.y, id: startEnemy.id };
  let dmg = Math.max(1, (b.dmg || 4) * 0.56);
  const baseRange = Math.min(420, 120 + level * 3.0);
  let jumps = 0;
  for (let i = 0; i < level; i++) {
    ...
    for (const e of run.enemies) {
      ...
      if (d < bd && d <= (baseRange + e.size / 2) ** 2) { bd = d; best = e; }
    }
    if (!best) break;
    visited.add(best.id);
    const n = norm(best.x - from.x, best.y - from.y);
    run.fx.push({ t: 'active_line', kind: 'weapon_chain', x1: Math.round(from.x), y1: Math.round(from.y), x2: Math.round(best.x), y2: Math.round(best.y), width: 7, tone: 'cyan' });
    run.fx.push({ t: 'impact', x: Math.round(best.x), y: Math.round(best.y), kind: 'bullet_chain', dx: Math.round(n.x * 100), dy: Math.round(n.y * 100) });
    damageEnemy(run, players, best, dmg, b.owner, (b.knock || 0) * 0.25, n.x, n.y);
    if (best.hp > 0 && b.elem) applyBulletElements(run, players, best, b, Math.max(0.25, 0.62 * Math.pow(0.96, i)));
    jumps++;
    from = { x: best.x, y: best.y, id: best.id };
    dmg = Math.max(1, dmg * 0.86);
  }
  if (jumps) run.fx.push({ t: 'active_mutation', label: `WPN CHAIN x${jumps}`, x: Math.round(startEnemy.x), y: Math.round(startEnemy.y), r: Math.round(80 + Math.min(170, jumps * 10)), tone: 'cyan', owner: b.owner || '' });
}
...
function explode(run, players, x, y, r, dmg, owner, hurtPlayers = false, style = 'blast', elem = '', elemPower = 0) {
  ...
  for (const e of [...run.enemies]) {
    if (dist2(e.x, e.y, x, y) < (r + e.size / 2) ** 2) {
      ...
      if (e.hp > 0 && elem) applyBulletElements(run, players, e, { owner, dmg, elem, elemPower }, 0.58);
    }
  }
  if (hurtPlayers) {
    for (const p of players.values()) {
      ...
      for (const e of run.enemies) {
        if (dist2(e.x, e.y, b.x, b.y) < ((e.size + b.size) / 2 + 4) ** 2) {
          if (b.aoe) { explode(run, players, b.x, b.y, b.aoe, b.dmg, b.owner, false, (b.kind === 'rocketgun' || b.mine) ? 'rocket' : 'blast', b.elem || '', b.elemPower || 0); rocketAftermath(run, players, b); }
            ...
            if (e.hp > 0) applyBulletElements(run, players, e, b, 1);
...
function makeUpgradeCoreChoice(p) {
  ...
  return addActiveChoiceMeta({
    ...
    preview: a.core === 'signal_spike' ? `Level ${a.level + 1}: +1 заряд SPIKE. Длительность и урон зоны растут мягко, без взрыва баланса.` : `Level ${a.level + 1}: ${c.upgrade?.join(' · ') || '+сила'}. Прирост должен быть заметен в бою.`,
...
function makeAbilityChestChoices(p, rng = Math.random) {
  ...
  if (a.core === 'signal_spike' || a.level < 3) choices.push(makeUpgradeCoreChoice(p));
...
function activeCoreDesc(p) {
  ...
  const chargeLine = a.core === 'signal_spike' ? `\nCHARGES: ${ensureSignalSpikeCharges(p)}/${signalSpikeMaxCharges(p)}. Каждый апгрейд SIGNAL SPIKE добавляет ещё один заряд; длительность и урон растут мягко.` : '';
