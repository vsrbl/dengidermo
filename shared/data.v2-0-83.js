...
export const UPGRADES = [
  ...
  { id: 'sek_swarm', label: 'SEK SWARM RMB', tier: 1, branch: 'SEK', desc: 'ПКМ выпускает рой самонаводящихся SEK-пуль сразу. Пули распределяются по разным врагам, а не летят одним веером в одну цель. Каждый стак: +5 пуль и чуть дольше cooldown.', apply: s => { s.sekSwarm += 1; } },
  ...
  { id: 'debtengine',label: 'DEBT ENGINE',         tier: 2, cursed: true, desc: 'Постоянный cursed-двигатель до конца забега: большой урон и удача, но каждая eligible combat-комната получает отдельный слой DEBT ENGINE: +STATIC RAIN LVL 1 THIS ROOM за стак. Он не заменяет основной modifier комнаты и не разгоняется по числу ударов.', apply: s => { s.dmgMul *= 1.35; s.luck += 2; s.debtEngine += 1; } },
...
export function spinCasino(rng, stakeKey, luck, unlockedSkins = []) {
  ...
  const r = rng();
  // v2.0.83: skins were opening too slowly. SKN casino outcomes are now visible
