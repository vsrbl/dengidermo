...
} from './data.v2-0-33.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-33.js';
...
// v2.0.33: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function explode(run, players, x, y, r, dmg, owner, hurtPlayers = false, style = 'blast') {
  ...
  for (const e of [...run.enemies]) {
    if (dist2(e.x, e.y, x, y) < (r + e.size / 2) ** 2) damageEnemy(run, players, e, activeScale(dmg), owner, 0, 0, 0);
  }
  if (hurtPlayers) {
    for (const p of players.values()) {
...
function activeShrapnel(run, p, x, y, power = 1) {
  ...
  for (let i = 0; i < n && run.bullets.length < MAX_BULLETS; i++) {
    ...
    run.bullets.push({ id: nid(), x, y, vx: Math.cos(a) * 520, vy: Math.sin(a) * 520, dmg: activeScale(7 + power * 2) * p.stats.dmgMul, from: 'p', owner: p.id, life: 0.75 * rangeMul, size: 4, proc: p.stats.procBlast * 0.35, kind: 'active_shrapnel', travelled: 0, maxDist: Math.round(450 * rangeMul), bounces: p.stats.bulletBounce || 0, rangeMul });
  }
...
function activeNeedles(run, p, x, y, count, speed, dmg, kind = 'active_noise') {
  ...
  for (let i = 0; i < n && run.bullets.length < MAX_BULLETS; i++) {
    ...
    run.bullets.push({ id: nid(), x, y, vx: Math.cos(a) * speed, vy: Math.sin(a) * speed, dmg: activeScale(dmg) * p.stats.dmgMul, from: 'p', owner: p.id, life: 0.62 * rangeMul, size: 3 + Math.min(3, n / 8), proc: p.stats.procBlast * 0.25, kind, travelled: 0, maxDist: Math.round(380 * rangeMul), bounces: p.stats.bulletBounce || 0, rangeMul });
  }
}
  ...
  for (const id of mods) {
    if (id === 'static') {
      ...
      for (const e of [...targets]) {
        ...
        e.activeSlowMul = Math.min(e.activeSlowMul || 1, activeSoftMul(0.42));
  ...
  if (core === 'blood_ring') {
    ...
    for (const b of run.bullets) {
      ...
      if (dist2(b.x, b.y, p.x, p.y) < (ctx.r + b.size) ** 2) {
        b.vx *= 0.22; b.vy *= 0.22; b.life = Math.min(b.life, 1.2 + lvl * 0.32); cut++;
        if (lvl >= 3 && Math.random() < 0.20) { b.from = 'p'; b.owner = p.id; b.dmg = 12 * p.stats.dmgMul; }
      }
    }
    for (const e of [...activeTargets(run, p.x, p.y, ctx.r)]) { e.activeSlowT = Math.max(e.activeSlowT || 0, 0.42); e.activeSlowMul = Math.min(e.activeSlowMul || 1, activeSoftMul(0.20 + lvl * 0.03)); ctx.hitCount++; }
    ctx.hitCount += cut;
    ...
    for (const e of [...run.enemies]) if (distToSegment2(e.x, e.y, startX, startY, end.x, end.y) < (width + e.size / 2) ** 2) {
      ...
      e.activeSlowMul = Math.min(e.activeSlowMul || 1, activeSoftMul(0.38));
    ...
    let erased = 0;
    for (const b of run.bullets) if (b.from === 'e' && dist2(b.x, b.y, p.x, p.y) < (ctx.r + b.size) ** 2) { b.vx *= 0.18; b.vy *= 0.18; if (Math.random() < 0.36 + lvl * 0.08) { b.life = -1; erased++; } }
    for (const e of [...activeTargets(run, p.x, p.y, ctx.r)]) { e.activeSlowT = Math.max(e.activeSlowT || 0, 0.62); e.activeSlowMul = Math.min(e.activeSlowMul || 1, activeSoftMul(0.18 + lvl * 0.03)); e.fireCd = Math.max(e.fireCd || 0, 0.75 + lvl * 0.16); ctx.hitCount++; }
  ...
  for (const f of [...run.activeFields]) {
    ...
    if (f.kind === 'static' || f.kind === 'red_static' || f.kind === 'freeze_aura' || f.kind === 'signal_spike' || f.kind === 'black_box' || f.kind === 'void_tear' || f.kind === 'void_line' || f.kind === 'anchor_field') {
      ...
      for (const b of run.bullets) if (dist2(b.x, b.y, f.x, f.y) < (f.r + b.size) ** 2) {
        const damp = Math.pow(activeSoftMul(f.damp || 0.45), dt * (f.kind === 'freeze_aura' ? 5.2 : f.kind === 'black_box' ? 6.2 : 3.0));
        b.vx *= damp; b.vy *= damp;
