...
} from './data.v2-0-50.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-50.js';
...
// v2.0.50: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // Deliberately NOT activeScale()'d: VOID LASER upgrades must be visibly readable.
    // Lvl I is shorter on purpose; each upgrade adds a big step in reach and uptime.
    const laserLen = Math.round(380 + Math.min(2600, extra * 260));
    ...
    const ttl = Math.min(9.0, 2.55 + extra * 1.10);
    ...
    run.fx.push({ t: 'active_line', kind: 'void_laser', id: p.id, label: `VOID LASER ${roman(lvl)}${cut ? ' / ERASE ' + cut : ''}`, x1: Math.round(startX), y1: Math.round(startY), x2: Math.round(end.x), y2: Math.round(end.y), width: visualWidth, hitWidth: width, laserLen, ttl, tone: 'purple' });
