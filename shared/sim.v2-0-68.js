...
} from './data.v2-0-68.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-68.js';
...
// v2.0.68: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss') {
    ...
    // v2.0.68: skin rooms were too rare. Raise the base rate and add soft pity.
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.68: VOID LASER is now a builder. Level I = one short segment.
