...
} from './data.v2-0-100.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-100.js';
...
const HERALD_PATH_FOLLOW_SPEED = 145; // v2.0.100: herald call-line reroutes slowly; dash changes the route, it does not cancel the line.
const clampStaticRainLevel = v => Math.max(0, Math.min(STATIC_RAIN_MAX_LEVEL, v | 0));


// v2.0.100: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function loopCostMul(run) {
  // v2.0.100: all prices scale about 1.5x faster than v2.0.100.
...
function rewardShellMarket(run, x, y) {
  // Shell Market modifier was removed in v2.0.100. Keep shell-break memory only.
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss' && !isGreedRoom(run)) {
    ...
    // v2.0.100: skin rooms were too rare. Raise the base rate and add soft pity.
...
function director(run, players, dt) {
  ...
  if (casinoVirusActive) {
    // v2.0.100: normal director remains alive during the 3 virus spins, but is capped.
...
function stepEnemies(run, players, dt) {
  ...
  for (const e of [...run.enemies]) {
    ...
    if (e.kind === 'shooter') {
      ...
      // v2.0.100: DMP is no longer a static turret. It slowly walks the nest toward the player,
      ...
      if ((e.summonWindT || 0) > 0) {
        ...
        // v2.0.100: the floor line is a pursuing signal, not a hitscan reticle.
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.100: VOID LASER is now a builder. Level I = one short segment.
