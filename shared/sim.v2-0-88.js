...
} from './data.v2-0-88.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-88.js';
...
const HERALD_PATH_FOLLOW_SPEED = 235; // v2.0.88: floor call-line follows dashes, but never teleports onto the player.
const clampStaticRainLevel = v => Math.max(0, Math.min(STATIC_RAIN_MAX_LEVEL, v | 0));


// v2.0.88: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function loopCostMul(run) {
  // v2.0.88: costs were far too flat. By loop 2 a player could sit on ~2k GLD
...
function roomObjectiveForPlan(plan = {}, depth = 0) {
  ...
  if (mods.includes('casino_virus')) return { id: 'virus_clear', label: 'VIRUS CLEAN', reward: 'FAVOR', goal: 'Survive 3 virus spins, then clear the room.' };
  if (mods.includes('prism_grid')) return { id: 'grid_slow_clear', label: 'GRID WALKER', reward: 'FAVOR', goal: 'Clear the slow-grid room.' };
  if (mods.includes('blood_tax')) return { id: 'blood_paid', label: 'BLOOD PAID', reward: 'FAVOR', goal: 'Buy with HP and clear the room.' };
  if (mods.includes('static_rain')) return { id: 'static_clean', label: 'STATIC CLEAN', reward: 'FAVOR', goal: 'Clear with low damage taken.' };
  if (mods.includes('skin_cache')) return { id: 'cache_claim', label: 'CACHE CLAIM', reward: 'FAVOR', goal: 'Clear and claim the cache.' };
  if (arch === 'panic_box' || arch === 'compact') return { id: 'fast_clear', label: 'FAST CLEAN', reward: 'FAVOR', goal: 'Clear fast.' };
  if (arch === 'wide' || arch === 'long_lane') return { id: 'no_hit', label: 'NO HIT TAPE', reward: 'FAVOR', goal: 'Clear without taking damage.' };
  return { id: 'clean_signal', label: 'CLEAN SIGNAL', reward: 'FAVOR', goal: 'Clear the room.' };
}
...
function activatePendingContractFavors(run) {
  ...
  for (const f of incoming) {
    if (f.id === 'clear_debt') {
      ...
      run.fx.push({ t: 'favor_used', id: f.id, label: favorLabel(f), body: ok ? 'NEXT ROOM DEBT CLEARED' : 'NO DEBT TO CLEAR' });
    }
  }
  if (incoming.length) run.fx.push({ t: 'favor_active', favors: incoming.map(f => ({ id: f.id, label: favorLabel(f), tier: f.tier, uses: Math.max(0, (f.uses || 0) - (f.used || 0)) })) });
}
function contractFavorPool(chain = 1) {
  if (chain >= 4) return ['epic_reroll', 'portal_insurance', 'double_favor', 'skin_signal'];
  if (chain >= 2) return ['free_reroll', 'clear_debt', 'skin_signal', 'portal_insurance'];
  return ['free_reroll', 'clear_debt', 'skin_signal'];
}
function rollContractFavor(run, chain = 1, slot = 0) {
  const pool = contractFavorPool(chain);
...
function grantContractFavors(run, chain = 1, count = 1) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (used.has(f.id)) {
      const pool = contractFavorPool(chain).filter(id => !used.has(id));
  ...
  run.fx.push({ t: 'favor_earned', favors: favors.map(f => ({ id: f.id, label: favorLabel(f), tier: f.tier, uses: f.uses })) });
  return favors;
}
function contractFavorSnapshot(run) {
  return {
    active: activeContractFavors(run).map(f => ({ id: f.id, label: favorLabel(f), labelRu: f.labelRu || '', tier: f.tier || 'common', uses: Math.max(0, (f.uses || 0) - (f.used || 0)), desc: favorDef(f.id)?.desc || '', nextRoomOnly: 1 })),
    pending: pendingContractFavors(run).map(f => ({ id: f.id, label: favorLabel(f), labelRu: f.labelRu || '', tier: f.tier || 'common', uses: Math.max(0, f.uses || 0), desc: favorDef(f.id)?.desc || '', nextRoomOnly: 1 })),
...
function roomObjectiveDoneRaw(obj, run, st = {}, time = 0) {
  ...
  if (obj.id === 'blood_paid') return (st.bloodTaxes || 0) > 0;
...
function roomObjectiveEarlyFailReason(obj, run, st = run?.roomStats || {}) {
  ...
  if (obj.id === 'no_hit' && (st.damageTaken || 0) > 0) return 'DAMAGE TAKEN';
  if (obj.id === 'static_clean' && (st.damageTaken || 0) > lowDamageLimit) return 'TOO MUCH DMG';
  if (obj.id === 'fast_clear' && time >= 42 && roomSolvedAt(run, st) <= 0) return 'TIME LOST';
  return '';
}
function roomObjectiveEarlyDoneRaw(obj, run, st = run?.roomStats || {}) {
  if (!obj) return false;
  if (obj.id === 'hunter_waves') return !!run?.hunterWave?.done;
  if (obj.id === 'virus_clear') return !!run?.casinoVirus?.done;
  if (obj.id === 'fast_clear') return roomSolvedAt(run, st) > 0 && roomSolvedTime(run, st) < 42;
  if (obj.id === 'clean_signal') return run?.kills >= (run?.plan?.quota || 0);
...
function rewardShellMarket(run, x, y) {
  // Shell Market modifier was removed in v2.0.88. Keep shell-break memory only.
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss' && !isGreedRoom(run)) {
    ...
    // v2.0.88: skin rooms were too rare. Raise the base rate and add soft pity.
    ...
    if (hasActiveContractFavor(run, 'skin_signal')) skinChance += 0.35;
...
function director(run, players, dt) {
  ...
  if (casinoVirusActive) {
    // v2.0.88: normal director remains alive during the 3 virus spins, but is capped.
...
function stepEnemies(run, players, dt) {
  ...
  for (const e of [...run.enemies]) {
    ...
    if (e.kind === 'shooter') {
      ...
      // v2.0.88: DMP is no longer a static turret. It slowly walks the nest toward the player,
      ...
      if ((e.summonWindT || 0) > 0) {
        ...
        // v2.0.88: the floor line is a pursuing signal, not a hitscan reticle.
...
function stepMovingWalls(run, players, dt) {
  ...
  for (const w of run.movingWalls) {
    ...
    for (const e of [...run.enemies]) if (pushCircleOutOfRect(e, (e.size || 24) / 2 + 8, w)) {
      // Walls are a pressure tool, not a free enemy grinder. Mobs take only
      // occasional chip damage and are strongly nudged away.
      e.wallScrapeCd = Math.max(0, (e.wallScrapeCd || 0) - dt);
      const cx = w.x + w.w / 2, cy = w.y + w.h / 2;
      const n = norm(e.x - cx, e.y - cy);
      e.x += n.x * 36 * dt; e.y += n.y * 36 * dt;
      if (e.wallScrapeCd <= 0) {
        damageEnemy(run, players, e, 5 + Math.floor((run.runDepth || 0) * 0.35), null, 0, 0, 0);
        e.wallScrapeCd = 0.70;
      }
    }
  }
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.88: VOID LASER is now a builder. Level I = one short segment.
...
export function buildSnapshot(run, players) {
  ...
  return {
    ...
    room: {
      ...
      runMemory: { ...(run.runMemory || {}) }, tapeLog: (run.tapeLog || []).slice(0, 10), skinPity: run.skinPity || 0, contractFavors: contractFavorSnapshot(run)
    },
