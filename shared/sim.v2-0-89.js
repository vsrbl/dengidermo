...
} from './data.v2-0-89.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-89.js';
...
const HERALD_PATH_FOLLOW_SPEED = 235; // v2.0.89: floor call-line follows dashes, but never teleports onto the player.
const clampStaticRainLevel = v => Math.max(0, Math.min(STATIC_RAIN_MAX_LEVEL, v | 0));


// v2.0.89: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function loopCostMul(run) {
  // v2.0.89: costs were far too flat. By loop 2 a player could sit on ~2k GLD
...
function roomTip(plan = {}, staticLevel = 0, staticMode = '') {
  ...
  if (mods.includes('moving_room')) return 'SHIFTING ZONES: hollow danger zones move, slow and lightly damage.';
...
function roomObjectiveForPlan(plan = {}, depth = 0) {
  ...
  if (arch === 'panic_box' || arch === 'compact') return { id: 'fast_clear', label: 'FAST CLEANUP', reward: 'FAVOR', goal: 'Kill every enemy before 42s.' };
...
function roomObjectiveProgress(run, st = run?.roomStats || {}) {
  ...
  const progress = {
    ...
    virus_clean: run?.casinoVirus ? `${Math.max(0, run.casinoVirus.spinsLeft || 0)} SPINS LEFT · ${Math.max(0, run.enemies?.length || 0)} LEFT` : '3 SPINS',
    wire_ghost: (st.wireTouches || 0) <= 0 ? 'CLEAN' : `${st.wireTouches} TOUCH`,
    grid_ghost: (st.prismHits || 0) <= 0 ? 'CLEAN' : `${st.prismHits} HIT`,
    grid_slow_clear: `${Math.max(0, run?.enemies?.length || 0)} LEFT`,
    blood_paid: `${Math.max(0, run?.enemies?.length || 0)} LEFT${isBloodTaxRoom(run) ? ' · HP PRICES' : ''}`, 
    static_clean: `${Math.round(st.damageTaken || 0)}/${lowDamageLimit} DMG`,
    cache_claim: `${Math.max(0, run?.enemies?.length || 0)} LEFT`,
    fast_clear: `${Math.round(time)}/42s · ${Math.max(0, run?.enemies?.length || 0)} LEFT`,
    no_hit: (st.damageTaken || 0) <= 0 ? 'CLEAN' : `${Math.round(st.damageTaken || 0)} DMG`,
    clean_signal: `${Math.max(0, run?.enemies?.length || 0)} LEFT`
  };
...
const CONTRACT_FAVOR_DEFS = {
  ...
  clear_debt: { id: 'clear_debt', label: 'CLEAR STATIC STACK', labelRu: 'СНЯТЬ СТАТИК-СТАК', tier: 'common', uses: 1, desc: 'Removes one banked Static Storm stack before the next room starts.' },
  skin_signal: { id: 'skin_signal', label: 'SKIN CACHE SIGNAL', labelRu: 'СИГНАЛ СКИН-ТАЙНИКА', tier: 'rare', uses: 1, desc: 'Raises hidden skin-cache chance in the next room only.' },
...
function activatePendingContractFavors(run) {
  ...
  for (const f of incoming) {
    if (f.id === 'clear_debt') {
      ...
      run.fx.push({ t: 'favor_used', id: f.id, label: favorLabel(f), body: ok ? 'NEXT ROOM STATIC STACK CLEARED' : 'NO STATIC STACK TO CLEAR' });
    }
  }
...
function roomObjectiveDoneRaw(obj, run, st = {}, time = 0) {
  ...
  if (obj.id === 'fast_clear') return time > 0 && time < 42 && !roomHasLiveEnemies(run);
...
function roomObjectiveEarlyDoneRaw(obj, run, st = run?.roomStats || {}) {
  ...
  if (obj.id === 'fast_clear') return roomSolvedAt(run, st) > 0 && roomSolvedTime(run, st) < 42 && !roomHasLiveEnemies(run);
...
function rewardShellMarket(run, x, y) {
  // Shell Market modifier was removed in v2.0.89. Keep shell-break memory only.
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss' && !isGreedRoom(run)) {
    ...
    // v2.0.89: skin rooms were too rare. Raise the base rate and add soft pity.
...
function director(run, players, dt) {
  ...
  if (casinoVirusActive) {
    // v2.0.89: normal director remains alive during the 3 virus spins, but is capped.
...
function stepEnemies(run, players, dt) {
  ...
  for (const e of [...run.enemies]) {
    ...
    if (e.kind === 'shooter') {
      ...
      // v2.0.89: DMP is no longer a static turret. It slowly walks the nest toward the player,
      // while keeping enough distance to remain a protection core instead of a body-blocker.
      const desired = dT > 560 ? 0.52 : dT < 390 ? -0.20 : 0.04;
      const side = { x: -toT.y * 0.06, y: toT.x * 0.06 };
      const drift = { x: toT.x * desired + side.x, y: toT.y * desired + side.y };
      steerMove(run, e, drift, spd * (desired < 0 ? 0.42 : 0.55), dt, { target });
      e.dirX = toT.x; e.dirY = toT.y;
      e.fxT = (e.fxT || 0) - dt;
      if (e.fxT <= 0) {
        e.fxT = 0.18;
      ...
      if ((e.summonWindT || 0) > 0) {
        ...
        // v2.0.89: the floor line is a pursuing signal, not a hitscan reticle.
...
function spawnCasinoVirusEvent(run, players) {
  ...
  if (roll < 0.26) {
    ...
    run.fx.push({ t: 'contract_done', label: 'CASINO VIRUS', body: 'ROOM STATIC STORM STARTED' });
  } else if (roll < 0.66) {
    label = 'BIG STATIC STORM';
    // BIG STATIC STORM creates a full-room enhanced rain even when no previous virus rain exists.
    ...
    cv.rainKind = 'big';
    run.fx.push({ t: 'contract_done', label: 'CASINO VIRUS', body: 'BIG ROOM STATIC STORM STARTED' });
  } else if (roll < 0.80) {
...
function stepMovingWalls(run, players, dt) {
  ...
  for (const w of run.movingWalls) {
    ...
    for (const p of players.values()) if (p.alive && rectHitCircle(p.x, p.y, PLAYER_SIZE / 2, w)) {
      p.slowT = Math.max(p.slowT || 0, 0.22);
      p.slowMul = Math.min(p.slowMul || 1, 0.56);
      if (w.dmgCd <= 0) { damagePlayer(run, p, 5 + Math.floor((run.runDepth || 0) * 0.25), w.x + w.w / 2, w.y + w.h / 2); w.dmgCd = 0.85; }
    }
    for (const e of run.enemies) if (e.hp > 0 && rectHitCircle(e.x, e.y, (e.size || 24) / 2, w)) {
      e.activeSlowT = Math.max(e.activeSlowT || 0, 0.30);
...
function openChest(run, players, p, o) {
  ...
  if (o.chest === 'basic_chest') {
    ...
    p.hp = Math.min(p.hp, maxHp(p));
    addStaticDebt(run);
    rewards.push(u.label, 'CURSE: STATIC STACK');
  }
...
function beginTransition(run, players) {
  ...
  const fast = time > 0 && time < 42 && !roomHasLiveEnemies(run) ? 1 : 0;
  const tapes = [];
...
function buildActiveCasinoRoll(run, p, ctx) {
  ...
  else if (r < 0.18) { outcome = 'DEBT'; label = 'STATIC STACK'; tone = 'purple'; symbols = ['STC', 'STACK', 'Q']; }
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.89: VOID LASER is now a builder. Level I = one short segment.
