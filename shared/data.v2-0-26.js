...
export function spinCasino(rng, stakeKey, luck, unlockedSkins = []) {
  ...
  else if (r < 0.055 + l * 0.9) outcome = 'WPN';
  else if (r < 0.105 + l * 0.95) outcome = 'ABL';
  else if (r < 0.155 + l * 0.65) outcome = 'SKN';
  else if (r < 0.265 + l) outcome = 'HEA';
  else if (r < 0.415 + l) outcome = 'EXP';
  else if (r < 0.585 + l) outcome = 'GLD';
  else if (r < 0.70) outcome = 'STC';
  else outcome = 'LOSE';
  const sym = () => ['GLD','HEA','EXP','WPN','ABL','SKN','STC'][Math.floor(rng()*7)];
  let symbols;
