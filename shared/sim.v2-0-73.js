...
} from './data.v2-0-73.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-73.js';
...
// v2.0.73: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function roomObjectiveStatus(run, st = run?.roomStats || {}) {
  ...
  const time = roomSolvedTime(run, st);
  const solved = roomSolvedAt(run, st) > 0;
  // v2.0.73 rule: every room contract is finalised ONLY when the portal opens.
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss') {
    ...
    // v2.0.73: skin rooms were too rare. Raise the base rate and add soft pity.
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.73: VOID LASER is now a builder. Level I = one short segment.
