...
export const UPGRADES = [
  ...
  { id: 'bullet_ricochet', label: 'BULLET RICOCHET +1', tier: 1, branch: 'ALL', desc: 'Обычные пули игрока получают один отскок от стен. Работает на SHG, SEK, фрагменты и дронов; ракеты остаются взрывными.', apply: s => { s.bulletBounce += 1; } },
  { id: 'bullet_range',    label: 'BULLET RANGE +22%',  tier: 1, branch: 'ALL', desc: 'Дальность полёта обычных пуль растёт: дробовик живёт дальше, SEEKER имеет больший лимит дистанции.', apply: s => { s.bulletRange *= 1.22; } },
...
export const HERO_UPGRADES = UPGRADES.filter(u => !WEAPON_BRANCHES.includes(u.branch));
...
export const WEAPON_CHEST_REWARDS = [
  ...
  { id: 'bullet_ricochet', kind: 'weapon_upgrade', upgrade: 'bullet_ricochet', label: 'BULLET RICOCHET +1', desc: 'Общий апгрейд оружия: пули SHG/SEK/дронов получают дополнительный отскок от стен. Ракеты не рикошетят.' },
  { id: 'bullet_range', kind: 'weapon_upgrade', upgrade: 'bullet_range', label: 'BULLET RANGE +22%', desc: 'Общий апгрейд оружия: дробь летит дальше, SEEKER получает больший лимит дистанции.' },
