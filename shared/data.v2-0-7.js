...
export const UPGRADES = [
  { id: 'dmg',      label: 'DMG +15%',             tier: 0, desc: 'All damage scales upward.', apply: s => { s.dmgMul *= 1.15; } },
  { id: 'fire',     label: 'FIRE RATE +12%',       tier: 0, desc: 'Weapon cooldowns shrink.', apply: s => { s.fireMul *= 1.12; } },
  { id: 'spd',      label: 'SPD +8%',              tier: 0, desc: 'Move speed up.', apply: s => { s.spdMul *= 1.08; } },
  { id: 'maxhp',    label: 'HP +20',               tier: 0, desc: 'Max health up.', apply: s => { s.maxHpAdd += 20; } },
  { id: 'magnet',   label: 'MAGNET +40%',          tier: 0, desc: 'Pickup pull radius grows.', apply: s => { s.magnetMul *= 1.4; } },
  { id: 'dash',     label: 'DASH +1',              tier: 1, desc: 'More dash charges.', apply: s => { s.dashAdd += 1; } },
  { id: 'drone',    label: 'DRONE +1',             tier: 1, desc: 'Adds auto-shooting companion.', apply: s => { s.drones += 1; } },
  { id: 'orbital',  label: 'ORBITAL +1',           tier: 1, desc: 'Adds contact-damage orbital.', apply: s => { s.orbitals += 1; } },
  { id: 'luck',     label: 'LUCK +1',              tier: 1, desc: 'Better upgrade/casino rolls.', apply: s => { s.luck += 1; } },
  { id: 'proc',     label: 'BLAST PROC 10%',       tier: 1, desc: 'Bullet hits can create blasts. Over 100% adds extra blasts.', apply: s => { s.procBlast += 0.10; } },
  { id: 'echo',     label: 'ECHO SHOT 12%',        tier: 1, desc: 'More extra shots. Over 100% adds extra bullets.', apply: s => { s.echoShot += 0.12; } },
  { id: 'leech',    label: 'LIFESTEAL 2%',         tier: 1, desc: 'Heal from dealt damage.', apply: s => { s.lifesteal += 0.02; } },
  { id: 'goldgun',  label: 'GLD ON KILL +40%',     tier: 1, desc: 'More gold from enemy deaths.', apply: s => { s.goldMul *= 1.4; } },

  // weapon branches
  { id: 'shg_bounce', label: 'SHG RICOCHET +1',    tier: 1, branch: 'SHG', desc: 'Shotgun pellets bounce once more.', apply: s => { s.shgBounce += 1; } },
  { id: 'shg_teeth',  label: 'SHG TEETH +2 PELLETS', tier: 1, branch: 'SHG', desc: 'Shotgun gains two extra pellets.', apply: s => { s.shgPellets += 2; } },
  { id: 'sek_split',  label: 'SEK SPLIT ON KILL',  tier: 1, branch: 'SEK', desc: 'Seeker kills release small seeking fragments.', apply: s => { s.sekSplit += 1; } },
  { id: 'sek_chain',  label: 'SEK CHAIN LOCK',     tier: 1, branch: 'SEK', desc: 'Seeker homing and lifetime improve.', apply: s => { s.sekChain += 1; } },
  { id: 'rkt_cluster',label: 'RKT CLUSTER +2',     tier: 1, branch: 'RKT', desc: 'Rockets burst into mini explosions.', apply: s => { s.rktCluster += 1; } },
  { id: 'rkt_mines',  label: 'RKT STATIC MINES',   tier: 1, branch: 'RKT', desc: 'Rockets leave delayed square mines.', apply: s => { s.rktMines += 1; } },

  // ability / active branches
  { id: 'voidstep',  label: 'DASH: VOID STEP',     tier: 1, branch: 'DASH', desc: 'Dash leaves a damaging cyan cut.', apply: s => { s.voidStep += 1; } },
  { id: 'dashcut',   label: 'DASH CUTS BULLETS',  tier: 1, branch: 'DASH', desc: 'Dash erases enemy bullets near its path.', apply: s => { s.dashCut += 1; } },
  { id: 'dashclone', label: 'DASH CLONE',         tier: 1, branch: 'DASH', desc: 'Dash releases an echo burst at the start point.', apply: s => { s.dashClone += 1; } },
  { id: 'q_snap',    label: 'Q: FIELD SNAP',      tier: 1, branch: 'Q', desc: 'Q pulls enemies inward and damages them.', apply: s => { s.activeSnap += 1; } },
  { id: 'q_blood',   label: 'Q: BLOOD PULSE',     tier: 1, branch: 'Q', desc: 'Q spends HP for a red square blast.', apply: s => { s.activeBlood += 1; } },
  { id: 'q_over',    label: 'Q: OVERCLOCK',       tier: 1, branch: 'Q', desc: 'Q temporarily boosts fire rate.', apply: s => { s.activeOver += 1; } },

  // high rarity rule-breakers
  { id: 'droneproc', label: 'DRONES COPY PROC',    tier: 2, desc: 'Drone bullets inherit some BLAST PROC.', apply: s => { s.droneProc += 1; } },
  { id: 'orbreflect',label: 'ORBITALS REFLECT',    tier: 2, desc: 'Orbitals can erase enemy bullets.', apply: s => { s.orbReflect += 1; } },
  { id: 'debtengine',label: 'DEBT ENGINE',         tier: 2, cursed: true, desc: 'Huge damage and luck, but next rooms get static debt.', apply: s => { s.dmgMul *= 1.35; s.luck += 2; s.debtEngine += 1; } },
