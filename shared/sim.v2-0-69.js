...
} from './data.v2-0-69.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-69.js';
...
// v2.0.69: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function roomObjectivePayoutText(obj = {}, depth = 0, chain = 0) {
  ...
  const extra = contractChainPayout(depth, chain);
  const parts = [];
  if (base.gld) parts.push(`+${base.gld}G`);
  if (base.exp) parts.push(`+${base.exp}XP`);
  if (obj.id === 'cache_claim') parts.push('+SKN');
  if (extra.gld || extra.exp) parts.push(`CHAIN x${Math.max(0, chain | 0)} +${extra.gld}G +${extra.exp}XP`);
  return parts.join(' · ') || obj.reward || '';
}
function decorateRoomObjective(obj, depth = 0, chain = 0) {
  if (!obj) return null;
  return { ...obj, reward: roomObjectivePayoutText(obj, depth, chain) };
}
function evaluateRoomObjective(run, st = {}, time = 0) {
  ...
  return { ...obj, reward: roomObjectivePayoutText(obj, depth, previewChain), done: done ? 1 : 0, bonusGld: done ? base.gld : 0, bonusExp: done ? base.exp : 0, progress: roomObjectiveProgress(run, st) };
}
...
function makeNextRoomPreview(run) {
  ...
  return {
    ...
    objective: decorateRoomObjective(roomObjectiveForPlan(plan, depth), depth, Math.max(1, (run.runMemory?.contractStreak || 0) + 1))
  };
}
...
export function createRun(seedBase) {
  return {
    ...
    roomStats: null, roomObjective: null, nextRoomPreview: null, roomSockets: [], roomWires: [], prismLaneT: 0, pendingPrismLanes: [], pendingBloodTax: [], portalOpenedAt: 0, huntedExitOpenedAt: 0, huntedExitSpawnT: 0,
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss') {
    ...
    // v2.0.69: skin rooms were too rare. Raise the base rate and add soft pity.
...
export function resetRun(run, players) {
  ...
  run.staticRainFromPending = false;
  run.roomStaticRainFalls = 0;
  run.roomStats = null; run.roomObjective = null; run.nextRoomPreview = null; run.roomSockets = []; run.roomWires = []; run.pendingPrismLanes = []; run.pendingBloodTax = []; run.portalOpenedAt = 0; run.huntedExitOpenedAt = 0; run.huntedExitSpawnT = 0;
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.69: VOID LASER is now a builder. Level I = one short segment.
...
export function buildSnapshot(run, players) {
  ...
  return {
    ...
    room: {
      ...
      objective: run.roomObjective ? { ...decorateRoomObjective(run.roomObjective, run.runDepth || 0, Math.max(1, (run.runMemory?.contractStreak || 0) + 1)), progress: roomObjectiveProgress(run) } : null,
