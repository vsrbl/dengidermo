...
} from './data.v2-0-45.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-45.js';
...
// v2.0.45: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    const laserLen = Math.round(activeScale(560 + Math.min(1240, extra * 55)));
    const aim = activeAimPoint(p, laserLen, 150);
    const end = collideWalls(aim.x, aim.y, 5, run.plan.walls || [], p.x, p.y);
    const width = Math.max(6, Math.round(activeScale(7 + Math.min(10, extra * 0.35))));
    const visualWidth = Math.max(2, Math.round(width * 0.26));
    const ttl = activeScale(0.40 + Math.min(1.25, extra * 0.07));
    const laserDmg = (34 + lvl * 15) * p.stats.dmgMul;
    ...
    p.invuln = Math.max(p.invuln || 0, 0.06 + lvl * 0.018);
    let cut = 0;
    ...
    for (const e of [...run.enemies]) if (distToSegment2(e.x, e.y, startX, startY, end.x, end.y) < (width + e.size / 2) ** 2) {
      e.activeSlowT = Math.max(e.activeSlowT || 0, 0.08 + lvl * 0.016);
    ...
    activeField(run, {
      ...
      ttl, tickEvery: 0.09, dmg: (4 + lvl * 2.4) * p.stats.dmgMul, slow: 0.20, damp: 0.02
    });
    ...
    run.fx.push({ t: 'active_mutation', label: `VOID LASER ${roman(lvl)}`, x: Math.round(startX + (end.x - startX) * 0.12), y: Math.round(startY + (end.y - startY) * 0.12), r: Math.max(24, width * 2), tone: 'purple', squareBlast: 1 });
