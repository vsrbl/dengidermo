...
export const UPGRADES = [
  ...
  { id: 'sek_swarm', label: 'SEK SWARM RMB', tier: 1, branch: 'SEK', desc: 'ПКМ выпускает рой самонаводящихся SEK-пуль сразу. Каждый стак: +5 пуль и чуть дольше cooldown.', apply: s => { s.sekSwarm += 1; } },
...
export const WEAPON_CHEST_REWARDS = [
  ...
  { id: 'sek_swarm', kind: 'weapon_upgrade', upgrade: 'sek_swarm', reqWeapon: 'seeker', label: 'SEK SWARM RMB', desc: 'Апгрейд SEK: ПКМ выпускает рой самонаводящихся пуль сразу.' },
...
export function spinCasino(rng, stakeKey, luck, unlockedSkins = []) {
  ...
  const r = rng();
  // v2.0.76: skins were opening too slowly. SKN casino outcomes are now visible
