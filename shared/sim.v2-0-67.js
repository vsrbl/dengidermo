...
} from './data.v2-0-67.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-67.js';
...
// v2.0.67: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss') {
    ...
    // v2.0.67: skin rooms were too rare. Raise the base rate and add soft pity.
...
function beginTransition(run, players) {
  ...
  const contractBonusGld = contractChain >= 2 ? Math.min(80, 4 * contractChain + Math.floor((run.runDepth || 0) * 0.65)) : 0;
  const contractBonusExp = contractChain >= 2 ? Math.min(70, 3 * contractChain + Math.floor((run.runDepth || 0) * 0.55)) : 0;
  ...
  if (objResult?.done) addRunTape(run, `CONTRACT ${objResult.label}`, 'gold');
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.67: VOID LASER is now a builder. Level I = one short segment.
