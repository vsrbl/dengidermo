...
} from './data.v2-0-79.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-79.js';
...
// v2.0.79: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function loopCostMul(run) {
  // v2.0.79: costs were far too flat. By loop 2 a player could sit on ~2k GLD
...
function roomObjectiveStatus(run, st = run?.roomStats || {}) {
  ...
  const time = roomSolvedTime(run, st);
  const solved = roomSolvedAt(run, st) > 0;
  // v2.0.79 rule: every room contract is finalised ONLY when the portal opens.
...
function rewardShellMarket(run, x, y) {
  // Shell Market modifier was removed in v2.0.79. Keep shell-break memory only.
...
export function startRoom(run, players) {
  ...
  if ((run.plan.modifierIds || []).includes('casino_virus')) {
    run.casinoVirus = { spinsLeft: 3, nextSpin: 30, done: false, lastLabel: 'WAITING', activeRainStacks: 0, rainT: 0, spinSeq: 0 };
  }
  ...
  if (run.plan.category !== 'boss' && !isGreedRoom(run)) {
    ...
    // v2.0.79: skin rooms were too rare. Raise the base rate and add soft pity.
...
function spawnCasinoVirusEvent(run, players) {
  ...
  if (roll < 0.26) {
    ...
    cv.rainT = 0.25;
    run.fx.push({ t: 'contract_done', label: 'CASINO VIRUS', body: 'STATIC RAIN STARTED' });
  } else if (roll < 0.66) {
    label = 'BIG STATIC RAIN';
    cv.activeRainStacks = Math.max(cv.activeRainStacks || 0, 5);
    cv.rainT = 0.18;
    run.fx.push({ t: 'contract_done', label: 'CASINO VIRUS', body: 'BIG STATIC RAIN STARTED' });
  } else if (roll < 0.80) {
  ...
  cv.lastLabel = label; cv.spinSeq = (cv.spinSeq || 0) + 1;
  run.fx.push({ t: 'casino_virus_spin', label, spinsLeft: cv.spinsLeft, nextSpin: Math.round(cv.nextSpin || 0), seq: cv.spinSeq, x: Math.round(center.x), y: Math.round(center.y) });
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.79: VOID LASER is now a builder. Level I = one short segment.
