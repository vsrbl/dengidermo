...
} from './data.v2-0-76.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-76.js';
...
// v2.0.76: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function nextStaticRainLevel(run) {
  if (!run) return 0;
  const debt = run.staticDebt === true ? 1 : Math.max(0, run.staticDebt || 0);
  const carry = Math.max(0, run.staticRainCarry || 0);
  // While a seeding STATIC RAIN room is still being played, show the real strike count.
  // After transition begins, the same value is stored in staticRainCarry, so do not count it twice.
...
function staticRainCurrentMode(run) {
  if (!run?.plan?.modifierIds?.includes('static_rain') || !(run.staticRainStacks > 0)) return '';
...
function roomModRewardTags(mods = [], special = '') {
  const tags = [];
  if (mods.includes('greed')) tags.push('GLD↑');
  if (mods.includes('casino_virus')) tags.push('BET↑');
  if (mods.includes('shell_market')) tags.push('SHELL GLD');
  if (mods.includes('blood_tax')) tags.push('GLD RISK');
  if (mods.includes('static_wires')) tags.push('WIRE CTRL');
  if (mods.includes('hunted_exit')) tags.push('EARLY EXIT');
...
function roomDangerScore(plan = {}, staticLevel = 0) {
  ...
  for (const m of mods) {
    ...
    else if (m === 'hunted_exit') score += 1.05;
    else if (m === 'prism_grid' || m === 'anchor_gravity' || m === 'shell_market') score += 0.8;
    else if (m === 'static_wires' || m === 'blood_tax' || m === 'casino_virus') score += 0.65;
    else if (m === 'debt_floor' || m === 'hunter_contract' || m === 'mirror_room' || m === 'echo_walls') score += 0.7;
...
function roomThreatTags(plan = {}, staticLevel = 0) {
  ...
  if (mods.includes('hunted_exit')) tags.push('OVERSTAY HUNTERS');
  if (mods.includes('prism_grid')) tags.push('PRISM LANES');
  if (mods.includes('anchor_gravity')) tags.push('GRAVITY SOCKETS');
  if (mods.includes('static_wires')) tags.push('SLOW WIRES');
  if (mods.includes('shell_market')) tags.push('ARMOR WALL');
  if (mods.includes('blood_tax')) tags.push('DEATH ZONES');
  if (mods.includes('echo_walls') || mods.includes('mirror_room')) tags.push('ECHO ROUTES');
  if (mods.includes('casino_virus')) tags.push('BET CHAOS');
  if (mods.includes('hunter_contract') || special === 'signal_contract') tags.push('PRIORITY TARGET');
...
function roomTip(plan = {}, staticLevel = 0, staticMode = '') {
  ...
  if (mods.includes('static_rain')) return staticMode === 'paid' ? 'STATIC PAYOFF: survive the level; strikes here do not seed another debt.' : 'STATIC SEED: each real strike raises the next eligible room level.';
  if (mods.includes('hunted_exit')) return 'EARLY PORTAL: leave when it opens, or overstay for greed and hunter waves.';
  if (mods.includes('prism_grid')) return 'LANE ROOM: keep moving across lines, not along them.';
  if (mods.includes('anchor_gravity')) return 'SOCKET ROOM: fight near edges; sockets bend bullets and pickups.';
  if (mods.includes('static_wires')) return 'WIRE ROOM: drag enemies through wires, but do not kite inside them.';
  if (mods.includes('shell_market')) return 'ARMOR ROOM: crack shells first; broken shells pay GLD.';
  if (mods.includes('blood_tax')) return 'GREED ROOM: kill from distance, collect after blood squares fade.';
  if (mods.includes('echo_walls') || mods.includes('mirror_room')) return 'ECHO ROOM: change route after every wave; old paths replay.';
  if (mods.includes('casino_virus')) return 'CASINO ROOM: take value, but do not chase every roll.';
...
function roomObjectiveForPlan(plan = {}, depth = 0) {
  ...
  const special = roomPlanSpecial(plan);
  const cat = roomPlanCategory(plan);
  if (cat === 'boss') return { id: 'boss_cut', label: 'BOSS CUT', reward: 'GLD+EXP', goal: 'Kill the boss.' };
  if (special === 'chill_room') return { id: 'lounge_cashout', label: 'SAFE CASHOUT', reward: 'SHOP', goal: 'Leave when ready.' };
  if (mods.includes('hunted_exit')) return { id: 'hunted_open', label: 'HUNTED EXIT', reward: 'GLD+EXP', goal: 'Open the hunted exit portal.' };
  if (mods.includes('static_wires')) return { id: 'wire_ghost', label: 'WIRE GHOST', reward: 'EXP', goal: 'Clear without touching static wires.' };
  if (mods.includes('prism_grid')) return { id: 'grid_ghost', label: 'GRID GHOST', reward: 'EXP', goal: 'Clear without taking prism lane hits.' };
  if (mods.includes('shell_market')) return { id: 'shell_breaker', label: 'SHELL BREAKER', reward: 'GLD', goal: 'Break 3 shells.' };
  if (mods.includes('blood_tax')) return { id: 'blood_paid', label: 'BLOOD PAID', reward: 'GLD', goal: 'Trigger 3 blood tax drops and survive.' };
  if (mods.includes('static_rain')) return { id: 'static_clean', label: 'STATIC CLEAN', reward: 'EXP', goal: 'Clear with low damage taken.' };
  if (mods.includes('skin_cache')) return { id: 'cache_claim', label: 'CACHE CLAIM', reward: 'SKN', goal: 'Clear and claim the cache.' };
  if (arch === 'panic_box' || arch === 'compact') return { id: 'fast_clear', label: 'FAST CLEAN', reward: 'EXP', goal: 'Clear fast.' };
  if (arch === 'wide' || arch === 'long_lane') return { id: 'no_hit', label: 'NO HIT TAPE', reward: 'GLD', goal: 'Clear without taking damage.' };
  return { id: 'clean_signal', label: 'CLEAN SIGNAL', reward: 'GLD+EXP', goal: 'Clear the room.' };
}
function roomObjectiveProgress(run, st = run?.roomStats || {}) {
  ...
  const progress = {
    ...
    overstay_survive: `${st.huntedWaves || 0}/1`,
    hunted_open: roomSolvedAt(run, st) > 0 ? 'PORTAL OPEN' : `${run?.kills || 0}/${Math.max(3, Math.floor((run?.plan?.quota || 0) * 0.68))}`,
    wire_ghost: (st.wireTouches || 0) <= 0 ? 'CLEAN' : `${st.wireTouches} TOUCH`,
    grid_ghost: (st.prismHits || 0) <= 0 ? 'CLEAN' : `${st.prismHits} HIT`,
    shell_breaker: `${st.shellBreaks || 0}/3`,
    blood_paid: `${st.bloodTaxes || 0}/3`,
    static_clean: `${Math.round(st.damageTaken || 0)}/${lowDamageLimit} DMG`,
    cache_claim: 'CLEAR',
    fast_clear: `${Math.round(time)}/42s`,
    no_hit: (st.damageTaken || 0) <= 0 ? 'CLEAN' : `${Math.round(st.damageTaken || 0)} DMG`,
    clean_signal: `${run?.kills || 0}/${run?.plan?.quota || 0}`
  };
...
function roomObjectivePayoutText(obj = {}, depth = 0, chain = 0) {
  // Player-facing preview must not look like already-granted loot.
  // Exact +GLD/+EXP/+SKN is shown only in ROOM CHECK after payout is actually applied.
  if (!obj?.id) return '';
  if (obj.id === 'lounge_cashout') return 'SHOP';
  if (obj.id === 'cache_claim') return 'ON CLEAR';
  return 'ON COMPLETE';
}
...
function roomObjectiveDoneRaw(obj, run, st = {}, time = 0) {
  ...
  if (obj.id === 'overstay_survive') return (st.huntedWaves || 0) >= 1;
  if (obj.id === 'hunted_open') return roomSolvedAt(run, st) > 0;
  if (obj.id === 'wire_ghost') return (st.wireTouches || 0) <= 0;
  if (obj.id === 'grid_ghost') return (st.prismHits || 0) <= 0;
  if (obj.id === 'shell_breaker') return (st.shellBreaks || 0) >= 3;
  if (obj.id === 'blood_paid') return (st.bloodTaxes || 0) >= 3;
  if (obj.id === 'static_clean') return (st.damageTaken || 0) <= lowDamageLimit;
  if (obj.id === 'cache_claim') return true;
  if (obj.id === 'fast_clear') return time > 0 && time < 42;
  if (obj.id === 'no_hit') return (st.damageTaken || 0) <= 0;
  if (obj.id === 'clean_signal') return true;
  return false;
}
function roomObjectiveFailReason(obj, run, st = {}, time = 0) {
  ...
  if (obj.id === 'fast_clear' && time >= 42) return 'TIME LOST';
  if (!solved) return '';
  if (obj.id === 'overstay_survive' && (st.huntedWaves || 0) < 1) return 'NO HUNTER WAVE';
  if (obj.id === 'shell_breaker' && (st.shellBreaks || 0) < 3) return 'SHELLS LEFT';
  if (obj.id === 'blood_paid' && (st.bloodTaxes || 0) < 3) return 'BLOOD LOW';
  return '';
}
function roomObjectiveStatus(run, st = run?.roomStats || {}) {
  ...
  const time = roomSolvedTime(run, st);
  const solved = roomSolvedAt(run, st) > 0;
  // v2.0.76 rule: every room contract is finalised ONLY when the portal opens.
  // Before that moment the UI may show progress/risk, but never DONE/FAILED.
...
function evaluateRoomObjective(run, st = {}, time = 0) {
  ...
  return { ...obj, reward: roomObjectivePayoutText(obj, depth, previewChain), done: done ? 1 : 0, status: status.status, statusLabel: status.statusLabel, failReason: status.failReason, bonusGld: done ? base.gld : 0, bonusExp: done ? base.exp : 0, progress: roomObjectiveProgress(run, st) };
}
...
function settleRoomObjectiveAtPortalOpen(run) {
  ...
  const time = roomSolvedTime(run, st);
  const result = evaluateRoomObjective(run, st, time);
  if (!result) return null;
  run.roomObjectiveSettlement = { ...result, settledAt: roomSolvedAt(run, st), solvedTime: Math.round(time), progress: result.progress };
  run.roomObjectiveFrozenStats = st;
  run.fx.push({
    t: result.done ? 'contract_done' : 'contract_fail',
    label: result.label,
    body: result.done ? 'LOCKED AT PORTAL' : (result.failReason || 'LOCKED AT PORTAL')
  });
...
function makeNextRoomPreview(run) {
  ...
  return {
    ...
    objective: decorateRoomObjective(roomObjectiveForPlan(plan, depth), depth, Math.max(1, (run.runMemory?.contractStreak || 0) + 1), { status: 'planned', statusLabel: 'NEXT', progress: '—' })
  };
}
...
function updateRunMemoryFromRoom(run, st, flags = {}) {
  ...
  mem.contractStreak = flags.objectiveDone ? (mem.contractStreak || 0) + 1 : 0;
...
function rewardShellMarket(run, x, y) {
  ...
  if (!run?.plan?.modifierIds?.includes('shell_market')) return;
  if (Math.random() < 0.78) {
    const val = 6 + Math.round(Math.random() * 12) + Math.floor((run.runDepth || 0) * 0.55);
    dropPickup(run, x + (Math.random() - 0.5) * 36, y + (Math.random() - 0.5) * 36, 'GLD', val);
    run.fx.push({ t: 'casino_tick', x: Math.round(x), y: Math.round(y), good: 1 });
  }
}
...
function shellChance(run, kind, def, elite, type) {
  ...
  const shellMarket = run?.plan?.modifierIds?.includes('shell_market') ? 1 : 0;
  if (type === 'linked') {
...
export function createRun(seedBase) {
  return {
    ...
    roomStats: null, roomObjective: null, roomObjectiveSettlement: null, roomObjectiveFrozenStats: null, nextRoomPreview: null, devNextRoomOverride: null, roomSockets: [], roomWires: [], prismLaneT: 0, pendingPrismLanes: [], pendingBloodTax: [], portalOpenedAt: 0, huntedExitOpenedAt: 0, huntedExitSpawnT: 0,
...
export function startRoom(run, players) {
  ...
  const incomingRainStacks = clampStaticRainLevel(debtStacks + carryStacks);
  run.staticRainStacks = 0;
  run.staticRainCanSeedNext = false;
  run.staticRainFromPending = false;
  run.roomStaticRainFalls = 0;
  initRoomStats(run);
  run.roomObjective = roomObjectiveForPlan(run.plan, run.runDepth);
  ...
  if ((run.plan.modifierIds || []).includes('static_wires')) {
    run.roomWires = makeRoomWires(seed, run.plan.roomArchetype || 'standard');
  }
  if (incomingRainStacks > 0 && run.plan.category !== 'boss' && run.plan.specialRoomId !== 'chill_room') {
    ...
    // Pending static is spent on this room. It must not cascade forever from its own strikes,
    // even if the generated room also rolled natural STATIC RAIN.
    run.staticRainCanSeedNext = false;
    run.staticDebt = 0;
    run.staticRainCarry = 0;
  } else if (naturalStaticRain && run.plan.category !== 'boss' && run.plan.specialRoomId !== 'chill_room') {
  ...
  if (run.plan.category !== 'boss') {
    ...
    // v2.0.76: skin rooms were too rare. Raise the base rate and add soft pity.
    // Rule of thumb: normal rooms can show SKN CACHE often enough to feel collectible;
    // special/greed/casino/mirror/debt rooms push the chance higher.
    ...
    if (mods.includes('greed') || mods.includes('casino_virus') || mods.includes('mirror_room') || mods.includes('hunter_contract') || mods.includes('debt_floor') || mods.includes('hunted_exit')) skinChance += 0.034;
  ...
  for (const p of players.values()) {
    ...
    if (p.stats.debtEngine > 0 && run.plan.category !== 'boss' && run.plan.specialRoomId !== 'chill_room') {
      if (!run.plan.modifierIds.includes('static_rain')) run.plan.modifierIds.push('static_rain');
      run.staticRainStacks = Math.max(run.staticRainStacks || 0, 1);
      // Debt-engine rooms can seed the next static level, but not when this room is already
      // paying off pending static. That keeps the payoff room from becoming an endless chain.
      if (!run.staticRainFromPending) run.staticRainCanSeedNext = true;
    }
  }
...
export function resetRun(run, players) {
  ...
  run.staticRainFromPending = false;
  run.roomStaticRainFalls = 0;
  run.roomStats = null; run.roomObjective = null; run.roomObjectiveSettlement = null; run.roomObjectiveFrozenStats = null; run.nextRoomPreview = null; run.devNextRoomOverride = null; run.roomSockets = []; run.roomWires = []; run.pendingPrismLanes = []; run.pendingBloodTax = []; run.portalOpenedAt = 0; run.huntedExitOpenedAt = 0; run.huntedExitSpawnT = 0;
...
const ENCOUNTER_PACKS = [
  ...
  {
    id: 'shell_market_wall', label: 'SHELL MARKET WALL', intent: 'armor', minLoop: 1, weight: 2.3, requireMod: 'shell_market', minGap: 7.2, maxGap: 10.4, armorCap: 6,
    note: 'More shells, but breaking shells pays GLD shards.',
    roles: [
      { pick: ['warden','tank','charger'], count: [1, 2], opts: { forcePlain: true, packRole: 'market_shell' } },
      { pick: ['leech','shooter'], count: [0, 2], opts: { noArmor: true, packRole: 'support' } },
      { pick: ['grunt','runner'], count: [2, 4], opts: { noArmor: true, packRole: 'battery' } }
    ]
  },
  {
    id: 'echo_wall_scramble', label: 'ECHO WALL SCRAMBLE', intent: 'mirror', minLoop: 2, weight: 2.1, requireMod: 'echo_walls', minGap: 6.4, maxGap: 9.0,
  ...
  {
    id: 'static_wire_crossfire', label: 'STATIC WIRE CROSSFIRE', intent: 'control', minLoop: 1, weight: 2.0, requireMod: 'static_wires', minGap: 6.6, maxGap: 9.8, supportCap: 5,
    note: 'Static wires slow routes while ranged/control mobs punish greedy paths.',
    roles: [
      { pick: ['pulse','prism','shooter'], count: [1, 2], opts: { noArmor: true, packRole: 'wire_guard' } },
      { pick: ['runner','charger'], count: [1, 3], opts: { noArmor: true, packRole: 'wire_push' } },
      { pick: ['damper','anchor'], count: [0, 1], opts: { forcePlain: true, packRole: 'wire_core' } }
    ]
  },
  {
    id: 'hunted_exit_rush', label: 'HUNTED EXIT RUSH', intent: 'chaos', minLoop: 1, weight: 2.1, requireMod: 'hunted_exit', minGap: 5.4, maxGap: 7.9,
    note: 'Portal opens early, but the room keeps sending fast hunters if players overstay.',
    roles: [
      { pick: ['runner','charger','bouncer'], count: [2, 4], opts: { noArmor: true, packRole: 'hunter' } },
      { pick: ['glitch','shooter'], count: [1, 2], opts: { noArmor: true, packRole: 'exit_tax' } }
    ]
  },
  {
...
function pickDirectorMode(run) {
  ...
  if (mods.includes('shell_market')) return { id: 'shell_market', label: 'SHELL MARKET', intents: { armor: 2.4, support: 1.2 } };
  if (mods.includes('echo_walls')) return { id: 'echo_walls', label: 'ECHO WALLS', intents: { mirror: 2.2, ranged: 1.2 } };
  if (mods.includes('anchor_gravity')) return { id: 'anchor_gravity', label: 'ANCHOR GRAVITY', intents: { control: 2.2, ranged: 1.1 } };
  if (mods.includes('static_wires')) return { id: 'static_wires', label: 'STATIC WIRES', intents: { control: 1.8, ranged: 1.4, swarm: 1.1 } };
  if (mods.includes('hunted_exit')) return { id: 'hunted_exit', label: 'HUNTED EXIT', intents: { chaos: 1.8, swarm: 1.6, director: 1.2 } };
...
function modPackFitMul(run, pack) {
  ...
  if (mods.includes('shell_market') && ['armor','support'].includes(pack.intent)) m *= 1.22;
  ...
  if (mods.includes('static_wires') && ['control','ranged'].includes(pack.intent)) m *= 1.18;
  if (mods.includes('hunted_exit') && ['chaos','swarm'].includes(pack.intent)) m *= 1.2;
  return m;
}
...
function damagePlayer(run, p, dmg, srcX, srcY) {
  ...
  if (run.roomStats) run.roomStats.damageTaken += Math.max(0, dmg || 0);
  p.hp -= dmg;
...
function killEnemy(run, players, e, killer) {
  ...
  if (run.plan?.modifierIds?.includes('blood_tax') && !ENEMIES[e.kind]?.boss) {
    const val = 2 + Math.round(Math.random() * 6) + (e.elite ? 8 : 0);
    dropPickup(run, e.x + (Math.random() - 0.5) * 34, e.y + (Math.random() - 0.5) * 34, 'GLD', val);
    if (Math.random() < (e.elite ? 0.80 : 0.30)) {
      const r = e.elite ? 92 : 68;
      if (!run.pendingBloodTax) run.pendingBloodTax = [];
      run.pendingBloodTax.push({ x: e.x, y: e.y, r, at: (run.now || 0) + 0.82, dmgP: e.elite ? 18 : 10, dmgE: e.elite ? 48 : 30 });
      run.fx.push({ t: 'blood_tax_warn', x: Math.round(e.x), y: Math.round(e.y), r, dur: 0.82 });
    }
  }
...
function fireWeapon(run, players, p, dt) {
  ...
  const echoBase = p.stats.echoShot + ((run.plan.modifierIds.includes('mirror_room') || run.plan.modifierIds.includes('echo_walls')) ? 0.18 : 0);
...
function explode(run, players, x, y, r, dmg, owner, hurtPlayers = false, style = 'blast', elem = '', elemPower = 0, opts = {}) {
  ...
  for (const e of [...run.enemies]) {
    if (dist2(e.x, e.y, x, y) < (r + e.size / 2) ** 2) {
      ...
      damageEnemy(run, players, e, activeScale(dmg), owner, knock, n.x, n.y);
    }
  }
  if (hurtPlayers) {
    for (const p of players.values()) {
...
function rocketExplode(run, players, b, x = b.x, y = b.y, r = b.aoe || 70, dmg = b.dmg) {
  explode(run, players, x, y, r, dmg, b.owner, false, 'rocket', b.elem || '', b.elemPower || 0, rocketControlOpts(b));
}
...
function stepBullets(run, players, dt) {
  ...
  for (const b of run.bullets) {
    ...
    if (b.detonateDist && b.travelled >= b.detonateDist) {
      if (b.kind === 'rocketgun' || b.mine) rocketExplode(run, players, b, b.x, b.y, b.aoe || 70, b.dmg);
      else explode(run, players, b.x, b.y, b.aoe || 70, b.dmg, b.owner, false, 'blast', b.elem || '', b.elemPower || 0);
    ...
    if (hitWall) {
      ...
      if (b.aoe) { if (b.kind === 'rocketgun' || b.mine) rocketExplode(run, players, b, b.x, b.y, b.aoe, b.dmg); else explode(run, players, b.x, b.y, b.aoe, b.dmg, b.owner, false, 'blast', b.elem || '', b.elemPower || 0); rocketAftermath(run, players, b); }
      b.life = -1; continue;
    }
    if (b.from === 'p') {
      for (const e of run.enemies) {
...
function stepMods(run, players, dt) {
  ...
  if (run.plan.modifierIds.includes('hunted_exit') && !run.portal.open && run.plan.quota > 0 && run.kills >= Math.max(3, Math.floor(run.plan.quota * 0.68))) {
    openPortal(run);
    run.huntedExitOpenedAt = run.now || 0;
    run.huntedExitSpawnT = 2.8;
    run.fx.push({ t: 'hunted_exit', x: run.portal.x, y: run.portal.y, left: Math.max(0, run.plan.quota - run.kills) });
  }
  if (run.plan.modifierIds.includes('hunted_exit') && run.portal.open && run.huntedExitOpenedAt > 0 && run.phase === 'play') {
    run.huntedExitSpawnT = Math.max(0, (run.huntedExitSpawnT || 0) - dt);
    const overstay = (run.now || 0) - (run.huntedExitOpenedAt || 0);
    if (run.huntedExitSpawnT <= 0 && overstay > 3.5 && run.enemies.length < MAX_ENEMIES - 3) {
      const n = 2 + Math.min(4, Math.floor(overstay / 12));
      const kinds = ['runner','charger','glitch','bouncer'];
      for (let i = 0; i < n; i++) spawnEnemy(run, players, kinds[Math.floor(Math.random() * kinds.length)], overstay > 16);
      if (run.roomStats) run.roomStats.huntedWaves = (run.roomStats.huntedWaves || 0) + 1;
      run.fx.push({ t: 'director_wave', label: 'HUNTED EXIT', intent: 'chaos', x: Math.round(run.portal.x), y: Math.round(run.portal.y), count: n });
      run.huntedExitSpawnT = Math.max(3.8, 7.4 - Math.min(2.8, overstay * 0.08)) + Math.random() * 1.0;
    }
  }
  if (run.plan.modifierIds.includes('static_wires') && run.roomWires?.length) {
    for (const w of run.roomWires) {
      const width = w.w || 40;
      for (const p of players.values()) {
        if (!p.alive) continue;
        if (distToSegment2(p.x, p.y, w.x1, w.y1, w.x2, w.y2) < (width + PLAYER_SIZE / 2) ** 2) {
          p.slowT = Math.max(p.slowT || 0, 0.26);
          p.slowMul = Math.min(p.slowMul || 1, 0.74);
          if (run.roomStats && Math.random() < 0.025) run.roomStats.wireTouches = (run.roomStats.wireTouches || 0) + 1;
        }
      }
      for (const e of run.enemies) {
        if (!e || e.hp <= 0 || ENEMIES[e.kind]?.boss) continue;
        if (distToSegment2(e.x, e.y, w.x1, w.y1, w.x2, w.y2) < (width + e.size / 2) ** 2) {
          e.activeSlowT = Math.max(e.activeSlowT || 0, 0.34);
          e.activeSlowMul = Math.min(e.activeSlowMul || 1, 0.58);
        }
      }
      for (const b of run.bullets) {
        if (distToSegment2(b.x, b.y, w.x1, w.y1, w.x2, w.y2) < (width + (b.size || 5)) ** 2) {
          const mul = b.from === 'e' ? Math.pow(0.28, dt) : Math.pow(0.58, dt);
          b.vx *= mul; b.vy *= mul;
        }
      }
    }
...
function beginTransition(run, players) {
  ...
  const fast = time > 0 && time < 42 ? 1 : 0;
  const tapes = [];
  if (noHit) tapes.push('NO HIT TAPE');
  if (fast) tapes.push('FAST CLEAN');
  if ((st.shellBreaks || 0) >= 3) tapes.push('SHELL MARKET REC');
  if ((st.prismHits || 0) <= 0 && run.plan?.modifierIds?.includes('prism_grid')) tapes.push('GRID DODGED');
  if ((st.bloodTaxes || 0) >= 3) tapes.push('BLOOD TAX PAID');
  if ((st.wireTouches || 0) <= 0 && run.plan?.modifierIds?.includes('static_wires')) tapes.push('WIRE GHOST');
  if ((st.huntedWaves || 0) > 0) tapes.push('OVERSTAY SURVIVED');
  ...
  const objectiveBonusGld = objResult?.done ? (objResult.bonusGld || 0) : 0;
  const objectiveBonusExp = objResult?.done ? (objResult.bonusExp || 0) : 0;
  ...
  const contractPayout = contractChainPayout(run.runDepth || 0, contractChain);
  const contractBonusGld = contractPayout.gld;
  const contractBonusExp = contractPayout.exp;
  if (bonusGld || bonusExp || objectiveBonusGld || objectiveBonusExp || contractBonusGld || contractBonusExp) {
    for (const p of players.values()) if (p.connected) {
      if (bonusGld || objectiveBonusGld || contractBonusGld) p.economy.money += bonusGld + objectiveBonusGld + contractBonusGld;
      if (bonusExp || objectiveBonusExp || contractBonusExp) addXp(run, p, bonusExp + objectiveBonusExp + contractBonusExp);
    }
  }
  updateRunMemoryFromRoom(run, st, { noHit, fast, bonusGld, bonusExp, objectiveSeen: !!objResult, objectiveDone: !!objResult?.done, objectiveGld: objectiveBonusGld, objectiveExp: objectiveBonusExp, contractGld: contractBonusGld, contractExp: contractBonusExp });
  ...
  if (objResult?.done) {
    addRunTape(run, `CONTRACT PAID ${objResult.label}`, 'gold');
    run.fx.push({ t: 'contract_paid', label: objResult.label, body: `+${objectiveBonusGld || 0}G +${objectiveBonusExp || 0}XP` });
  } else if (objResult) {
...
function activeCoreDesc(p) {
  ...
  const chargeLine = a.core === 'signal_spike'
    ...
    : (a.core === 'void_cut' ? `\nLINK POINTS: ${roman(voidLaserMaxSegments(p))}. Каждый уровень добавляет ещё одну точку связи и длину сегмента.` : '');
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.76: VOID LASER is now a builder. Level I = one short segment.
...
function spawnSeekerSwarm(run, players, p) {
  ...
  run.fx.push({ t: 'active_mutation', label: `SEK SWARM x${count}`, x: Math.round(originX), y: Math.round(originY), r: Math.round(60 + count * 3), tone: 'cyan', owner: p.id });
...
function doSecondaryWeapon(run, players, p) {
  ...
  if (wid === 'rocketgun') return detonateOldestRemoteRocket(run, players, p);
  if (wid === 'seeker') return spawnSeekerSwarm(run, players, p);
  if (wid === 'shotgun') return fireShotgunLongshot(run, players, p);
  return false;
}
...
export function buildSnapshot(run, players) {
  ...
  const nextStatic = nextStaticRainLevel(run);
  ...
  if (nextPreview) {
    const appliesNextStatic = nextStatic > 0 && nextPreview.cat !== 'boss' && nextPreview.special !== 'chill_room';
    if (appliesNextStatic && !nextPreview.mods.includes('static_rain')) nextPreview.mods.push('static_rain');
    const ni = roomIntel(nextPreview, appliesNextStatic ? nextStatic : 0, appliesNextStatic ? 'paid' : '');
    nextPreview = { ...nextPreview, ...ni, staticRainLevel: appliesNextStatic ? nextStatic : 0, staticBanked: nextStatic > 0 && !appliesNextStatic ? nextStatic : 0 };
  }
  const fx = run.fx;
  run.fx = [];
  return {
    ...
    room: {
      ...
      mods: run.plan.modifierIds, quota: run.plan.quota, kills: run.kills, archetype: run.plan.roomArchetype || 'standard',
      ...
      staticRainStacks: run.staticRainStacks || 0, staticRainNext: nextStatic, staticRainMode: staticMode,
      ...
      next: nextPreview, sockets: run.roomSockets || [], wires: run.roomWires || [], betStakes: casinoStakeTable(run),
      runMemory: { ...(run.runMemory || {}) }, tapeLog: (run.tapeLog || []).slice(0, 10), skinPity: run.skinPity || 0
    },
