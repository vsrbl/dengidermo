...
} from './data.v2-0-65.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-65.js';
...
// v2.0.65: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
export function createRun(seedBase) {
  return {
    ...
    roomStats: null, nextRoomPreview: null, roomSockets: [], roomWires: [], prismLaneT: 0, pendingPrismLanes: [], pendingBloodTax: [], huntedExitOpenedAt: 0, huntedExitSpawnT: 0,
    runMemory: { roomsCleared: 0, totalKills: 0, totalGld: 0, totalExp: 0, totalDamageTaken: 0, noHitStreak: 0, fastStreak: 0, bestNoHitStreak: 0, bestFastStreak: 0, skinRoomsSeen: 0, staticPaid: 0, shellBreaks: 0, huntedWaves: 0 },
    tapeLog: [],
    plan: null,
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss') {
    ...
    // v2.0.65: skin rooms were too rare. Raise the base rate and add soft pity.
...
export function resetRun(run, players) {
  ...
  run.staticRainFromPending = false;
  run.roomStaticRainFalls = 0;
  run.roomStats = null; run.nextRoomPreview = null; run.roomSockets = []; run.roomWires = []; run.pendingPrismLanes = []; run.pendingBloodTax = []; run.huntedExitOpenedAt = 0; run.huntedExitSpawnT = 0;
  run.runMemory = { roomsCleared: 0, totalKills: 0, totalGld: 0, totalExp: 0, totalDamageTaken: 0, noHitStreak: 0, fastStreak: 0, bestNoHitStreak: 0, bestFastStreak: 0, skinRoomsSeen: 0, staticPaid: 0, shellBreaks: 0, huntedWaves: 0 };
  run.tapeLog = [];
  for (const p of players.values()) {
...
function beginTransition(run, players) {
  ...
  updateRunMemoryFromRoom(run, st, { noHit, fast, bonusGld, bonusExp });
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.65: VOID LASER is now a builder. Level I = one short segment.
