...
import { ROOM_MODS, ROOM_SEQUENCE, SPECIAL_ROOMS } from './data.v2-0-76.js';
...
function chooseRoomArchetype(rng, category, specialRoomId, modifierIds = [], loopIndex = 0) {
  ...
  if (modifierIds.includes('static_wires')) return rng() < 0.58 ? 'long_lane' : 'compact';
  if (modifierIds.includes('hunted_exit')) return rng() < 0.54 ? 'long_lane' : 'standard';
  if (modifierIds.includes('blood_tax') || modifierIds.includes('casino_virus')) return rng() < 0.55 ? 'compact' : 'panic_box';
  if (modifierIds.includes('shell_market') || modifierIds.includes('hunter_contract')) return rng() < 0.58 ? 'wide' : 'standard';
...
export function generateRoom(seed, runDepth, loopIndex, override = null) {
  ...
  if (!forcedMods && specialRoomId === 'signal_contract') {
    const contractMods = ['greed', 'static_rain', 'debt_floor', 'hunter_contract', 'casino_virus', 'mirror_room', 'prism_grid', 'blood_tax', 'shell_market', 'echo_walls', 'anchor_gravity', 'static_wires', 'hunted_exit'];
  ...
  const forcedArchetype = forced?.archetype && ['panic_box','compact','standard','wide','long_lane','lounge','boss'].includes(forced.archetype) ? String(forced.archetype) : '';
  ...
  if (modifierIds.includes('hunted_exit')) baseQuota = Math.max(6, Math.round(baseQuota * 0.86));
  return {
