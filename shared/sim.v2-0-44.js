...
} from './data.v2-0-44.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-44.js';
...
// v2.0.44: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    const laserLen = Math.round(activeScale(760 + Math.min(520, Math.max(0, lvl - 1) * 180)));
    const aim = activeAimPoint(p, laserLen, 150);
    const end = collideWalls(aim.x, aim.y, 5, run.plan.walls || [], p.x, p.y);
    const width = Math.round(activeScale(12 + Math.min(10, Math.max(0, lvl - 1) * 2.5)));
    const visualWidth = Math.max(2, Math.round(width * 0.42));
    const ttl = activeScale(2.6 + Math.min(2.0, Math.max(0, lvl - 1) * 0.55));
    const laserDmg = (30 + lvl * 14) * p.stats.dmgMul;
    ...
    p.invuln = Math.max(p.invuln || 0, 0.10 + lvl * 0.035);
    let cut = 0;
    ...
    for (const e of [...run.enemies]) if (distToSegment2(e.x, e.y, startX, startY, end.x, end.y) < (width + e.size / 2) ** 2) {
      e.activeSlowT = Math.max(e.activeSlowT || 0, 0.16 + lvl * 0.035);
      e.activeSlowMul = Math.min(e.activeSlowMul || 1, activeSoftMul(0.48));
      ctx.damageDone += activeDamageEnemy(run, players, e, laserDmg, p.id);
      ctx.hitCount++;
    }
    activeField(run, {
      kind: 'void_laser', owner: p.id, x: ctx.x, y: ctx.y, r: width,
      x1: Math.round(startX), y1: Math.round(startY), x2: Math.round(end.x), y2: Math.round(end.y), width, visualWidth,
      ttl, tickEvery: 0.16, dmg: (8 + lvl * 4.2) * p.stats.dmgMul, slow: 0.48, damp: 0.06
    });
    run.fx.push({ t: 'active_line', kind: 'void_laser', id: p.id, label: `VOID LASER ${roman(lvl)}${cut ? ' / ERASE ' + cut : ''}`, x1: Math.round(startX), y1: Math.round(startY), x2: Math.round(end.x), y2: Math.round(end.y), width: visualWidth, hitWidth: width, tone: 'purple' });
    run.fx.push({ t: 'active_mutation', label: `VOID LASER ${roman(lvl)}`, x: ctx.x, y: ctx.y, r: Math.max(44, width * 3), tone: 'purple', squareBlast: 1 });
