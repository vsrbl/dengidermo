...
} from './data.v2-0-59.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-59.js';
...
// v2.0.59: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function nextStaticRainLevel(run) {
  ...
  const fromCurrentRoom = run.staticRainCanSeedNext ? Math.max(0, run.roomStaticRainFalls || 0) : 0;
...
export function startRoom(run, players) {
  ...
  if (naturalStaticRain && run.staticRainStacks > 0) run.staticRainCanSeedNext = true;
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.59: VOID LASER is now a builder. Level I = one short segment.
...
export function buildSnapshot(run, players) {
  ...
  return {
    ...
    room: {
      ...
      staticRainStacks: run.staticRainStacks || 0, staticRainNext: nextStaticRainLevel(run), next: run.nextRoomPreview || null, sockets: run.roomSockets || [], wires: run.roomWires || [], betStakes: casinoStakeTable(run)
    },
