...
import { ROOM_MODS, ROOM_SEQUENCE, SPECIAL_ROOMS } from './data.v2-0-63.js';
...
export function generateRoom(seed, runDepth, loopIndex) {
  ...
  const modifierIds = [];
  if (loopIndex >= 1 && category !== 'boss' && specialRoomId !== 'chill_room') {
  ...
  const roomArchetype = chooseRoomArchetype(rng, category, specialRoomId, modifierIds, loopIndex);
