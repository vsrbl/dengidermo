...
import { ROOM_MODS, ROOM_SEQUENCE, SPECIAL_ROOMS } from './data.v2-0-77.js';
...
export function generateRoom(seed, runDepth, loopIndex, override = null) {
  ...
  if (!forcedMods && specialRoomId === 'signal_contract') {
    const contractMods = ['greed', 'static_rain', 'hunter_contract', 'casino_virus', 'moving_room', 'prism_grid', 'blood_tax', 'echo_walls', 'anchor_gravity', 'static_wires'];
  ...
  if (modifierIds.includes('casino_virus')) baseQuota = Math.max(8, Math.round(baseQuota * 0.75));
  return {
