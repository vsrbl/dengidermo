...
} from './data.v2-0-36.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-36.js';
...
// v2.0.36: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function stepEnemies(run, players, dt) {
  ...
  for (const e of [...run.enemies]) {
    ...
    if ((e.frozenT || 0) > 0) {
      ...
      // Frozen means no movement, no firing, no windup progress and no contact damage.
      continue;
    }
...
function activeCasinoRoll(run, p, ctx, opts = {}) {
  ...
  if (r < 0.11) {
    outcome = 'HIT'; label = 'CASINO HIT'; tone = 'red'; symbols = ['DMG', 'DMG', 'BAD'];
    ...
    outcome = 'DEBT'; label = 'STATIC DEBT'; tone = 'purple'; symbols = ['STC', 'DEBT', 'Q'];
    run.staticDebt = true;
  } else if (r < 0.38) {
    outcome = 'DOUBLE'; label = 'Q x2'; tone = 'cyan'; symbols = ['Q', 'Q', 'COPY'];
    if (!run.pendingActives) run.pendingActives = [];
    run.pendingActives.push({ owner: p.id, at: run.now + 0.34, core: ctx.core || ensureActive(p).core, level: lvl, echo: 1, skipCasino: 1 });
  } else if (r < 0.43) {
    outcome = 'TEN'; label = 'Q x10'; tone = 'green'; symbols = ['Q', 'Q', '10'];
    if (!run.pendingActives) run.pendingActives = [];
    for (let i = 0; i < 10; i++) {
      run.pendingActives.push({ owner: p.id, at: run.now + 0.18 + i * 0.16, core: ctx.core || ensureActive(p).core, level: 1, echo: 1, skipCasino: 1 });
    }
  } else if (r < 0.62) {
    outcome = 'GLD'; label = 'CASINO GLD'; tone = 'green'; symbols = ['GLD', 'GLD', 'PAY'];
    dropPickup(run, ctx.x, ctx.y, 'GLD', 10 + lvl * 5 + Math.round(Math.random() * 28));
  } else if (r < 0.78) {
    outcome = 'EXP'; label = 'CASINO EXP'; tone = 'cyan'; symbols = ['EXP', 'EXP', 'UP'];
    dropPickup(run, ctx.x, ctx.y, 'EXP', 10 + lvl * 6 + Math.round(Math.random() * 18));
  } else {
    outcome = 'HEAL'; label = 'CASINO HEAL'; tone = 'green'; symbols = ['HEA', 'HEA', 'OK'];
    p.hp = Math.min(maxHp(p), p.hp + 10 + lvl * 5 + (ctx.hitCount || 0));
  }

  run.fx.push({ t: 'active_casino_roll', id: p.id, x: Math.round(ctx.x), y: Math.round(ctx.y), symbols, outcome, label, tone });
  run.fx.push({ t: 'active_mutation', label, x: Math.round(ctx.x), y: Math.round(ctx.y), r: outcome === 'TEN' ? 155 : outcome === 'DOUBLE' ? 130 : 104, tone, squareBlast: outcome === 'HIT' || outcome === 'DEBT' ? 1 : 0 });
}
...
function stepActiveFields(run, players, dt) {
  ...
  for (const f of [...run.activeFields]) {
    ...
    if (f.tickT <= 0) {
      ...
      if (f.kind === 'blood_ring' || f.kind === 'red_static' || f.kind === 'snap_field' || f.kind === 'signal_spike' || f.kind === 'void_tear' || f.kind === 'void_line' || f.kind === 'anchor_field') {
