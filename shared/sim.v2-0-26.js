...
} from './data.v2-0-26.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-26.js';
...
function activeField(run, f) {
  ...
  run.activeFields.push({ id: nid(), tickT: 0.08, fxT: 0.02, ...f });
}
...
function applyActiveUnstableReactions(run, players, p, ctx, opts = {}) {
  ...
  if (activeHasAll(p, 'static', 'blood') && roll()) {
    activeField(run, { kind: 'red_static', owner: p.id, x: ctx.x, y: ctx.y, r: Math.round((ctx.r || 130) * 0.66), ttl: 1.55 + lvl * 0.18, tickEvery: 0.38, dmg: (5 + lvl * 2) * p.stats.dmgMul, slow: 0.48, damp: 0.33 });
  ...
  if (activeHasAll(p, 'static', 'shrapnel') && roll()) {
    ...
    activeField(run, { kind: 'static', owner: p.id, x: ctx.x, y: ctx.y, r: 72 + lvl * 12, ttl: 1.1, dmg: 0, slow: 0.34, damp: 0.26 });
...
function applyActiveMutations(run, players, p, ctx, opts = {}) {
  ...
  for (const id of mods) {
    if (id === 'static') {
      activeField(run, { kind: 'static', owner: p.id, x: ctx.x, y: ctx.y, r: Math.round((ctx.r || 120) * 0.72), ttl: 2.2 + lvl * 0.25, dmg: 0, slow: 0.38, damp: 0.42 });
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ctx.r = 118 + lvl * 28;
    activeField(run, { kind: 'blood_ring', owner: p.id, follow: 1, x: p.x, y: p.y, r: ctx.r, ttl: 2.0 + lvl * 0.35, tickEvery: 0.32, dmg: (8 + lvl * 5) * p.stats.dmgMul });
    run.fx.push({ t: 'active', id: p.id, label: `BLOOD RING ${roman(lvl)}`, x: Math.round(p.x), y: Math.round(p.y), r: ctx.r });
  } else if (core === 'field_snap') {
    ctx.r = 245 + lvl * 35;
    const pull = 58 + lvl * 18;
    for (const e of [...activeTargets(run, p.x, p.y, ctx.r)]) {
      const n = norm(p.x - e.x, p.y - e.y);
      e.x += n.x * pull; e.y += n.y * pull;
      ctx.damageDone += activeDamageEnemy(run, players, e, (14 + lvl * 7) * p.stats.dmgMul, p.id);
      ctx.hitCount++;
    }
    for (const pk of run.pickups) if (dist2(pk.x, pk.y, p.x, p.y) < (ctx.r + 80) ** 2) { const n = norm(p.x - pk.x, p.y - pk.y); pk.x += n.x * 90; pk.y += n.y * 90; }
    ...
    for (const b of run.bullets) {
      ...
      if (dist2(b.x, b.y, p.x, p.y) < (ctx.r + b.size) ** 2) {
        b.vx *= 0.18; b.vy *= 0.18; b.life = Math.min(b.life, 0.55 + lvl * 0.2); cut++;
        if (lvl >= 3 && Math.random() < 0.25) { b.from = 'p'; b.owner = p.id; b.dmg = 10 * p.stats.dmgMul; }
      }
    }
    ctx.hitCount = cut;
    activeField(run, { kind: 'static', owner: p.id, follow: 1, x: p.x, y: p.y, r: ctx.r, ttl: 0.9 + lvl * 0.28, dmg: 0, slow: 0.45, damp: 0.28, bulletsOnly: 1 });
    ...
    ctx.r = 175 + lvl * 35;
    for (const e of [...activeTargets(run, p.x, p.y, ctx.r)]) {
      const shell = activeCrackShell(run, e, 54 + lvl * 34, true);
      if (shell) ctx.hitCount++;
      ctx.damageDone += shell;
      if (!shell) { ctx.damageDone += activeDamageEnemy(run, players, e, (10 + lvl * 4) * p.stats.dmgMul, p.id); ctx.hitCount++; }
    }
...
function stepActiveFields(run, players, dt) {
  ...
  for (const f of [...run.activeFields]) {
    ...
    if (f.fxT <= 0) { f.fxT = 0.18; run.fx.push({ t: 'active_field', kind: f.kind, x: Math.round(f.x), y: Math.round(f.y), r: f.r, tone: f.kind === 'blood_ring' ? 'red' : (f.kind === 'red_static' ? 'purple' : 'cyan') }); }
    if (f.kind === 'static' || f.kind === 'red_static') {
      for (const e of run.enemies) if (!f.bulletsOnly && dist2(e.x, e.y, f.x, f.y) < (f.r + e.size / 2) ** 2) { e.activeSlowT = 0.18; e.activeSlowMul = f.slow || 0.55; }
      for (const b of run.bullets) if (dist2(b.x, b.y, f.x, f.y) < (f.r + b.size) ** 2) {
        const damp = Math.pow(f.damp || 0.45, dt * 3.0);
        b.vx *= damp; b.vy *= damp;
        if (Math.hypot(b.vx, b.vy) < 32 && b.from === 'e') { b.life = -1; run.fx.push({ t: 'bullet_stop', x: Math.round(b.x), y: Math.round(b.y), kind: b.kind || '' }); }
      }
    }
    if (f.tickT <= 0) {
      ...
      if (f.kind === 'blood_ring' || f.kind === 'red_static') {
        ...
        if (p && activeHasMutation(p, 'leech')) p.hp = Math.min(maxHp(p), p.hp + 1.2);
