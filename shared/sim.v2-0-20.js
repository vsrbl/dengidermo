...
} from './data.v2-0-20.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-20.js';
...
function fireWeapon(run, players, p, dt) {
  ...
  const life = (w.life + (w.id === 'seeker' ? p.stats.sekChain * 0.10 : 0)) * (w.id === 'rocketgun' ? 1 : rangeMul);
  ...
  for (let s = 0; s < shots; s++) {
    ...
    for (let i = 0; i < pellets; i++) {
      ...
      run.bullets.push({
        ...
        bounces: w.id === 'rocketgun' ? 0 : ((p.stats.bulletBounce || 0) + (w.id === 'shotgun' ? (p.stats.shgBounce || 0) : 0)),
  ...
  for (const b of run.bullets) {
    ...
    if (hitWall) {
      if (b.bounces > 0) {
        ...
        run.fx.push({ t: 'ricochet', x: Math.round(b.x), y: Math.round(b.y), kind: b.kind, dx: Math.round((b.vx || 0) / 10), dy: Math.round((b.vy || 0) / 10) });
        continue;
      }
...
export function handleCasino(run, players, p, stakeKey) {
  ...
  if (pl.ability) {
    // Casino ABL must always give a real, visible ability reward.
    // If the player has no Q-active yet, prefer installing one; otherwise roll dash/Q/mobility upgrades.
    const qPool = ['q_snap', 'q_blood', 'q_over'];
    const fullPool = ['dash', 'voidstep', 'dashcut', 'dashclone', 'q_snap', 'q_blood', 'q_over'];
    const hasQ = p.stats.activeSnap || p.stats.activeBlood || p.stats.activeOver;
    const pool = hasQ ? fullPool : qPool;
    const id = pool[Math.floor(Math.random() * pool.length)];
    const u = UPGRADES.find(x => x.id === id);
    if (u) {
      u.apply(p.stats);
      pl.abilityLabel = u.label;
      if (id === 'dash') pl.dash = 1;
      if (id === 'dash') p.dashCharges = Math.min(dashMax(p), p.dashCharges + 1);
      run.fx.push({ t: 'ability_get', id: p.id, label: u.label, x: Math.round(p.x), y: Math.round(p.y) });
    } else {
