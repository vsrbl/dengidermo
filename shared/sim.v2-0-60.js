...
} from './data.v2-0-60.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-60.js';
...
// v2.0.60: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function makeNextRoomPreview(run) {
  ...
  return {
    ...
    mods: (plan.modifierIds || []).slice(0, 4), quota: plan.quota || 0, rewardTags: roomModRewardTags(plan.modifierIds || [], plan.specialRoomId || '')
  };
}
...
export function startRoom(run, players) {
  ...
  run.fx.push({ t: 'room', roomId: run.plan.roomId, loop: loopIndex, depth: run.runDepth, mods: run.plan.modifierIds, cat: run.plan.category, special: run.plan.specialRoomId, archetype: run.plan.roomArchetype || 'standard', director: run.director?.label || '', skinRarity: run.skinRoomReward?.rarity || '' });
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.60: VOID LASER is now a builder. Level I = one short segment.
...
export function buildSnapshot(run, players) {
  ...
  return {
    ...
    room: {
      ...
      staticRainStacks: run.staticRainStacks || 0, staticRainNext: nextStaticRainLevel(run), staticRainMode: staticRainCurrentMode(run), next: run.nextRoomPreview || null, sockets: run.roomSockets || [], wires: run.roomWires || [], betStakes: casinoStakeTable(run)
    },
