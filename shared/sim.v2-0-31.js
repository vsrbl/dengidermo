...
import {
  ...
  rollUpgradeChoices, defaultStats, spinCasino, rollRoomSkin, UPGRADE_LABELS, SKIN_PRESETS
} from './data.v2-0-31.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-31.js';
...
function makeUpgradeCoreChoice(p) {
  ...
  return addActiveChoiceMeta({
    ...
    preview: `Level ${a.level + 1}: ${c.upgrade?.join(' · ') || '+сила'}. Прирост должен быть заметен в бою.`,
...
function makeMutationChoice(mutId, p, replaceIdx = -1) {
  ...
  return addActiveChoiceMeta({
    ...
    preview: `${m.label} встраивается в текущую Q. При 2+ мутациях сигнал может грязно сшиться во время каста.`,
...
function activeExposeEnemy(run, e, lvl, owner) {
  ...
  e.exposedT = Math.max(e.exposedT || 0, 3.6 + lvl * 0.65);
  e.exposedMul = Math.max(oldMul, 1.24 + lvl * 0.08);
...
function applyActiveUnstableReactions(run, players, p, ctx, opts = {}) {
  ...
  if (activeHasAll(p, 'chain', 'shrapnel') && roll()) {
    activeNeedles(run, p, ctx.x, ctx.y, 10 + lvl * 2, 520, 5 + lvl * 2, 'wire_storm');
    activeNoise(run, 'WIRE STORM', ctx.x, ctx.y, (ctx.r || 130) + 35, 'cyan');
    fired++;
  }
  ...
  if (activeHasAll(p, 'hunger', 'blood') && roll()) {
    ...
    explode(run, players, ctx.x, ctx.y, 70 + lvl * 14, bite * p.stats.dmgMul, p.id, false, 'blood');
  ...
  if (activeHasAll(p, 'bad_tape', 'casino') && roll()) {
    ...
    else run.staticDebt = true;
    activeNoise(run, 'FALSE REEL', ctx.x, ctx.y, 118, Math.random() < 0.5 ? 'green' : 'purple');
    fired++;
  }
  if (ctx.core === 'black_box' && activeHasAll(p, 'chain') && roll()) {
    for (const b of run.bullets) if (b.from === 'e' && dist2(b.x, b.y, ctx.x, ctx.y) < ((ctx.r || 160) + b.size) ** 2 && Math.random() < 0.55) b.life = -1;
    activeNoise(run, 'BOX CHAIN', ctx.x, ctx.y, (ctx.r || 150) + 45, 'purple');
    fired++;
  }
}
...
function applyActiveMutations(run, players, p, ctx, opts = {}) {
  ...
  for (const id of mods) {
    if (id === 'static') {
      activeField(run, { kind: 'static', owner: p.id, x: ctx.x, y: ctx.y, r: Math.round((ctx.r || 120) * 0.86), ttl: activeDurationForLevel(lvl, 4.5, 6.0), dmg: 0, slow: 0.30, damp: 0.24 });
      run.fx.push({ t: 'active_mutation', label: 'STATIC', x: Math.round(ctx.x), y: Math.round(ctx.y), r: Math.round((ctx.r || 120) * 0.86), tone: 'cyan' });
    } else if (id === 'blood') {
      ...
      dashFx(run, p, p.x - (p.dirX || 1) * 20, p.y - (p.dirY || 0) * 20, p.x + (p.dirX || 1) * 20, p.y + (p.dirY || 0) * 20, { phase: 1 });
      ...
      const targets = activeTargets(run, ctx.x, ctx.y, (ctx.r || 150) + 90)
        .sort((a, b) => dist2(a.x, a.y, ctx.x, ctx.y) - dist2(b.x, b.y, ctx.x, ctx.y))
        .slice(0, 3 + lvl * 2);
      let jump = 0;
      for (const e of [...targets]) {
        if (!run.enemies.includes(e)) continue;
        e.activeSlowT = Math.max(e.activeSlowT || 0, 0.22 + lvl * 0.04);
        e.activeSlowMul = Math.min(e.activeSlowMul || 1, 0.42);
        ctx.damageDone += activeDamageEnemy(run, players, e, (8 + lvl * 4 + jump * 1.4) * p.stats.dmgMul, p.id);
        ctx.hitCount++; jump++;
      }
      if (targets.length) run.fx.push({ t: 'active_mutation', label: `CHAIN x${targets.length}`, x: Math.round(ctx.x), y: Math.round(ctx.y), r: (ctx.r || 120) + 70, tone: 'cyan' });
    } else if (id === 'anchor') {
      activeField(run, { kind: 'anchor_field', owner: p.id, x: ctx.x, y: ctx.y, r: Math.round((ctx.r || 130) * 0.58 + 88), ttl: activeDurationForLevel(lvl, 2.9, 4.4), tickEvery: 0.30, pull: activeRadiusForLevel(lvl, 120, 240), dmg: (2 + lvl * 2) * p.stats.dmgMul, slow: 0.32, damp: 0.20 });
      run.fx.push({ t: 'active_mutation', label: 'ANCHOR', x: Math.round(ctx.x), y: Math.round(ctx.y), r: Math.round((ctx.r || 130) * 0.62 + 95), tone: 'purple' });
    } else if (id === 'hunger') {
      const targets = activeTargets(run, ctx.x, ctx.y, (ctx.r || 130) + 60);
      const bite = Math.min(46, 7 + targets.length * 3.4 + lvl * 4);
      if (targets.length) {
        explode(run, players, ctx.x, ctx.y, 62 + lvl * 13 + Math.min(52, targets.length * 4), bite * p.stats.dmgMul, p.id, false, 'blood');
        ctx.damageDone += bite;
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    const startR = activeRadiusForLevel(lvl, 112, 156);
    ctx.r = activeRadiusForLevel(lvl, 205, 335);
    activeField(run, { kind: 'blood_ring', owner: p.id, follow: 1, x: p.x, y: p.y, r: startR, startR, growTo: ctx.r, growTime: 1.15 + lvl * 0.18, ttl: activeDurationForLevel(lvl, 4.8, 7.5), tickEvery: 0.24, dmg: (11 + lvl * 9) * p.stats.dmgMul });
    ...
    for (const pk of run.pickups) if (dist2(pk.x, pk.y, p.x, p.y) < (ctx.r + 140) ** 2) { const n = norm(p.x - pk.x, p.y - pk.y); pk.x += n.x * 160; pk.y += n.y * 160; }
    ...
    for (const b of run.bullets) {
      ...
      if (dist2(b.x, b.y, p.x, p.y) < (ctx.r + b.size) ** 2) {
        b.vx *= 0.07; b.vy *= 0.07; b.life = Math.min(b.life, 1.2 + lvl * 0.32); cut++;
        if (lvl >= 3 && Math.random() < 0.34) { b.from = 'p'; b.owner = p.id; b.dmg = 12 * p.stats.dmgMul; }
      }
    }
    for (const e of [...activeTargets(run, p.x, p.y, ctx.r)]) { e.activeSlowT = Math.max(e.activeSlowT || 0, 0.42); e.activeSlowMul = Math.min(e.activeSlowMul || 1, 0.20 + lvl * 0.03); ctx.hitCount++; }
    ctx.hitCount += cut;
    activeField(run, { kind: 'freeze_aura', owner: p.id, follow: 1, x: p.x, y: p.y, r: ctx.r, ttl: activeDurationForLevel(lvl, 5.1, 7.8), dmg: 0, slow: 0.20, damp: 0.07 });
    ...
    const aim = activeAimPoint(p, activeRadiusForLevel(lvl, 460, 760), 150);
    const end = collideWalls(aim.x, aim.y, 8, run.plan.walls || [], p.x, p.y);
    const width = activeRadiusForLevel(lvl, 42, 74);
    const ttl = activeDurationForLevel(lvl, 3.2, 5.4);
    ...
    p.invuln = Math.max(p.invuln || 0, 0.18 + lvl * 0.06);
    let cut = 0;
    ...
    for (const e of [...run.enemies]) if (distToSegment2(e.x, e.y, startX, startY, end.x, end.y) < (width + e.size / 2) ** 2) {
      activeExposeEnemy(run, e, Math.max(1, lvl - 1), p.id);
      e.activeSlowT = Math.max(e.activeSlowT || 0, 0.28 + lvl * 0.06);
      e.activeSlowMul = Math.min(e.activeSlowMul || 1, 0.38);
      ctx.damageDone += activeDamageEnemy(run, players, e, (22 + lvl * 13) * p.stats.dmgMul, p.id);
      ctx.hitCount++;
    }
    activeField(run, {
      kind: 'void_line', owner: p.id, x: ctx.x, y: ctx.y, r: width,
      x1: Math.round(startX), y1: Math.round(startY), x2: Math.round(end.x), y2: Math.round(end.y), width,
      ttl, tickEvery: 0.18, dmg: (9 + lvl * 5) * p.stats.dmgMul, slow: 0.34, damp: 0.08
    });
    run.fx.push({ t: 'active_line', id: p.id, label: `VOID CUT ${roman(lvl)}${cut ? ' / ERASE ' + cut : ''}`, x1: Math.round(startX), y1: Math.round(startY), x2: Math.round(end.x), y2: Math.round(end.y), width, tone: 'purple' });
    run.fx.push({ t: 'active', id: p.id, label: `VOID CUT ${roman(lvl)}`, x: ctx.x, y: ctx.y, r: Math.max(95, width * 2) });
  } else if (core === 'signal_spike') {
    const aim = activeAimPoint(p, activeRadiusForLevel(lvl, 420, 680), 140);
    ctx.x = Math.round(aim.x); ctx.y = Math.round(aim.y); ctx.r = activeRadiusForLevel(lvl, 170, 285);
    activeField(run, { kind: 'signal_spike', owner: p.id, x: ctx.x, y: ctx.y, r: ctx.r, ttl: activeDurationForLevel(lvl, 5.4, 8.1), tickEvery: 0.38, pull: activeRadiusForLevel(lvl, 42, 98), dmg: (10 + lvl * 7) * p.stats.dmgMul, slow: 0.42, damp: 0.21 });
    for (const b of run.bullets) if (b.from === 'e' && dist2(b.x, b.y, ctx.x, ctx.y) < (ctx.r + b.size) ** 2) { b.vx *= 0.25; b.vy *= 0.25; ctx.hitCount++; }
    run.fx.push({ t: 'active', id: p.id, label: `SIGNAL SPIKE ${roman(lvl)}`, x: ctx.x, y: ctx.y, r: ctx.r });
  } else if (core === 'black_box') {
    ctx.r = activeRadiusForLevel(lvl, 230, 385);
    let erased = 0;
    for (const b of run.bullets) if (b.from === 'e' && dist2(b.x, b.y, p.x, p.y) < (ctx.r + b.size) ** 2) { b.vx *= 0.04; b.vy *= 0.04; if (Math.random() < 0.55 + lvl * 0.12) { b.life = -1; erased++; } }
    for (const e of [...activeTargets(run, p.x, p.y, ctx.r)]) { e.activeSlowT = Math.max(e.activeSlowT || 0, 0.62); e.activeSlowMul = Math.min(e.activeSlowMul || 1, 0.18 + lvl * 0.03); e.fireCd = Math.max(e.fireCd || 0, 0.75 + lvl * 0.16); ctx.hitCount++; }
    activeField(run, { kind: 'black_box', owner: p.id, follow: 1, x: p.x, y: p.y, r: ctx.r, ttl: activeDurationForLevel(lvl, 4.7, 7.2), tickEvery: 0.32, dmg: (2 + lvl * 1.5) * p.stats.dmgMul, slow: 0.18, damp: 0.045 });
    run.fx.push({ t: 'active', id: p.id, label: `BLACK BOX ${roman(lvl)}${erased ? ' / ERASE ' + erased : ''}`, x: Math.round(p.x), y: Math.round(p.y), r: ctx.r });
    ...
    for (const e of [...activeTargets(run, p.x, p.y, ctx.r)]) {
      const n = norm(e.x - p.x, e.y - p.y);
      e.x += n.x * (34 + lvl * 15); e.y += n.y * (34 + lvl * 15);
      activeExposeEnemy(run, e, lvl + 1, p.id);
      ctx.damageDone += activeDamageEnemy(run, players, e, (24 + lvl * 18) * p.stats.dmgMul, p.id);
      ctx.hitCount++;
    }
    if (debtRoll) { run.staticDebt = true; run.fx.push({ t: 'debt', id: p.id, x: Math.round(p.x), y: Math.round(p.y) }); }
...
function stepActiveFields(run, players, dt) {
  ...
  for (const f of [...run.activeFields]) {
    ...
    const fieldTone = f.kind === 'blood_ring' ? 'red'
      : (f.kind === 'red_static' || f.kind === 'void_tear' || f.kind === 'void_line' || f.kind === 'black_box' || f.kind === 'anchor_field') ? 'purple'
      : 'cyan';
    if (f.fxT <= 0) {
      f.fxT = f.kind === 'void_line' ? 0.10 : 0.18;
      if (f.kind === 'void_line') run.fx.push({ t: 'active_line', kind: f.kind, x1: f.x1, y1: f.y1, x2: f.x2, y2: f.y2, width: f.width || f.r || 42, tone: fieldTone });
      else run.fx.push({ t: 'active_field', kind: f.kind, x: Math.round(f.x), y: Math.round(f.y), r: f.r, tone: fieldTone });
    }
    if (f.kind === 'static' || f.kind === 'red_static' || f.kind === 'freeze_aura' || f.kind === 'signal_spike' || f.kind === 'black_box' || f.kind === 'void_tear' || f.kind === 'void_line' || f.kind === 'anchor_field') {
      if (f.kind === 'void_line') {
      ...
      for (const e of run.enemies) if (!f.bulletsOnly && dist2(e.x, e.y, f.x, f.y) < (f.r + e.size / 2) ** 2) {
        e.activeSlowT = Math.max(e.activeSlowT || 0, f.kind === 'black_box' ? 0.34 : 0.22);
        e.activeSlowMul = Math.min(e.activeSlowMul || 1, f.slow || 0.55);
        if (f.kind === 'signal_spike' || f.kind === 'anchor_field') { const n = norm(f.x - e.x, f.y - e.y); e.x += n.x * (f.pull || 65) * dt; e.y += n.y * (f.pull || 65) * dt; }
        if (f.kind === 'black_box') e.fireCd = Math.max(e.fireCd || 0, 0.18);
      }
      for (const b of run.bullets) if (dist2(b.x, b.y, f.x, f.y) < (f.r + b.size) ** 2) {
        const damp = Math.pow(f.damp || 0.45, dt * (f.kind === 'freeze_aura' ? 5.2 : f.kind === 'black_box' ? 6.2 : 3.0));
        b.vx *= damp; b.vy *= damp;
        const stopSpeed = f.kind === 'freeze_aura' ? 58 : f.kind === 'black_box' ? 78 : 32;
    ...
    if (f.kind === 'snap_field') {
      for (const e of run.enemies) if (dist2(e.x, e.y, f.x, f.y) < (f.r + e.size / 2) ** 2) {
        ...
        e.activeSlowMul = Math.min(e.activeSlowMul || 1, f.slow || 0.72);
      }
      ...
      for (const b of run.bullets) if (b.from === 'e' && dist2(b.x, b.y, f.x, f.y) < (f.r + b.size) ** 2) { const damp = Math.pow(f.damp || 0.36, dt * 2.4); b.vx *= damp; b.vy *= damp; }
    }
    if (f.tickT <= 0) {
      f.tickT = f.tickEvery || 0.35;
      if (f.kind === 'blood_ring' || f.kind === 'red_static' || f.kind === 'snap_field' || f.kind === 'signal_spike' || f.kind === 'black_box' || f.kind === 'void_tear' || f.kind === 'void_line' || f.kind === 'anchor_field') {
        ...
        if (f.kind === 'void_line') {
          ...
          run.fx.push({ t: 'active_line_tick', kind: f.kind, x1: f.x1, y1: f.y1, x2: f.x2, y2: f.y2, width: rr, tone: 'purple' });
        } else {
