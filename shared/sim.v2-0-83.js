...
} from './data.v2-0-83.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-83.js';
...
const HERALD_PATH_FOLLOW_SPEED = 235; // v2.0.83: floor call-line follows dashes, but never teleports onto the player.
const clampStaticRainLevel = v => Math.max(0, Math.min(STATIC_RAIN_MAX_LEVEL, v | 0));


// v2.0.83: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function loopCostMul(run) {
  // v2.0.83: costs were far too flat. By loop 2 a player could sit on ~2k GLD
...
function roomObjectiveStatus(run, st = run?.roomStats || {}) {
  ...
  const time = roomSolvedTime(run, st);
  const solved = roomSolvedAt(run, st) > 0;
  // v2.0.83 rule: every room contract is finalised ONLY when the portal opens.
...
function rewardShellMarket(run, x, y) {
  // Shell Market modifier was removed in v2.0.83. Keep shell-break memory only.
...
function fireEchoMimicShot(run, players, e, target, toT, def) {
  ...
  e.mimicWeapon = wid;
  run.fx.push({ t: 'echo_shot', id: e.id, x: Math.round(e.x), y: Math.round(e.y), weapon: wid });
}
  ...
  for (const d of dampers) {
    ...
    for (const e of run.enemies) {
      ...
      if (['shooter','prism','pulse','orbiter','echo'].includes(e.kind)) {
        ...
        e.rallyT = Math.max(e.rallyT || 0, 0.75);
        if (dist2(e.x, e.y, d.x, d.y) > (def.fieldR * 0.78) ** 2) {
          e.rallyTargetId = '';
          const pull = norm(d.x - e.x, d.y - e.y);
          steerMove(run, e, pull, Math.max(30, enemySpeed(e) * 0.42), dt, { target: d });
        }
        guarded++;
      }
      if (guarded >= 9) break;
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss' && !isGreedRoom(run)) {
    ...
    // v2.0.83: skin rooms were too rare. Raise the base rate and add soft pity.
...
function director(run, players, dt) {
  ...
  if (casinoVirusActive) {
    // v2.0.83: normal director remains alive during the 3 virus spins, but is capped.
...
function stepEnemies(run, players, dt) {
  ...
  for (const e of [...run.enemies]) {
    ...
    if (e.kind === 'shooter') {
      ...
      // v2.0.83: DMP is no longer a static turret. It slowly walks the nest toward the player,
      // while keeping enough distance to remain a protection core instead of a body-blocker.
      const desired = dT > 500 ? 1 : dT < 330 ? -0.35 : 0.18;
      const side = { x: -toT.y * 0.25, y: toT.x * 0.25 };
      const drift = { x: toT.x * desired + side.x, y: toT.y * desired + side.y };
      steerMove(run, e, drift, spd * (desired < 0 ? 0.55 : 0.82), dt, { target });
      e.dirX = toT.x; e.dirY = toT.y;
      e.fxT = (e.fxT || 0) - dt;
      if (e.fxT <= 0) {
        e.fxT = 0.18;
      ...
      if ((e.summonWindT || 0) > 0) {
        ...
        // v2.0.83: the floor line is a pursuing signal, not a hitscan reticle.
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.83: VOID LASER is now a builder. Level I = one short segment.
