...
} from './data.v2-0-77.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-77.js';
...
// v2.0.77: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
const REMOVED_ROOM_MODS = new Set(['debt_floor', 'shell_market', 'hunted_exit', 'mirror_room']);
...
function bloodTaxHpCost(cost) { return Math.max(1, Math.round(Number(cost) || 0)); }
...
function roomModRewardTags(mods = [], special = '') {
  const tags = [];
  if (mods.includes('greed')) tags.push('GLD ONLY');
...
function roomDangerScore(plan = {}, staticLevel = 0) {
  ...
  for (const m of mods) {
    ...
    else if (m === 'static_wires' || m === 'blood_tax') score += 0.65;
...
function roomThreatTags(plan = {}, staticLevel = 0) {
  ...
  if (mods.includes('moving_room')) tags.push('MOVING SPIKES');
  ...
  if (mods.includes('echo_walls')) tags.push('50% ECHO');
...
function roomTip(plan = {}, staticLevel = 0, staticMode = '') {
  ...
  if (mods.includes('casino_virus')) return 'CASINO VIRUS: survive 3 timed spins, then clear enemies to open portal.';
  if (mods.includes('moving_room')) return 'SHIFTING WALLS: spiked walls move and can box players or mobs.';
  ...
  if (mods.includes('echo_walls')) return 'ECHO WALLS: every shot has 50% chance to echo, including enemy shots.';
  if (mods.includes('greed')) return 'GREED SIGNAL: room pays only GLD; damage spends personal GLD and can go negative.';
...
function roomObjectiveForPlan(plan = {}, depth = 0) {
  ...
  if (mods.includes('hunter_contract')) return { id: 'hunter_waves', label: 'HUNTER WAVES', reward: 'GLD+EXP', goal: 'Survive every locked hunter wave.' };
  if (mods.includes('casino_virus')) return { id: 'virus_clear', label: 'VIRUS CLEAN', reward: 'GLD+EXP', goal: 'Survive 3 virus spins, then clear the room.' };
  if (mods.includes('static_wires')) return { id: 'wire_ghost', label: 'WIRE GHOST', reward: 'EXP', goal: 'Clear without touching static wires.' };
  if (mods.includes('prism_grid')) return { id: 'grid_slow_clear', label: 'GRID WALKER', reward: 'EXP', goal: 'Clear the slow-grid room.' };
  if (mods.includes('blood_tax')) return { id: 'blood_paid', label: 'BLOOD PAID', reward: 'GLD', goal: 'Buy with HP and clear the room.' };
...
function roomObjectiveProgress(run, st = run?.roomStats || {}) {
  ...
  const progress = {
    ...
    virus_clear: run?.casinoVirus ? `${Math.max(0, run.casinoVirus.spinsLeft || 0)} SPINS LEFT` : '3 SPINS',
    wire_ghost: (st.wireTouches || 0) <= 0 ? 'CLEAN' : `${st.wireTouches} TOUCH`,
    grid_ghost: (st.prismHits || 0) <= 0 ? 'CLEAN' : `${st.prismHits} HIT`,
    grid_slow_clear: 'CLEAR',
    blood_paid: isBloodTaxRoom(run) ? 'HP PRICES' : `${st.bloodTaxes || 0}/3`,
...
function roomObjectiveDoneRaw(obj, run, st = {}, time = 0) {
  ...
  if (obj.id === 'hunter_waves') return roomSolvedAt(run, st) > 0 && !!run?.hunterWave?.done;
  if (obj.id === 'virus_clear') return roomSolvedAt(run, st) > 0 && !!run?.casinoVirus?.done;
  if (obj.id === 'wire_ghost') return (st.wireTouches || 0) <= 0;
  if (obj.id === 'grid_ghost') return (st.prismHits || 0) <= 0;
  if (obj.id === 'grid_slow_clear') return true;
  if (obj.id === 'blood_paid') return true;
...
function roomObjectiveFailReason(obj, run, st = {}, time = 0) {
  ...
  if (obj.id === 'virus_clear' && !run?.casinoVirus?.done) return 'SPINS LEFT';
  return '';
}
function roomObjectiveStatus(run, st = run?.roomStats || {}) {
  ...
  const time = roomSolvedTime(run, st);
  const solved = roomSolvedAt(run, st) > 0;
  // v2.0.77 rule: every room contract is finalised ONLY when the portal opens.
...
function rewardShellMarket(run, x, y) {
  // Shell Market modifier was removed in v2.0.77. Keep shell-break memory only.
...
function quotaCanOpenPortal(run) {
  ...
  if (hasMod(run, 'casino_virus')) return !!run.casinoVirus?.done && run.enemies.length <= 0;
  return run.kills >= run.plan.quota;
}
...
export function startRoom(run, players) {
  ...
  if ((run.plan.modifierIds || []).includes('casino_virus')) {
    run.casinoVirus = { spinsLeft: 3, nextSpin: 30, done: false };
  }
  ...
  if (run.plan.category !== 'boss' && !isGreedRoom(run)) {
    ...
    // v2.0.77: skin rooms were too rare. Raise the base rate and add soft pity.
  ...
  if (run.plan.category === 'boss') spawnEnemy(run, players, 'boss', false);
    ...
    run.fx.push({ t: 'contract', label: 'HUNTER WAVES', body: `${run.hunterWave?.total || 2} WAVES · PORTAL LOCKED` });
  }
...
function pickDirectorMode(run) {
  ...
  if (mods.includes('moving_room')) return { id: 'moving_room', label: 'SHIFTING WALLS', intents: { control: 2.0, swarm: 1.1, chaos: 1.1 } };
...
function spawnCasinoVirusEvent(run, players) {
  const cv = run.casinoVirus;
  if (!cv || cv.spinsLeft <= 0) return;
  cv.spinsLeft -= 1;
  const roll = Math.random();
  let label = 'VIRUS PACK';
  const center = spawnClusterPoint(run, players);
  if (roll < 0.26) {
    label = 'VIRUS MOB PACK';
    const n = 5 + Math.floor(Math.random() * 5) + Math.floor((run.runDepth || 0) / 4);
    const pool = ['runner','glitch','bouncer','shooter','charger'];
    for (let i = 0; i < n && run.enemies.length < MAX_ENEMIES; i++) spawnEnemy(run, players, pool[Math.floor(Math.random() * pool.length)], i > 3, offsetSpawnPos(run, center, i, n));
  } else if (roll < 0.48) {
    label = 'VIRUS STATIC RAIN';
    const r = 76, n = 2;
    for (let i = 0; i < n; i++) {
      const x = center.x + (Math.random() - 0.5) * 260;
      const y = center.y + (Math.random() - 0.5) * 220;
      run.fx.push({ t: 'rain_warn', x: Math.round(x), y: Math.round(y), r, dur: 1.05, stacks: 2 });
      run.pendingStrikes.push({ x, y, r, dmgP: 17, dmgE: 55, at: (run.now || 0) + 1.05, stacks: 2 });
    }
  } else if (roll < 0.66) {
    label = 'BIG STATIC RAIN';
    const r = 128;
    run.fx.push({ t: 'rain_warn', x: Math.round(center.x), y: Math.round(center.y), r, dur: 1.30, stacks: 5 });
    run.pendingStrikes.push({ x: center.x, y: center.y, r, dmgP: 28, dmgE: 110, at: (run.now || 0) + 1.30, stacks: 5 });
  } else if (roll < 0.80) {
    label = 'VIRUS ELITE PACK';
    const pool = ['tank','charger','leech','warden','damper','prism'];
    const n = 3 + Math.floor(Math.random() * 3);
    for (let i = 0; i < n && run.enemies.length < MAX_ENEMIES; i++) spawnEnemy(run, players, pool[Math.floor(Math.random() * pool.length)], true, offsetSpawnPos(run, center, i, n));
  } else if (roll < 0.92) {
    label = 'VIRUS MINI BOSS';
    const e = spawnEnemy(run, players, 'herald', true, center);
    e.maxHp = Math.round(e.maxHp * 1.65); e.hp = e.maxHp; e.dmg = Math.round(e.dmg * 1.25); e.virusBoss = true;
  } else {
    label = 'VIRUS SLOT JACKPOT';
    const n = 6;
  ...
  run.fx.push({ t: 'casino_virus_spin', label, spinsLeft: cv.spinsLeft, x: Math.round(center.x), y: Math.round(center.y) });
  if (cv.spinsLeft <= 0) cv.done = run.enemies.length <= 0;
}
function stepCasinoVirus(run, players, dt) {
  if (!hasMod(run, 'casino_virus') || !run.casinoVirus || run.portal.open) return;
  const cv = run.casinoVirus;
  if (cv.spinsLeft > 0) {
    cv.nextSpin = Math.max(0, (cv.nextSpin || 30) - dt);
    if (cv.nextSpin <= 0) { spawnCasinoVirusEvent(run, players); cv.nextSpin = 30; }
...
function stepMovingWalls(run, players, dt) {
  ...
  for (const w of run.movingWalls) {
    ...
    w.dmgCd = Math.max(0, (w.dmgCd || 0) - dt);
    for (const p of players.values()) if (p.alive && pushCircleOutOfRect(p, PLAYER_SIZE / 2 + 3, w)) {
      p.x = clamp(p.x, 55, run.plan.w - 55); p.y = clamp(p.y, 55, run.plan.h - 55);
      if (w.dmgCd <= 0) { damagePlayer(run, p, 11 + Math.floor((run.runDepth || 0) * 0.45), w.x + w.w / 2, w.y + w.h / 2); w.dmgCd = 0.45; }
    }
    for (const e of [...run.enemies]) if (pushCircleOutOfRect(e, (e.size || 24) / 2 + 4, w)) damageEnemy(run, players, e, 18 + Math.floor((run.runDepth || 0) * 1.2), null, 0, 0, 0);
  }
}
  ...
  if (cost > 0) {
    if (isBloodTaxRoom(run)) {
      ...
      if (!canPayBloodCost(p, hpCost)) {
        run.fx.push({ t: 'denied', id: p.id, obj: o.id, x: o.x, y: o.y, cost: hpCost, have: Math.round(p.hp), chest: def.label, hpCost: 1 });
        return;
      }
      p.hp -= hpCost;
...
export function handleCasino(run, players, p, stakeKey, knownUnlockedSkins = []) {
  ...
  if (isBloodTaxRoom(run)) {
    ...
    if (!canPayBloodCost(p, hpCost)) {
      run.fx.push({ t: 'denied', id: p.id, obj: near.id, hpCost: 1, cost: hpCost, have: Math.round(p.hp) });
  ...
  if (isGreedRoom(run)) {
    ...
    const gld = Math.round((stake * (1.7 + Math.random() * 2.1)) * stakeScale);
    res = { symbols: ['GLD','GLD','GLD'], outcome: 'GLD', payload: { gld }, stake };
    p.economy.money += gld;
    recordPickupStat(run, 'GLD', gld);
  } else {
  ...
  const fx = { ok: true, t: 'casino', id: p.id, seq, symbols: res.symbols, outcome: res.outcome, payload: res.payload, stake, greed: isGreedRoom(run) ? 1 : 0, bloodTax: isBloodTaxRoom(run) ? 1 : 0 };
  run.fx.push(fx);
  return fx;
}
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.77: VOID LASER is now a builder. Level I = one short segment.
