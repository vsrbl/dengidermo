...
} from './data.v2-0-90.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-90.js';
...
const HERALD_PATH_FOLLOW_SPEED = 235; // v2.0.90: floor call-line follows dashes, but never teleports onto the player.
const clampStaticRainLevel = v => Math.max(0, Math.min(STATIC_RAIN_MAX_LEVEL, v | 0));


// v2.0.90: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function loopCostMul(run) {
  // v2.0.90: costs were far too flat. By loop 2 a player could sit on ~2k GLD
...
function contractFavorPool(chain = 1, run = null) {
  let pool = chain >= 4
    ? ['epic_reroll', 'portal_insurance', 'double_favor', 'skin_signal']
    : chain >= 2
      ? ['free_reroll', 'clear_debt', 'skin_signal', 'portal_insurance']
      : ['free_reroll', 'clear_debt', 'skin_signal'];
  ...
  return pool.length ? pool : ['free_reroll', 'skin_signal'];
}
...
function rewardShellMarket(run, x, y) {
  // Shell Market modifier was removed in v2.0.90. Keep shell-break memory only.
...
export function startRoom(run, players) {
  ...
  if ((run.plan.modifierIds || []).includes('casino_virus')) {
    run.casinoVirus = { spinsLeft: 3, nextSpin: 30, done: false, lastLabel: 'WAITING', activeRainStacks: 0, rainT: 0, spinSeq: 0, rainKind: '', lastSymbols: [], pendingEvent: null };
  }
  ...
  if (run.plan.category !== 'boss' && !isGreedRoom(run)) {
    ...
    // v2.0.90: skin rooms were too rare. Raise the base rate and add soft pity.
...
function director(run, players, dt) {
  ...
  if (casinoVirusActive) {
    // v2.0.90: normal director remains alive during the 3 virus spins, but is capped.
...
function stepEnemies(run, players, dt) {
  ...
  for (const e of [...run.enemies]) {
    ...
    if (e.kind === 'shooter') {
      ...
      // v2.0.90: DMP is no longer a static turret. It slowly walks the nest toward the player,
      ...
      if ((e.summonWindT || 0) > 0) {
        ...
        // v2.0.90: the floor line is a pursuing signal, not a hitscan reticle.
...
function buildCasinoVirusEvent(run, players) {
  ...
  const center = spawnClusterPoint(run, players);
  if (roll < 0.26) return { kind: 'mob_pack', label: 'VIRUS MOB PACK', center, n: 5 + Math.floor(Math.random() * 5) + Math.floor((run.runDepth || 0) / 4), pool: ['runner','glitch','bouncer','shooter','charger'] };
  if (roll < 0.48) return { kind: 'static', label: 'VIRUS STATIC STORM', center, stacks: 2 };
  if (roll < 0.66) return { kind: 'big_static', label: 'BIG STATIC STORM', center, stacks: 5 };
  if (roll < 0.80) return { kind: 'elite_pack', label: 'VIRUS ELITE PACK', center, n: 3 + Math.floor(Math.random() * 3), pool: ['tank','charger','leech','warden','damper','prism'] };
  if (roll < 0.92) return { kind: 'mini_boss', label: 'VIRUS MINI BOSS', center };
...
function stepCasinoVirus(run, players, dt) {
  ...
  if (cv.pendingEvent) {
    ...
    if (now >= applyAt) {
      ...
      cv.nextSpin = cv.spinsLeft > 0 ? 30 : 0;
    } else {
      return;
    }
  }
  if (cv.spinsLeft > 0) {
    cv.nextSpin = Math.max(0, (Number.isFinite(Number(cv.nextSpin)) ? Number(cv.nextSpin) : 30) - dt);
    if (cv.nextSpin <= 0) scheduleCasinoVirusEvent(run, players);
  } else if (!cv.pendingEvent && liveEnemyCount(run) <= 0) {
    cv.done = true;
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.90: VOID LASER is now a builder. Level I = one short segment.
