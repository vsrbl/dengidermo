...
} from './data.v2-0-30.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-30.js';
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    const aim = activeAimPoint(p, activeRadiusForLevel(lvl, 330, 540), 120);
    const end = collideWalls(aim.x, aim.y, PLAYER_SIZE / 2, run.plan.walls || [], p.x, p.y);
    const width = activeRadiusForLevel(lvl, 70, 118);
    ctx.x = Math.round((startX + end.x) / 2); ctx.y = Math.round((startY + end.y) / 2); ctx.r = Math.round(Math.hypot(end.x - startX, end.y - startY) * 0.5 + width);
    p.x = end.x; p.y = end.y; p.invuln = Math.max(p.invuln || 0, 0.42 + lvl * 0.12);
    let cut = 0;
    ...
    for (const e of [...run.enemies]) if (distToSegment2(e.x, e.y, startX, startY, end.x, end.y) < (width + e.size / 2) ** 2) {
      ...
      ctx.damageDone += activeDamageEnemy(run, players, e, (28 + lvl * 16) * p.stats.dmgMul, p.id);
      ctx.hitCount++;
    }
    activeField(run, { kind: 'void_tear', owner: p.id, x: ctx.x, y: ctx.y, r: width + 55, ttl: activeDurationForLevel(lvl, 1.8, 2.8), tickEvery: 0.24, dmg: (3 + lvl * 2) * p.stats.dmgMul, slow: 0.30, damp: 0.11 });
    dashFx(run, p, startX, startY, end.x, end.y, { phase: 1 });
    run.fx.push({ t: 'active', id: p.id, label: `VOID CUT ${roman(lvl)}${cut ? ' / BULLETS ' + cut : ''}`, x: ctx.x, y: ctx.y, r: ctx.r });
...
function stepActiveFields(run, players, dt) {
  ...
  for (const f of [...run.activeFields]) {
    ...
    const fieldTone = f.kind === 'blood_ring' ? 'red'
      : (f.kind === 'red_static' || f.kind === 'void_tear' || f.kind === 'black_box' || f.kind === 'anchor_field') ? 'purple'
      : 'cyan';
    if (f.fxT <= 0) { f.fxT = 0.18; run.fx.push({ t: 'active_field', kind: f.kind, x: Math.round(f.x), y: Math.round(f.y), r: f.r, tone: fieldTone }); }
    if (f.kind === 'static' || f.kind === 'red_static' || f.kind === 'freeze_aura' || f.kind === 'signal_spike' || f.kind === 'black_box' || f.kind === 'void_tear' || f.kind === 'anchor_field') {
    ...
    if (f.tickT <= 0) {
      ...
      if (f.kind === 'blood_ring' || f.kind === 'red_static' || f.kind === 'snap_field' || f.kind === 'signal_spike' || f.kind === 'black_box' || f.kind === 'void_tear' || f.kind === 'anchor_field') {
