...
} from './data.v2-0-66.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-66.js';
...
// v2.0.66: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function roomObjectiveForPlan(plan = {}, depth = 0) {
  ...
  if (mods.includes('hunted_exit')) return { id: 'overstay_survive', label: 'OVERSTAY TAPE', reward: 'GLD+EXP', goal: 'Survive at least one hunter wave.' };
...
function roomObjectiveProgress(run, st = run?.roomStats || {}) {
  ...
  if (!obj) return null;
  const time = Math.max(0, (run.now || 0) - (st.startedAt || run.now || 0));
...
function evaluateRoomObjective(run, st = {}, time = 0) {
  ...
  const lowDamageLimit = 60 + Math.floor((run?.runDepth || 0) * 1.5);
  let done = false;
  if (obj.id === 'boss_cut') done = true;
  else if (obj.id === 'lounge_cashout') done = true;
  else if (obj.id === 'overstay_survive') done = (st.huntedWaves || 0) >= 1;
  else if (obj.id === 'wire_ghost') done = (st.wireTouches || 0) <= 0;
  else if (obj.id === 'grid_ghost') done = (st.prismHits || 0) <= 0;
  else if (obj.id === 'shell_breaker') done = (st.shellBreaks || 0) >= 3;
  else if (obj.id === 'blood_paid') done = (st.bloodTaxes || 0) >= 3;
  else if (obj.id === 'static_clean') done = (st.damageTaken || 0) <= lowDamageLimit;
  else if (obj.id === 'cache_claim') done = true;
  else if (obj.id === 'fast_clear') done = time > 0 && time < 42;
  else if (obj.id === 'no_hit') done = (st.damageTaken || 0) <= 0;
  else if (obj.id === 'clean_signal') done = true;
  const depth = run?.runDepth || 0;
  const gld = done ? (obj.id.includes('shell') || obj.id.includes('blood') || obj.id.includes('no_hit') || obj.id === 'clean_signal' ? 12 + Math.floor(depth * 1.4) : 6 + Math.floor(depth * 0.7)) : 0;
  const exp = done ? (obj.id.includes('fast') || obj.id.includes('wire') || obj.id.includes('grid') || obj.id.includes('static') || obj.id === 'clean_signal' ? 10 + Math.floor(depth * 1.25) : 5 + Math.floor(depth * 0.65)) : 0;
  return { ...obj, done: done ? 1 : 0, bonusGld: gld, bonusExp: exp, progress: roomObjectiveProgress(run, st) };
}
...
function makeNextRoomPreview(run) {
  ...
  return {
    ...
    objective: roomObjectiveForPlan(plan, depth)
  };
}
function initRoomStats(run) {
  run.roomStats = { kills: 0, gld: 0, exp: 0, hea: 0, damageTaken: 0, shellBreaks: 0, prismHits: 0, bloodTaxes: 0, wireTouches: 0, huntedWaves: 0, startedAt: run.now || 0 };
}
...
function updateRunMemoryFromRoom(run, st, flags = {}) {
  ...
  mem.totalGld = (mem.totalGld || 0) + Math.max(0, st?.gld || 0) + Math.max(0, flags.bonusGld || 0);
  mem.totalExp = (mem.totalExp || 0) + Math.max(0, st?.exp || 0) + Math.max(0, flags.bonusExp || 0);
...
export function createRun(seedBase) {
  return {
    ...
    roomStats: null, roomObjective: null, nextRoomPreview: null, roomSockets: [], roomWires: [], prismLaneT: 0, pendingPrismLanes: [], pendingBloodTax: [], huntedExitOpenedAt: 0, huntedExitSpawnT: 0,
    runMemory: { roomsCleared: 0, totalKills: 0, totalGld: 0, totalExp: 0, totalDamageTaken: 0, noHitStreak: 0, fastStreak: 0, bestNoHitStreak: 0, bestFastStreak: 0, skinRoomsSeen: 0, staticPaid: 0, shellBreaks: 0, huntedWaves: 0, objectivesSeen: 0, objectivesDone: 0, objectiveGld: 0, objectiveExp: 0 },
    tapeLog: [],
    plan: null,
...
export function startRoom(run, players) {
  ...
  if (run.plan.category !== 'boss') {
    ...
    // v2.0.66: skin rooms were too rare. Raise the base rate and add soft pity.
...
export function resetRun(run, players) {
  ...
  run.staticRainFromPending = false;
  run.roomStaticRainFalls = 0;
  run.roomStats = null; run.roomObjective = null; run.nextRoomPreview = null; run.roomSockets = []; run.roomWires = []; run.pendingPrismLanes = []; run.pendingBloodTax = []; run.huntedExitOpenedAt = 0; run.huntedExitSpawnT = 0;
  run.runMemory = { roomsCleared: 0, totalKills: 0, totalGld: 0, totalExp: 0, totalDamageTaken: 0, noHitStreak: 0, fastStreak: 0, bestNoHitStreak: 0, bestFastStreak: 0, skinRoomsSeen: 0, staticPaid: 0, shellBreaks: 0, huntedWaves: 0, objectivesSeen: 0, objectivesDone: 0, objectiveGld: 0, objectiveExp: 0 };
  run.tapeLog = [];
  for (const p of players.values()) {
...
function beginTransition(run, players) {
  ...
  const objResult = evaluateRoomObjective(run, st, time);
  ...
  if (bonusGld || bonusExp || objectiveBonusGld || objectiveBonusExp) {
    for (const p of players.values()) if (p.connected) {
      if (bonusGld || objectiveBonusGld) p.economy.money += bonusGld + objectiveBonusGld;
      if (bonusExp || objectiveBonusExp) addXp(run, p, bonusExp + objectiveBonusExp);
    }
  }
  updateRunMemoryFromRoom(run, st, { noHit, fast, bonusGld, bonusExp, objectiveSeen: !!objResult, objectiveDone: !!objResult?.done, objectiveGld: objectiveBonusGld, objectiveExp: objectiveBonusExp });
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.66: VOID LASER is now a builder. Level I = one short segment.
...
export function buildSnapshot(run, players) {
  ...
  return {
    ...
    room: {
      ...
      objective: run.roomObjective ? { ...run.roomObjective, progress: roomObjectiveProgress(run) } : null,
