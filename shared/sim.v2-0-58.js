...
} from './data.v2-0-58.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-58.js';
...
// v2.0.58: active abilities were reading too hard. Keep the fantasy, but cut duration/radius/power by ~1.5x.
...
function initRoomStats(run) {
  run.roomStats = { kills: 0, gld: 0, exp: 0, hea: 0, damageTaken: 0, startedAt: run.now || 0 };
}
...
export function createRun(seedBase) {
  return {
    ...
    roomStats: null, nextRoomPreview: null, roomSockets: [], prismLaneT: 0, pendingPrismLanes: [], pendingBloodTax: [],
    plan: null,
...
export function resetRun(run, players) {
  ...
  run.staticRainFromPending = false;
  run.roomStaticRainFalls = 0;
  run.roomStats = null; run.nextRoomPreview = null; run.roomSockets = []; run.pendingPrismLanes = []; run.pendingBloodTax = [];
  for (const p of players.values()) {
...
function stepMods(run, players, dt) {
  ...
  if (run.pendingPrismLanes?.length) {
    for (const lane of [...run.pendingPrismLanes]) {
      ...
      for (const p of players.values()) if (p.alive && distToSegment2(p.x, p.y, lane.x1, lane.y1, lane.x2, lane.y2) < (lane.w + PLAYER_SIZE / 2) ** 2) damagePlayer(run, p, lane.dmgP, (lane.x1 + lane.x2) / 2, (lane.y1 + lane.y2) / 2);
...
function beginTransition(run, players) {
  ...
  run.fx.push({ t: 'room_invoice', roomId: run.plan?.roomId || '', kills: st.kills ?? run.kills, gld: Math.round(st.gld || 0), exp: Math.round(st.exp || 0), hea: Math.round(st.hea || 0), dmg: Math.round(st.damageTaken || 0), noHit: (st.damageTaken || 0) <= 0 ? 1 : 0, fast: time > 0 && time < 42 ? 1 : 0, staticPaid: run.staticRainFromPending ? 1 : 0, nextStatic: nextStaticRainLevel(run), archetype: run.plan?.roomArchetype || 'standard' });
  run.phase = 'install';
  run.phaseT = 0;
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    // v2.0.58: VOID LASER is now a builder. Level I = one short segment.
...
export function buildSnapshot(run, players) {
  ...
  return {
    ...
    room: {
      ...
      staticRainStacks: run.staticRainStacks || 0, staticRainNext: nextStaticRainLevel(run), next: run.nextRoomPreview || null, sockets: run.roomSockets || [], betStakes: casinoStakeTable(run)
    },
