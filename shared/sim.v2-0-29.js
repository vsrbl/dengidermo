...
} from './data.v2-0-29.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-29.js';
...
function castActiveCore(run, players, p, opts = {}) {
  ...
  if (core === 'blood_ring') {
    ...
    run.fx.push({ t: 'dash', id: p.id, fx: Math.round(startX), fy: Math.round(startY), tx: Math.round(end.x), ty: Math.round(end.y) });
