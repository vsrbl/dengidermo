...
} from './data.v2-0-23.js';
import { generateRoom, spawnPoint, enemySpawnPoint, portalSpot, mulberry32, WALL_T } from './mapgen.v2-0-23.js';
...
function makeMutationChoice(mutId, p, replaceIdx = -1) {
  ...
  return addActiveChoiceMeta({
    ...
    preview: `${ACTIVE_CORES[a.core]?.label || 'Q'} + ${m.label}: ${m.desc}`,
...
function activeCoreDesc(p) {
  ...
  return `${c.label} ${roman(a.level || 1)} — ${c.desc}` + (muts.length ? ` Мутации: ${muts.map(m => `${m.label}: ${m.desc}`).join(' · ')}` : ' Мутаций пока нет.');
}
...
function stepActiveFields(run, players, dt) {
  ...
  for (const f of [...run.activeFields]) {
    ...
    if (f.fxT <= 0) { f.fxT = 0.18; run.fx.push({ t: 'active_field', kind: f.kind, x: Math.round(f.x), y: Math.round(f.y), r: f.r, tone: f.kind === 'blood_ring' ? 'red' : 'cyan' }); }
    if (f.kind === 'static') {
    ...
    if (f.tickT <= 0) {
      ...
      if (f.kind === 'blood_ring') {
        ...
        run.fx.push({ t: 'active_tick', kind: f.kind, x: Math.round(f.x), y: Math.round(f.y), r: f.r, tone: 'red' });
      }
    }
  }
...
export function buildSnapshot(run, players) {
  ...
  for (const p of players.values()) {
    ...
    ps.push([
      ...
      p.skin?.fill || '#f3f3f3', p.skin?.outline || '#00ff66', p.skin?.barrel || '#00ff66'
    ]);
  }
