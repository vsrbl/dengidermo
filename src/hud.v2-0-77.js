...
import { P, ENEMY_KINDS } from './state.v2-0-77.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-77.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, localText, esc as escHtml } from './i18n.v2-0-77.js';
...
function roomModHint(m, room = {}) {
  ...
  const hints = {
    ...
    echo_walls: localText('50% шанс echo для всех выстрелов, включая вражеские. Echo-пули отмечены ghost-square визуалом.', '50% echo chance for all shots, including enemy shots. Echo bullets use a ghost-square visual marker.'),
...
export class Hud {
  ...
  setTab(visible, state) {
    ...
    const modChip = (m, r = room) => `<span class="term" ${explainAttr(roomModLabel(m, r), roomModHint(m, r), m === 'static_rain' || m === 'static_wires' || m === 'prism_grid' ? 'cyan' : m === 'blood_tax' || m === 'moving_room' || m === 'hunter_contract' ? 'red' : m === 'echo_walls' || m === 'anchor_gravity' || m === 'casino_virus' ? 'purple' : m === 'greed' ? 'gold' : '')}>${esc(roomModLabel(m, r))}</span>`;
