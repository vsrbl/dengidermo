...
import { P, ENEMY_KINDS } from './state.v2-0-23.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-23.js';
...
export class Hud {
  ...
  openAbilityChest(choices = []) {
    ...
    choices.forEach((opt, i) => {
      const d = document.createElement('div');
      d.className = 'choice ability-choice' + (opt.disabled ? ' disabled' : '') + (opt.tone ? ' tone-' + opt.tone : '');
      const locked = opt.disabled ? `<span class="lock">${esc(opt.disabledReason || 'ЗАКРЫТО')}</span>` : '';
      const action = opt.actionLabel ? `<div class="choice-sub">${esc(opt.actionLabel)}</div>` : '';
      const preview = opt.preview ? `<div class="choice-preview">${esc(opt.preview)}</div>` : '';
      d.innerHTML = `<span class="key">[${i + 1}]</span>${esc(opt.label || opt.id)}${locked}${action}${preview}`;
      const title = opt.disabled ? `${opt.label || opt.id} / НЕДОСТУПНО` : (opt.label || opt.id);
      ...
      this.setExplain(d, title, body, opt.disabled ? 'red' : (opt.tone || 'cyan'));
      d.addEventListener('click', () => {
        if (opt.disabled) { this.playUiSound('denied'); return; }
        this.pickAbility(i);
      });
