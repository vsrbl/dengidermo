...
import { VERSION, PROTOCOL, GAME_SPEED } from '../shared/protocol.v2-0-21.js';
import { LocalRoom } from './local.v2-0-21.js';
...
export class Net {
  ...
  startSolo(name) {
    ...
    this.room.addHost(this.id, name);
  }
  ...
  connect(url, name) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws = ws;
      ws.onopen = () => { this._wsSend({ t: 'hello', name, proto: PROTOCOL }); };
      ws.onmessage = (ev) => {
  ...
  async _onSignal(m) {
    switch (m.t) {
      case 'host_ok': {
        ...
        this.room.addHost(this.id, this._hostName);
        break;
      }
