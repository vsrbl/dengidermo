...
import { Net, VERSION, GAME_SPEED } from './net.v2-0-50.js';
import { Input } from './input.v2-0-50.js';
import { GameState, P } from './state.v2-0-50.js';
import { Effects } from './effects.v2-0-50.js';
import { Renderer } from './render.v2-0-50.js';
import { Hud } from './hud.v2-0-50.js';
import { AudioBus } from './audio.v2-0-50.js';
import { SKIN_PRESETS, SKIN_RARITIES, DEFAULT_UNLOCKED_SKINS, ACTIVE_CORES, ACTIVE_MUTATIONS } from '../shared/data.v2-0-50.js';
import { setupLanguageButtons, onLangChange, t, skinNote, labelStatus, getLang, locRole, locAction } from './i18n.v2-0-50.js';
...
async function connect() {
  ...
  try {
    ...
    setStatus(t('statusOnline'), 'ok');
    return true;
  } catch (e) {
    setStatus(t('statusNetDown'), 'err');
    return false;
  }
}
...
$('btn-join').addEventListener('click', async () => {
  ...
  if (code.length !== 4) { setStatus(t('roomCode4'), 'err'); return; }
...
fetch((isLocal ? `http://${location.hostname}:10777` : cfg.BACKEND_HTTP_URL) + '/health')
  .then(r => r.json())
  .then(h => {
    setStatus(t('statusNetReady'), 'ok');
    if (h.version !== VERSION) setStatus(t('updateRequired'), 'err');
  })
  .catch(() => setStatus(t('statusNetSleep')));
...
net.on('error', (m) => { if (!inGame) setStatus(m.error === 'room not found' ? t('roomNotFound') : m.error === 'room full' ? t('roomFull') : m.error, 'err'); });
net.on('room_closed', () => location.reload());
net.on('_closed', () => {
  if (inGame) {
    $('menu').classList.remove('hidden');
    setStatus(t('lostConnection'), 'err');
    inGame = false;
  }
});
