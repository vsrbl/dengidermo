...
export class AudioBus {
  constructor() {
    ...
    this.cooldowns = {
      ...
      active_over: 0.24, active_void_laser: 0.65, active: 0.24, enemy: 0.18, bet_open: 0.18, casino_win: 0.24,
  ...
  play(type) {
    ...
    switch (type) {
      case 'shot_shg':
        ...
        break;
      case 'shot_sek':
        // higher-pitched SHG-like dry snap: same chunky family as SHG, but brighter and tighter.
        this.tone(330, 0.044, 'square', 0.052, 0.56);
        this.tone(760, 0.027, 'square', 0.034, 0.46, 0.005);
        this.noise(0.030, 0.038, 3100, 5.6);
        this.noise(0.012, 0.017, 6800, 13, 0.006);
        break;
      case 'shot':
  ...
  updateMusic(state, dt = 0.016) {
    ...
    const staticLike = mods.includes('static_rain') || mods.includes('prism_grid') || mods.includes('anchor_gravity') || mods.includes('static_wires');
    const choir = boss || /HERALD|DAMPER|WARDEN|ANCHOR|PRISM|HUNTED/.test(String(room?.director || '')) || mods.includes('shell_market') || mods.includes('echo_walls') || mods.includes('hunted_exit');
    ...
    this.setMusicLayer('drone', inGameMusicAmount(room) * (chill ? 0.035 : 0.055 + intensity * 0.025), 1.1);
    this.setMusicLayer('sub', combat ? (0.012 + intensity * 0.045 + lowHp * 0.035) : (chill ? 0.010 : 0.004), 0.75);
    this.setMusicLayer('pulse', combat ? (0.015 + intensity * 0.07) : (portalOpen ? 0.018 : 0.004), 0.45);
    this.setMusicLayer('hat', combat && intensity > 0.22 ? ((intensity - 0.18) * 0.055) : 0.0001, 0.35);
    this.setMusicLayer('casino', casino ? (chill ? 0.026 : 0.012 + intensity * 0.025) : 0.0001, 0.9);
    this.setMusicLayer('choir', choir || boss ? (0.012 + intensity * 0.045) : 0.0001, 1.0);
  }
  ...
  handleFx(f, info = {}) {
    ...
    switch (f.t) {
      ...
      case 'hunted_exit': this.play('portal'); break;
      case 'active_line':
