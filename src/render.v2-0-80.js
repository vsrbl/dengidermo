...
import { P, ENEMY_KINDS, ENEMY_LABELS } from './state.v2-0-80.js';
  ...
  draw(state, effects, view, myPos, mouse, now) {
    ...
    if (room?.movingWalls?.length) {
      ...
      for (const w of room.movingWalls) {
        ...
        ctx.fillStyle = 'rgba(220,55,70,0.28)';
        ...
        for (let x = w.x; x <= w.x + w.w; x += step) {
          ctx.fillRect(Math.round(x - 3), Math.round(w.y - 6), 6, 6);
          ctx.fillRect(Math.round(x - 3), Math.round(w.y + w.h), 6, 6);
        }
        for (let y = w.y; y <= w.y + w.h; y += step) {
          ctx.fillRect(Math.round(w.x - 6), Math.round(y - 3), 6, 6);
          ctx.fillRect(Math.round(w.x + w.w), Math.round(y - 3), 6, 6);
        }
        this.label('SHIFT', w.x + w.w / 2, w.y - 12, COL.red, 8);
      }
      ctx.restore();
    }
    ...
    for (const e of view.enemies) {
      const [eid, kindIdx, ex, ey, hp01, size, st, elite, dirX, dirY, shellPct = 0, shellLock = 0, linkId = '', shellType = '', exposed = 0, frozen = 0, burn = 0, poison = 0, chill = 0, stun = 0] = e;
