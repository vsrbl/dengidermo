...
const MOD_LABELS_RU = {
  ...
  shell_market: 'БИРЖА ЩИТОВ', echo_walls: 'ЭХО-ВЫСТРЕЛЫ', static_wires: 'СТАТИК-ПРОВОДА', hunted_exit: 'ОХОТА У ВЫХОДА', skin_cache: 'ОБЛИК-ТАЙНИК', trojan: 'ТРОЯН'
};
const TAG_RU = {
...
export class Hud {
  ...
  update(state, dt) {
    ...
    if (lcHud) {
      if (lvc && lvc.label) {
        if (String(lvc.mode || '') === 'guns') {
          const color = /^#[0-9a-fA-F]{6}$/.test(String(lvc.color || '')) ? String(lvc.color) : '#ffd34d';
          const type = String(lvc.type || 'dmg');
          const cd = Math.max(0, Number(type === 'spark' ? lvc.sparkCd : lvc.cd || 0));
          const stateClass = cd > 0 ? 'cooldown' : 'ready';
          const pTargets = Math.max(0, Number(lvc.primaryTargets || 0) | 0);
          const pSlots = Math.max(1, Number(lvc.primarySlots || 1) | 0);
          const sTargets = Math.max(0, Number(lvc.sparkTargets || 0) | 0);
          const sActive = Math.max(0, Number(lvc.sparkActive || 0) | 0);
          const sMax = Math.max(1, Number(lvc.sparkMax || 1) | 0);
          const sparkCd = Math.max(0, Number(lvc.sparkCd || 0));
          const hold = Math.max(0, Number(lvc.sparkHold || 0));
          lcHud.className = `lc-selected ${stateClass} type-${type}`;
          lcHud.style.setProperty('--lc-color', color);
          lcHud.style.setProperty('--lc-fill', `${stateClass === 'ready' ? 100 : 35}%`);
          lcHud.innerHTML = `<i class="lc-fill" aria-hidden="true"></i><b>${escHtml(localText('ЖИВОЕ КАЗИНО', 'LIVING CASINO'))}</b><span>LVC ${pTargets}/${pSlots} · SPK ${sActive}/${sMax}</span><em>${escHtml(localText(`ЛКМ цели · ПКМ искры · ${sparkCd.toFixed(1)}s`, `LMB marks · RMB sparks · ${sparkCd.toFixed(1)}s`))}</em>`;
          this.setExplain(lcHud, localText('ЖИВОЕ КАЗИНО', 'LIVING CASINO'), localText(`ЛКМ ставит цели базовой пушке. ПКМ ставит цели Искрам контроля. Удержание искры: ${hold.toFixed(1)} сек.`, `LMB marks base-gun targets. RMB marks Control Spark targets. Spark hold: ${hold.toFixed(1)} sec.`), 'gold');
        } else {
          ...
          this.setExplain(lcHud, localText('ЖИВОЕ КАЗИНО', 'LIVING CASINO'), localText('Пушки сами ведут цели. ЛКМ и ПКМ закрепляют приоритеты.', 'Guns aim on their own. LMB and RMB assign priorities.'), 'gold');
        }
      } else {
  ...
  handleFx(f, myId, state) {
    ...
    switch (f.t) {
      ...
      case 'casino_mob_defeated': this.banner(localText('СКРЫТЫЙ КАЗИНО-ВИРУС УДАЛЁН', 'HIDDEN CASINO VIRUS DELETED'), localText('ЖИВОЕ КАЗИНО ОТКРЫТО', 'LIVING CASINO UNLOCKED'), 'gold'); this.feed(`${localText('СКРЫТЫЙ КАЗИНО-ВИРУС', 'HIDDEN CASINO VIRUS')}: +${f.gold || 0} GLD`, 'g'); break;
      case 'hero_unlock': this.feed(`${localText('ЯДРО ОТКРЫТО', 'CORE UNLOCKED')}: ${locLabel(f.label || f.hero || '')}`, 'g'); break;
      case 'trojan_chest': this.banner('TROJAN', localText('сундук заражён', 'chest infected'), 'red'); break;
      case 'boss_down': this.banner(f.virus ? localText('ВИРУС УДАЛЁН', 'VIRUS DELETED') : t('bossDown'), f.virus ? t('loot') : t('loot'), 'green'); break;
      case 'chest_open': {
