...
import { P, ENEMY_KINDS } from './state.v2-0-66.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-66.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, localText, esc as escHtml } from './i18n.v2-0-66.js';
...
function objectiveExplain(obj = {}) {
  if (!obj?.id) return localText('Дополнительная цель комнаты. Выполни её для маленького бонуса и записи в Tape Log.', 'Extra room objective. Complete it for a small bonus and a Tape Log entry.');
  const map = {
    ...
    overstay_survive: localText('Портал откроется раньше. Останься и переживи хотя бы одну hunter wave.', 'Portal opens early. Stay and survive at least one hunter wave.'),
    wire_ghost: localText('Пройди комнату, не касаясь static wires.', 'Clear the room without touching static wires.'),
    grid_ghost: localText('Пройди комнату, не получая урон от prism lanes.', 'Clear the room without taking prism lane hits.'),
    shell_breaker: localText('Сломай 3 shell/armor защиты.', 'Break 3 shell/armor defenses.'),
    blood_paid: localText('Создай 3 blood tax зоны и выживи до конца комнаты.', 'Create 3 blood tax zones and survive the room.'),
  ...
  return `${map[obj.id] || localText('Дополнительная цель комнаты.', 'Extra room objective.')}\n${localText('Прогресс', 'Progress')}: ${obj.progress || '—'}\n${localText('Награда', 'Reward')}: ${obj.reward || '—'}`;
}
function objectiveChip(obj = {}, label = 'OBJ') {
  if (!obj?.label) return '';
  const prog = obj.progress ? ` · ${obj.progress}` : '';
  const reward = obj.reward ? ` · ${obj.reward}` : '';
  return `<span class="term" data-explain-title="${esc(label)}: ${esc(obj.label)}" data-explain="${esc(objectiveExplain(obj))}">${esc(label)}: ${esc(obj.label)}${esc(prog)}${esc(reward)}</span>`;
}
...
export class Hud {
  ...
  update(state, dt) {
    ...
    const currentObjective = objectiveChip(room.objective, 'OBJ');
    ...
    const nextObjective = nx?.objective ? ` · ${objectiveChip(nx.objective, 'OBJ')}` : '';
  ...
  handleFx(f, myId, state) {
    ...
    switch (f.t) {
      ...
      case 'room_invoice': {
        ...
        if (f.objective?.done) marks.push(`OBJ ${f.objective.label} +${f.objectiveBonusGld || 0}G +${f.objectiveBonusExp || 0}XP`);
    ...
    $('tab-run').innerHTML =
      `<div class="tab-dossier">` +
        `<div class="tab-card current"><h3><span class="term" ${explainAttr('CURRENT ROOM', roomIntelExplain(room, false), 'cyan')}>NOW</span> ${esc(ARCH_LABELS[room.archetype] || String(room.archetype || 'STANDARD').toUpperCase())}</h3>` +
          ...
          (room.objective ? `<p>${objectiveChip(room.objective, 'OBJ')}</p>` : '') +
        ...
        `<div class="tab-card next"><h3><span class="term" ${explainAttr('NEXT ROOM', next ? roomIntelExplain(next, true) : '—', 'cyan')}>NEXT</span> ${next ? esc(ARCH_LABELS[next.archetype] || String(next.archetype || 'STANDARD').toUpperCase()) : '—'}</h3>` +
          ...
          (next?.objective ? `<p>${objectiveChip(next.objective, 'OBJ')}</p>` : '') +
        ...
        `<div class="tab-card memory"><h3><span class="term" ${explainAttr('RUN MEMORY', localText('Сводка по текущему забегу: зачистки, streaks и накопленные награды.', 'Current run summary: clears, streaks, and accumulated rewards.'), 'green')}>RUN MEMORY</span></h3>` +
          ...
          `<p>OBJ ${esc(mem.objectivesDone || 0)}/${esc(mem.objectivesSeen || 0)} · +${esc(Math.round(mem.objectiveGld || 0))} GLD · +${esc(Math.round(mem.objectiveExp || 0))} EXP</p></div>` +
