...
const RU = {
  ui: {
    ...
    modsTitle: 'ПРАВИЛА КОМНАТЫ', modsBody: 'Особые правила комнаты: темнота, статик-шторм, золотая лихорадка, вирус казино, движущиеся зоны, сетка замедления, кровавая оплата, эхо-выстрелы и гравитация.',
    ...
    runLost: 'ЗАБЕГ ПРОВАЛЕН', restart: 'перезапуск…', noResponse: 'НЕТ ОТВЕТА — СТАВКА НЕ ПОДТВЕРЖДЕНА', betFailed: 'СТАВКА НЕ ПРИНЯТА', invalidStake: 'НЕВЕРНАЯ СТАВКА', jackpot: 'ДЖЕКПОТ', lose: 'проигрыш', staticDebt: 'СТАТИКА', skin: 'СКИН', nextRoomDebt: 'СТАТИК-ШТОРМ → СЛЕД. КОМНАТА',
...
const EN = {
  ui: {
    ...
    modsTitle: 'ROOM RULES', modsBody: 'Special room rules: darkness, static storm, gold pressure, casino virus, shifting zones, slow grid, blood payment, echo shots, anchor gravity, chill rooms, and more.',
    ...
    runLost: 'RUN FAILED', restart: 'restarting…', noResponse: 'NO ANSWER — BET NOT CONFIRMED', betFailed: 'BET NOT ACCEPTED', invalidStake: 'INVALID STAKE', jackpot: 'JACKPOT', lose: 'loss', staticDebt: 'STATIC', skin: 'SKIN', nextRoomDebt: 'STATIC STORM → NEXT ROOM',
...
const EN_UPGRADE = {
  ...
  abl_dash: 'Adds one dash charge.', abl_voidstep: 'Dash path becomes a void rift. Enemies along the full dash take heavy damage. Stacks increase width and damage.', voidstep: 'Dash path becomes a void rift. Enemies along the full dash take heavy damage. Stacks increase width and damage.', dashcut: 'Dash stuns enemies near its path. More stacks increase stun radius and duration.', dashclone: 'At the start of your dash, you leave a short damaging echo burst. More stacks increase its damage and radius.', abl_dashcut: 'Dash stuns enemies near its path. More stacks increase stun radius and duration.', abl_dashclone: 'At the start of your dash, you leave a short damaging echo burst. More stacks increase its damage and radius.', abl_speed: 'Increases movement speed.', abl_dashflow: 'Dash charges recover faster.',
...
const RU_WEAPON = {
  SHG: 'Ближний дробовик. Стреляет залпами и сам восстанавливается.', SEK: 'Медленный самонаводящийся квадратный снаряд.', RKT: 'Тяжёлая ракетница. Взрывается при попадании, стене или лимите пути.'
};
const EN_SKIN_NOTE = {
...
export function cleanPlayerText(text) {
  ...
  if (!langIsEn()) {
    s = s.replace(/damage-over-time/gi, 'периодический урон')
      ...
      .replace(/reroll/gi, 'реролл').replace(/PROC/gi, 'шанс взрыва')
...
const RU_LABEL = {
  ...
  'DASH: VOID RIFT': 'РЫВОК: РАЗРЕЗ ПУСТОТЫ', 'DASH STUN': 'РЫВОК: СТАН', 'DASH CLONE': 'РЫВОК: ЭХО-ВСПЛЕСК', 'DASH ECHO BURST': 'РЫВОК: ЭХО-ВСПЛЕСК',
