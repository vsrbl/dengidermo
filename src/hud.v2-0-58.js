...
import { P, ENEMY_KINDS } from './state.v2-0-58.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-58.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, localText, esc as escHtml } from './i18n.v2-0-58.js';
...
export class Hud {
  ...
  update(state, dt) {
    ...
    const modLabels = (room.mods || []).map(m => {
      const label = MOD_LABELS[m] || m;
      return m === 'static_rain' && room.staticRainStacks > 0 ? `${label} LVL ${room.staticRainStacks}` : label;
    });
    ...
    const rainBits = [];
    if ((room.mods || []).includes('static_rain') && curRain > 0) rainBits.push(`<span class="term" data-explain-title="STATIC RAIN" data-explain="${esc(localText('Уровень статического дождя в текущей комнате. Жёстким он становится только на высоких уровнях.', 'Static rain level in this room. It only becomes brutal at high levels.'))}">STATIC RAIN LVL ${curRain}</span>`);
    if (nextRain > 0) rainBits.push(`<span class="term" data-explain-title="NEXT STATIC RAIN" data-explain="${esc(localText('В следующей подходящей комнате будет Static Rain этого уровня. После такой комнаты долг сгорает.', 'The next eligible room will get Static Rain at this level. After that room, the debt is spent.'))}">NEXT STATIC RAIN LVL ${nextRain}</span>`);
    const rainHud = rainBits.length ? `<div class="static-rain-status">${rainBits.join(' · ')}</div>` : '';
    const nx = room.next || null;
    const nextMods = nx ? (nx.mods || []).map(m => MOD_LABELS[m] || m).slice(0, 3).join(' + ') : '';
    const nextRewards = nx?.rewardTags?.length ? ` · ${nx.rewardTags.join(' / ')}` : '';
    const nextStatic = nextRain > 0 ? ` · STATIC LVL ${nextRain}` : '';
    const prophecyHud = nx ? `<div class="room-prophecy"><span class="term" data-explain-title="NEXT ROOM" data-explain="${esc(localText('Предпросмотр следующей комнаты: размер, правила и наградный сигнал. Это помогает заранее строить маршрут и билд.', 'Preview of the next room: size, rules, and reward signal. It helps you plan before entering.'))}">NEXT</span>: ${esc(ARCH_LABELS[nx.archetype] || String(nx.archetype || '').toUpperCase())}${nextMods ? ' · ' + esc(nextMods) : ''}${esc(nextRewards)}${esc(nextStatic)}</div>` : '';
    let goalHtml = '';
    ...
    obj.innerHTML = `${rainHud}${prophecyHud}<div>${goalHtml}</div>`;
  ...
  handleFx(f, myId, state) {
    ...
    switch (f.t) {
      ...
      case 'room_invoice': {
        ...
        const line = `KILLS ${f.kills || 0} · GLD +${f.gld || 0} · EXP +${f.exp || 0} · DMG ${f.dmg || 0}${marks.length ? ' · ' + marks.join(' / ') : ''}`;
  ...
  setTab(visible, state) {
    ...
    $('tab-run').innerHTML =
      ...
      `<span class="term" data-explain-title="${esc(t('rules'))}" data-explain="${esc(t('modsBody'))}">${esc(t('rules'))}</span>: ${modLabels.map(esc).join(', ') || '—'}   SIZE: ${esc(ARCH_LABELS[room.archetype] || String(room.archetype || 'STANDARD').toUpperCase())}${room.staticRainNext > 0 ? '   NEXT STATIC RAIN LVL ' + esc(room.staticRainNext) : ''}${room.skinReward ? '   SKN: ' + esc(rarityText(room.skinReward)) : ''}`;
