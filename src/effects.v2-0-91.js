...
import { t, denyText } from './i18n.v2-0-91.js';
  ...
  handleFx(f, info) {
    ...
    switch (f.t) {
      ...
      case 'active_denied':
        ...
        break;
      case 'portal_open':
        // v2.0.91: portal open is a hard square signal burst, never a round ring.
      ...
      case 'moving_zone_hit':
        this.add({ kind: 'denybox', x: f.x, y: f.y, ttl: 0.18, color: '#ff3048' });
        this.add({ kind: 'rocketBlast', x: f.x, y: f.y, r: Math.max(46, Math.min(120, Math.max(f.w || 0, f.h || 0) * 0.55)), ttl: 0.18, color: '#ff3048' });
        this.kick(2);
        break;
      case 'room_invoice':
      ...
      case 'tether':
        ...
        break;
      case 'herald_cast':
        this.add({ kind: 'heraldCastLine', x: f.x, y: f.y, x2: f.x2, y2: f.y2, points: Array.isArray(f.points) ? f.points : null, pfill: (f.p || 0) / 100, ttl: 0.16, color: '#ff3048', start: f.start ? 1 : 0 });
  ...
  drawWorld(ctx) {
    for (const e of this.list) {
      ...
      if (e.kind === 'muzzle') {
        ...
        ctx.setLineDash([]);
        ctx.globalAlpha = fade * 0.88;
        ctx.strokeStyle = e.color || '#ff3048'; ctx.lineWidth = 3.4;
        ctx.beginPath(); ctx.moveTo(pts[0].x, pts[0].y);
