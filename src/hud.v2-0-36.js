...
import { P, ENEMY_KINDS } from './state.v2-0-36.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-36.js';
...
export class Hud {
  ...
  activeRoll(f) {
    ...
    const symbols = Array.isArray(f.symbols) && f.symbols.length ? f.symbols : ['Q', '?', '?'];
    el.className = tone;
    el.innerHTML = `<div class="roll-title">CASINO MUTATION ROLL</div>` +
      `<div class="roll-symbols">${symbols.slice(0,3).map(x => `<span>${esc(String(x).slice(0,4))}</span>`).join('')}</div>` +
      `<div class="roll-result">${esc(f.label || f.outcome || 'ROLL')}</div>`;
    el.classList.remove('hidden');
    clearTimeout(this.activeRollTimer);
    this.activeRollTimer = setTimeout(() => el.classList.add('hidden'), 1800);
  ...
  openAbilityChest(choices = []) {
    ...
    choices.forEach((opt, i) => {
      ...
      const body = `${opt.actionLabel ? opt.actionLabel + ': ' : ''}${opt.preview ? opt.preview + '\n\n' : ''}${opt.desc || 'Награда сундука способностей.'}${opt.disabled ? `\n\nНедоступно: ${opt.disabledReason || 'условие не выполнено'}.` : '\n\nМожно выбрать сейчас.'}`;
