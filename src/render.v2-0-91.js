...
import { P, ENEMY_KINDS, ENEMY_LABELS } from './state.v2-0-91.js';
  ...
  draw(state, effects, view, myPos, mouse, now) {
    ...
    for (const e of view.enemies) {
      ...
      if (shellPct > 0 || shellLock) {
        ...
        ctx.strokeStyle = shellRegen ? COL.green : (shellLock ? COL.red : COL.cyan);
        ctx.lineWidth = 1;
        ctx.strokeRect(ex - bw / 2, by, bw, 4);
        ctx.fillStyle = shellRegen ? COL.green : (shellType === 'linked' ? COL.purple : COL.cyan);
