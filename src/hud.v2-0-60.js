...
import { P, ENEMY_KINDS } from './state.v2-0-60.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-60.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, localText, esc as escHtml } from './i18n.v2-0-60.js';
...
function roomModHint(m, room = {}) {
  ...
  const hints = {
    blackout: localText('Темнота режет обзор. Держи центр и слушай угрозы.', 'Visibility is reduced. Hold center space and listen for threats.'),
    ...
    hunter_contract: localText('В комнате есть приоритетная цель. Убей её, пока она не сломала темп.', 'There is a priority target. Kill it before it breaks the tempo.'),
    casino_virus: localText('Больше casino-хаоса и BET-сигналов. Выгодно, но нестабильно.', 'More casino chaos and BET signals. Rewarding, but unstable.'),
    ...
    prism_grid: localText('Комната про линии. Следи за cyan lanes и не стой в геометрии.', 'A lane-reading room. Watch cyan lanes and avoid standing in the grid.'),
    ...
    shell_market: localText('Больше shell/armor давления. Ломай броню ради GLD shards.', 'More shell/armor pressure. Break armor for GLD shards.'),
    echo_walls: localText('Отражения и echo-угрозы сильнее. Меняй маршрут после выстрелов.', 'Reflections and echo threats are stronger. Change route after shots.'),
    ...
    static_wires: localText('Static-линии замедляют всё, что их пересекает. Используй их как контроль.', 'Static wires slow anything crossing them. Use them as control.'),
    hunted_exit: localText('Портал откроется раньше. Останешься фармить — пойдут hunter waves.', 'Portal opens early. Stay to farm and hunter waves start coming.')
  };
...
export class Hud {
  ...
  update(state, dt) {
    ...
    const currentHud = `<div class="room-current"><span class="term" data-explain-title="CURRENT ROOM" data-explain="${esc(localText('Размер, правила и короткое объяснение текущей комнаты.', 'Size, rules, and short explanation for the current room.'))}">NOW</span>: ${esc(ARCH_LABELS[room.archetype] || String(room.archetype || 'STANDARD').toUpperCase())}${modLabels.length ? ' · ' + esc(modLabels.slice(0, 4).join(' + ')) : ' · CLEAN'}</div>` +
      `<div class="room-rule"><span class="term" data-explain-title="ROOM RULE" data-explain="${esc(currentRules)}">RULE</span>: ${esc(currentRules)}</div>`;
    const nextModIds = nx ? (nx.mods || []).slice(0, 4) : [];
    const nextModLabels = nx ? nextModIds.map(m => roomModLabel(m, null, m === 'static_rain' && appliesNextRoom ? nextRain : 0)) : [];
    if (appliesNextRoom && !nextModIds.includes('static_rain')) nextModLabels.push(`STATIC RAIN LVL ${nextRain}`);
    const nextMods = nextModLabels.slice(0, 4).join(' + ');
    const nextRewards = nx?.rewardTags?.length ? ` · ${nx.rewardTags.join(' / ')}` : '';
    const nextStatic = nextRain > 0 && !appliesNextRoom ? ` · STATIC BANKED LVL ${nextRain}` : '';
    const nextExplain = nx ? roomRuleSummary({ ...room, staticRainStacks: appliesNextRoom ? nextRain : 0, staticRainMode: appliesNextRoom ? 'paid' : '' }, nextModIds) : '';
    const prophecyHud = nx ? `<div class="room-prophecy"><span class="term" data-explain-title="NEXT ROOM" data-explain="${esc(nextExplain || localText('Предпросмотр следующей комнаты: размер, правила и наградный сигнал.', 'Preview of the next room: size, rules, and reward signal.'))}">NEXT</span>: ${esc(ARCH_LABELS[nx.archetype] || String(nx.archetype || '').toUpperCase())}${nextMods ? ' · ' + esc(nextMods) : ' · CLEAN'}${esc(nextRewards)}${esc(nextStatic)}</div>` : '';
    let goalHtml = '';
  ...
  handleFx(f, myId, state) {
    ...
    switch (f.t) {
      case 'room': {
        ...
        this.banner(f.cat === 'boss' ? t('bossFloor') : `${f.roomId}`, `${t('loop')} ${f.loop} · ${t('depth')} ${f.depth}${arch}${mods ? ' · ' + mods : ''}${skn}`,
  ...
  setTab(visible, state) {
    ...
    $('tab-run').innerHTML =
      ...
      `<span class="term" data-explain-title="${esc(t('rules'))}" data-explain="${esc(t('modsBody'))}">${esc(t('rules'))}</span>: ${modLabels.map(esc).join(', ') || '—'}   SIZE: ${esc(ARCH_LABELS[room.archetype] || String(room.archetype || 'STANDARD').toUpperCase())}${tabStatic}${room.skinReward ? '   SKN: ' + esc(rarityText(room.skinReward)) : ''}`;
