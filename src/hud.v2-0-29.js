...
import { P, ENEMY_KINDS } from './state.v2-0-29.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-29.js';
...
export class Hud {
  ...
  openAbilityChest(choices = []) {
    ...
    choices.forEach((opt, i) => {
      ...
      const locked = opt.disabled ? `<span class="lock">${esc(opt.disabledReason || 'ЗАКРЫТО')}</span>` : '';
      const role = opt.role ? `<span class="abl-role">${esc(opt.role)}</span>` : '';
      const preview = opt.preview ? `<div class="abl-section"><div class="abl-section-title">ДЕЙСТВИЕ</div><div>${esc(opt.preview)}</div></div>` : '';
      const desc = `<div class="abl-section"><div class="abl-section-title">СИГНАЛ</div><div>${esc(opt.desc || 'Награда сундука способностей.')}</div></div>`;
      const action = opt.actionLabel ? `<div class="abl-action">${esc(opt.actionLabel)}</div>` : '';
      d.innerHTML = `
        <div class="abl-card-top">
          <span class="key abl-key">[${i + 1}]</span>
          <div class="abl-title-wrap">
            <div class="abl-name">${esc(opt.label || opt.id)}</div>
            ${action}
          </div>
          <div class="abl-tags"><span class="rarity-tag">${esc(String(group).toUpperCase())}</span>${role}${locked}</div>
        </div>
        <div class="abl-card-body">${preview}${desc}</div>`;
      const title = opt.disabled ? `${opt.label || opt.id} / НЕДОСТУПНО` : `${opt.label || opt.id} / ${String(group).toUpperCase()}`;
