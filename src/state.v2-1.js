...
export class GameState {
  ...
  setWalls(walls, world) {
    ...
    this.visualWalls = [...this.baseWalls];
  ...
  addSnapshot(s) {
    ...
    this.latest = s;
    this.room = s.room;
    const tempWalls = (s.room?.tempWalls || []).filter(Boolean);
    this.visualWalls = [...this.baseWalls, ...tempWalls];
    this.walls = [...this.baseWalls, ...tempWalls.filter(w => !w.moving && !w.settling)];
    const me = this.me();
    if (me) {
  ...
  nearestInteractable(px, py) {
    ...
    for (const o of this.latest.objects) {
      ...
      if (d < bd) { bd = d; best = { kind: o[1], label: o[2], cost: o[6], currency: o[7] || 'GLD', value: o[8] || '', valueTier: o[9] || 0, id: o[0], contractFree: !!o[12] }; }
    }
    return best;
  }
}
