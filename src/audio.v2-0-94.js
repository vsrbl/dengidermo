...
export class AudioBus {
  constructor() {
    ...
    this.cooldowns = {
      ...
      contract: 0.35, debt: 0.28, shield: 0.12, echo_shot: 0.10, ui_click: 0.045
    };
    this.music = null;
    this.musicPulseT = 0;
    this.musicStepT = 0;
    this.prio = {
      ...
      shot_shg: 3, shot_sek: 3, shot: 2, impact: 2, install: 5, contract: 7, debt: 7, shield: 4, echo_shot: 5, ui_click: 3
    };
  ...
  play(type) {
    ...
    switch (type) {
      case 'shot_shg':
        ...
        break;
      case 'shot_sek':
        // v2.0.94: SEEKER is 50% quieter and pitched higher; still dry/terminal, no bubble chirp.
  ...
  musicRootFor(context = {}) {
    // v2.0.94: keep every music layer locked to the same bass center.
  ...
  playDirgePhrase(context = {}) {
    ...
    const scale = staticLike ? [0, -2, -5, -7, -10, -12, -15] : casino ? [0, -3, -5, -7, -10, -12, -17] : [0, -3, -5, -7, -8, -12, -15];
    const motifs = [
      [0, 1, 3, 1, 4],
      [0, 2, 1, 3, 5],
      [0, 3, 2, 4, 1, 5],
    ...
    const step = menu ? 0.86 : boss ? 0.62 : chill ? 1.02 : Math.max(0.52, 0.82 - intensity * 0.22);
    const leadVol = menu ? 0.006 : chill ? 0.006 : 0.008 + intensity * 0.010 + damage * 0.006;
    const bodyVol = menu ? 0.012 : chill ? 0.014 : 0.014 + intensity * 0.012;
    const filter = staticLike ? 360 : casino ? 520 : 430;

    // Background bass stays mostly behind the drone. It only comes forward under real pressure.
    this.musicNote(root * 0.5, step * (boss ? 5.2 : 4.8), 'sawtooth', bodyVol * 0.72, 190 + intensity * 70, 0, 0.998, -4);
    if (intensity > 0.74 || boss || damage > 0.62) this.musicNote(root * 0.25, step * 3.5, 'sine', 0.006 + intensity * 0.008, 110, step * 0.5, 1, 0);
    ...
    motif.forEach((m, i) => {
      ...
      const register = menu || chill ? 12 : (intensity > 0.7 ? 24 : 12);
      const f = semitone(m + register);
      const detune = (i % 2 ? -5 : 4) + (staticLike ? -3 : 0) + (casino ? 3 : 0);
      this.musicNote(f, dur, i % 4 === 0 ? 'triangle' : 'square', leadVol * (i === 0 ? 0.75 : 1), filter + i * 70 + intensity * 140, delay, 1, detune);
    ...
    if (crowd > 0.58 || boss) {
      ...
      this.musicNote(semitone(m), step * 2.2, 'sawtooth', 0.008 + intensity * 0.008, 260, step * 1.4, 1, -6);
    }
  }
  ...
  updateMusic(state, dt = 0.016) {
    ...
    this.setMusicLayer('drone', inGame * (menu ? 0.032 : (chill ? 0.032 : 0.048 + intensity * 0.018)), 1.2);
    this.setMusicLayer('sub', menu ? 0.0025 : (combat ? (0.0035 + Math.max(0, intensity - 0.58) * 0.040 + lowHp * 0.016 + (boss ? 0.012 : 0)) : (chill ? 0.004 : 0.0015)), 1.15);
    this.setMusicLayer('pulse', menu ? 0.0025 : (combat ? (0.004 + Math.max(0, intensity - 0.28) * 0.040) : (portalOpen ? 0.008 : 0.0015)), 0.75);
  ...
  handleFx(f, info = {}) {
    ...
    switch (f.t) {
      ...
      case 'echo_shot': this.play('echo_shot'); break;
      case 'split': case 'summon': case 'pulse_wave': case 'prism': case 'leech_link': this.play('enemy'); break;
