    ...
    for (const e of this.list) {
      ...
      if (e.kind === 'muzzle') {
        ...
        for (let i = 0; i < steps; i++) {
          const q = (i + 1) / steps;
          const s = e.r * (0.18 + p * 0.72 + q * 0.12);
          ctx.globalAlpha = fade * (0.46 - i * 0.09);
          ctx.strokeStyle = e.color; ctx.lineWidth = Math.max(1, 3 - p * 2 - i * 0.35);
          ctx.strokeRect(Math.round(e.x - s / 2), Math.round(e.y - s / 2), Math.round(s), Math.round(s));
        }
        ctx.setLineDash([]);
        ctx.globalAlpha = fade * 0.25; ctx.fillStyle = e.color;
        const core = 18 + p * 14;
