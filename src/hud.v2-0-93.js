...
import { P, ENEMY_KINDS } from './state.v2-0-93.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-93.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, priceText, localText, denyText, esc as escHtml } from './i18n.v2-0-93.js';
...
function roomModHint(m, room = {}) {
  ...
  const hints = {
    ...
    static_rain: mode === 'paid'
      ? localText(`Статик-шторм LVL ${lvl}: это сохранённый заряд из прошлой комнаты. Он тратится здесь: с неба падают квадратные удары, после этой комнаты этот уровень не переносится дальше.`, `Static Storm LVL ${lvl}: this is a banked charge from a previous room. It is spent here: square strikes fall during the room, and this level will not carry forward afterward.`)
      : localText(`Статик-шторм LVL ${Math.max(1, lvl)}: во время комнаты падают квадратные удары. Если это природный шторм комнаты, реальные удары могут зарядить следующий подходящий бой.`, `Static Storm LVL ${Math.max(1, lvl)}: square strikes fall during this room. If this is the room's natural storm, real strikes can charge the next eligible combat room.`),
    greed: localText('Золотая лихорадка усиливает золотые награды и GLD-напряжение комнаты. BET остаётся обычным рискованным казино, а не гарантированным золотом.', 'Gold Fever boosts gold pressure and GLD rewards. BET remains a risky casino spin, not guaranteed gold.'),
    ...
    casino_virus: localText('Вирус Казино крутит 3 слота. Выпавший эффект применяется только после окончания анимации. Если врагов нет, следующий слот ускоряется; портал откроется после 3 слотов и добивания всех врагов.', 'Casino Virus spins 3 slots. The result applies only after the animation ends. If no enemies are alive, the next slot speeds up; portal opens after 3 slots and full cleanup.'),
    moving_room: localText('Красные зоны полые: они не являются стенами, но замедляют и пульсом бьют игроков и врагов внутри.', 'Red zones are hollow: they are not walls, but they slow and pulse-damage players and enemies inside.'),
...
function roomIntelExplain(room = {}, isNext = false) {
  ...
  const mods = (room.mods || []).map(m => roomModLabel(m, room)).join(' / ') || localText('нет модификаторов', 'no modifiers');
  return `${base}
${dangerLabel(room)}
${localText('МОДИФИКАТОРЫ', 'MODIFIERS')}: ${mods}
${localText('УГРОЗЫ', 'THREAT')}: ${threats}
${localText('НАГРАДА', 'REWARD')}: ${rewards}
${localText('Подробности', 'Details')}: ${localText('наведи на подчёркнутое название модификатора.', 'hover an underlined modifier name.')}`;
}
...
function objectiveExplain(obj = {}) {
  ...
  const map = {
    ...
    static_clean: localText('Пройди комнату со статик-штормом и получи мало урона. Шторм — это серия квадратных ударов по арене.', 'Clear the Static Storm room while taking little damage. The storm is a series of square strikes across the arena.'),
  ...
  update(state, dt) {
    ...
    if (pendingNextRain > 0) {
      ...
      const body = nextStaticEligible(nx)
        ? localText('Это сохранённый заряд шторма: следующая подходящая комната получит квадратные удары этого уровня, затем заряд будет потрачен.', 'This is a banked storm charge: the next eligible room gets square strikes at this level, then the charge is spent.')
