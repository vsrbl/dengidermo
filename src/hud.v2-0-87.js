...
import { P, ENEMY_KINDS } from './state.v2-0-87.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-87.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, priceText, localText, denyText, esc as escHtml } from './i18n.v2-0-87.js';
...
function roomModLabel(m, room = null, forcedStaticLevel = 0) {
  const label = m === 'skin_cache' ? localText('СКРЫТЫЙ СКИН', 'HIDDEN SKIN') : (MOD_LABELS[m] || String(m || '').toUpperCase());
  if (m === 'static_rain') {
...
function contractCardHtml(obj = {}) {
  if (!obj?.label) return '';
  const status = objectiveStatusText(obj);
  const prog = obj.progress ? String(obj.progress) : '—';
  const fail = obj.failReason ? ` · ${obj.failReason}` : '';
  const reward = obj.status === 'failed'
    ? localText('СЕРИЯ СБРОШЕНА', 'CHAIN RESET')
    : localText('НАГРАДА ПОСЛЕ ПЕРЕХОДА', 'REWARD AFTER TRANSITION');
  const title = `${obj.label} · ${status}${fail}`;
...
export class Hud {
  ...
  update(state, dt) {
    ...
    if (curRain > 0) {
      ...
      const label = (room.mods || []).includes('static_rain') ? `NOW STATIC RAIN LVL ${curRain}${paid ? ' · PAID' : ''}` : `DEBT ENGINE RAIN LVL ${curRain}`;
      ...
      if (debtLayer > 0) {
        ...
        rainBits.push(`<span class="term" data-tone="cyan" data-explain-title="DEBT ENGINE" data-explain="${esc(body)}">DEBT ENGINE: +STATIC RAIN LVL ${debtLayer} THIS ROOM</span>`);
      }
    }
    ...
    if (pendingNextRain > 0) {
      const txt = nextStaticEligible(nx) ? `NEXT ROOM STATIC RAIN LVL ${pendingNextRain}` : `STATIC BANKED LVL ${pendingNextRain}`;
    ...
    if (debtEngineNextRain > 0) {
      ...
      rainBits.push(`<span class="term" data-tone="cyan" data-explain-title="DEBT ENGINE NEXT" data-explain="${esc(body)}">DEBT ENGINE: +STATIC RAIN LVL ${debtEngineNextRain} NEXT ROOM</span>`);
    }
    const rainHud = rainBits.length ? `<div class="static-rain-status">${rainBits.join(' · ')}</div>` : '';
    const virusHud = room.casinoVirus ? `<div class="static-rain-status"><span class="term" data-explain-title="CASINO VIRUS" data-explain="${esc(roomModHint('casino_virus', room))}">VIRUS REELS · ${Math.max(0, room.casinoVirus.spinsLeft || 0)} SPINS LEFT · NEXT ${Math.max(0, Math.ceil(room.casinoVirus.nextSpin || 0))}s${room.casinoVirus.activeRainStacks ? ' · STATIC RAIN LVL ' + room.casinoVirus.activeRainStacks : ''}</span></div>` : '';
    ...
    if ((nx?.staticRainLevel || 0) > 0 && !nextModIds.includes('static_rain')) nextModLabels.push(`STATIC RAIN LVL ${nx.staticRainLevel}`);
    ...
    const nextStatic = (nx?.staticBanked || 0) > 0 ? ` · STATIC BANKED LVL ${nx.staticBanked}` : (debtEngineNextRain > 0 ? ` · DEBT ENGINE +STATIC LVL ${debtEngineNextRain}` : '');
  ...
  handleFx(f, myId, state) {
    ...
    switch (f.t) {
      ...
      case 'room_invoice': {
        ...
        if (f.objective) {
          if (f.objective.done) { const fs = (f.contractFavorsEarned || []).map(x => this.favorUiLabel(x)).join(' + '); marks.push(`CONTRACT DONE ${f.objective.label}${fs ? ' · FAVOR ' + fs : ''}`); }
          else marks.push(`CONTRACT FAILED ${f.objective.label}${f.objective.failReason ? ' / ' + f.objective.failReason : ''}`);
        }
        if (f.contractChain >= 2) marks.push(`CONTRACT CHAIN x${f.contractChain}`);
      ...
      case 'casino_virus_spin': this.virusRoll(f); this.feed(`CASINO VIRUS: ${f.label || 'EVENT'} · ${f.spinsLeft || 0} SPINS LEFT`, 'p'); break;
      case 'director_wave':
  ...
  virusRoll(f) {
    const el = $('virus-roll');
    if (!el) {
      this.banner('CASINO VIRUS', `${f.label || 'VIRUS EVENT'} · ${f.spinsLeft || 0} SPINS LEFT`, 'purple');
      return;
    }
    ...
    el.innerHTML = `<div class="roll-title">CASINO VIRUS REELS</div>` +
      ...
      `<div class="roll-left">${Math.max(0, f.spinsLeft || 0)} SPINS LEFT</div>`;
      ...
      spans.forEach((sp, i) => {
        const timer = setTimeout(() => {
          ...
          if (i === 2) {
            ...
            if (left) left.textContent = `${Math.max(0, f.spinsLeft || 0)} SPINS LEFT${f.rainStacks ? ' · RAIN LVL ' + f.rainStacks : ''}`;
            this.banner('CASINO VIRUS', `${f.label || 'VIRUS EVENT'} · ${Math.max(0, f.spinsLeft || 0)} SPINS LEFT`, bad ? 'red' : 'purple');
  ...
  setTab(visible, state) {
    ...
    $('tab-run').innerHTML =
      `<div class="tab-dossier">` +
        ...
        `<div class="tab-card memory"><h3><span class="term" ${explainAttr('RUN MEMORY', localText('Сводка по текущему забегу: комнаты, серии и награды.', 'Current run summary: clears, streaks, and accumulated rewards.'), 'green')}>RUN MEMORY</span></h3>` +
          ...
          `<p>CONTRACTS ${esc(mem.objectivesDone || 0)}/${esc(mem.objectivesSeen || 0)} · FAVORS ${esc(mem.favorsEarned || 0)}</p></div>` +
        `<div class="tab-card tapes"><h3><span class="term" ${explainAttr('TAPE LOG', localText('Последние значимые записи текущего забега. Показаны здесь, а не в боевом HUD.', 'Recent meaningful run records. Shown here instead of the combat HUD.'), 'purple')}>TAPE LOG</span></h3>${tapeRows}</div>` +
      `</div>`;
