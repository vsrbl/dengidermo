...
import { Net, VERSION, BUILD_ID, PROTOCOL, GAME_SPEED } from './net.v2-0-94.js';
import { Input } from './input.v2-0-94.js';
import { GameState, P } from './state.v2-0-94.js';
import { Effects } from './effects.v2-0-94.js';
import { Renderer } from './render.v2-0-94.js';
import { Hud } from './hud.v2-0-94.js';
import { AudioBus } from './audio.v2-0-94.js';
import { SKIN_PRESETS, SKIN_RARITIES, DEFAULT_UNLOCKED_SKINS, ACTIVE_CORES, ACTIVE_MUTATIONS, ROOM_MODS, SPECIAL_ROOMS, ROOM_SEQUENCE } from '../shared/data.v2-0-94.js';
import { setupLanguageButtons, onLangChange, t, skinNote, labelStatus, getLang, locRole, locAction } from './i18n.v2-0-94.js';
...
$('music-volume')?.addEventListener('input', e => { audio.setMusicVolume?.(Number(e.currentTarget.value) / 100); paintRange(e.currentTarget); uiClick('ui_click'); });
$('sfx-volume')?.addEventListener('input', e => { audio.setSfxVolume?.(Number(e.currentTarget.value) / 100); paintRange(e.currentTarget); uiClick('ui_click'); });
syncAudioSliders();
onLangChange(() => {
  updateSkinPreview();
