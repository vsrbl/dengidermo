  ...
  handleFx(f, info) {
    ...
    switch (f.t) {
      ...
      case 'active_casino_roll': {
        ...
        this.add({ kind: 'casinoRollBurst', x: f.x, y: f.y, r: f.outcome === 'TEN' ? 160 : 110, ttl: 0.42, color: col, symbols: f.symbols || [] });
        if (mine) { this.kick(f.outcome === 'TEN' ? 8 : f.outcome === 'HIT' || f.outcome === 'DEBT' ? 6 : 4); this.zoomKick = Math.max(this.zoomKick, f.outcome === 'TEN' ? 0.44 : 0.22); }
        break;
      }
      case 'active_line':
