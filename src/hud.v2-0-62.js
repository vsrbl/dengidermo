...
import { P, ENEMY_KINDS } from './state.v2-0-62.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-62.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, localText, esc as escHtml } from './i18n.v2-0-62.js';
...
function roomIntelExplain(room = {}, isNext = false) {
  const base = isNext
    ? localText('Что ждёт в следующей комнате: размер, риск, угрозы, наградный сигнал и короткий совет.', 'What waits in the next room: size, risk, threats, reward signal, and a short tip.')
    : localText('Что происходит в текущей комнате: размер, риск, угрозы, наградный сигнал и короткий совет.', 'What is happening in the current room: size, risk, threats, reward signal, and a short tip.');
  ...
  const tip = room.tip || roomRuleSummary(room, room.mods || []);
  return `${base}
${dangerLabel(room)}
THREAT: ${threats}
REWARD: ${rewards}
...
export class Hud {
  ...
  update(state, dt) {
    ...
    const currentRules = room.tip || roomRuleSummary(room, room.mods || []);
    ...
    const currentHud = `<div class="room-current"><span class="term" data-explain-title="CURRENT ROOM" data-explain="${esc(roomIntelExplain(room, false))}">NOW</span>: ${esc(ARCH_LABELS[room.archetype] || String(room.archetype || 'STANDARD').toUpperCase())}${modLabels.length ? ' · ' + esc(modLabels.slice(0, 4).join(' + ')) : ' · CLEAN'}</div>` +
      ...
      `<div class="room-rule"><span class="term" data-explain-title="ROOM RULE" data-explain="${esc(currentRules)}">TIP</span>: ${esc(currentRules)}</div>`;
  ...
  handleFx(f, myId, state) {
    ...
    switch (f.t) {
      case 'room': {
        ...
        if (f.tip) this.feed(`ROOM TIP: ${f.tip}`, (f.danger | 0) >= 4 ? 'r' : 'c');
  ...
  setTab(visible, state) {
    ...
    $('tab-run').innerHTML =
      ...
      `<span class="term" data-explain-title="CURRENT INTEL" data-explain="${esc(roomIntelExplain(room, false))}">NOW</span> ${esc(ARCH_LABELS[room.archetype] || String(room.archetype || 'STANDARD').toUpperCase())}   ${esc(dangerLabel(room))}   THREAT: ${esc(tagJoin(room.threatTags, 'NORMAL'))}   REWARD: ${esc(tagJoin(room.rewardTags, 'NORMAL'))}<br>` +
      ...
      `<span class="term" data-explain-title="${esc(t('rules'))}" data-explain="${esc(t('modsBody'))}">${esc(t('rules'))}</span>: ${modLabels.map(esc).join(', ') || '—'}${tabStatic}${room.skinReward ? '   SKN: ' + esc(rarityText(room.skinReward)) : ''}` +
      (next ? `<br><span class="term" data-explain-title="NEXT INTEL" data-explain="${esc(roomIntelExplain(next, true))}">NEXT</span> ${esc(ARCH_LABELS[next.archetype] || String(next.archetype || 'STANDARD').toUpperCase())}   ${esc(dangerLabel(next))}   THREAT: ${esc(tagJoin(next.threatTags, 'NORMAL'))}   REWARD: ${esc(tagJoin(next.rewardTags, 'NORMAL'))}   TIP: ${esc(next.tip || '')}` : '');
