...
export function updateOrbiterEnemy(ctx) {
  ...
  rt.radius = Math.max(cfg.minRadius || 34, (rt.radius || cfg.radius || 176) - (cfg.shrinkPerSecond || 28) * dt);
  ...
  enemy.orbiterPressureRadius = cfg.pressureRadius || 126;
  const speed = enemySpeed(state, enemy, data, updateCtx);
  const suction = cfg.suction || 4.2;
  const desiredX = tangent.x * speed + fromTarget.x * (-radialError * suction);
  const desiredY = tangent.y * speed + fromTarget.y * (-radialError * suction);
