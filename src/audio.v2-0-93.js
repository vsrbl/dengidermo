...
export class AudioBus {
  constructor() {
    ...
    this.sfxGain = null;
    this.musicGain = null;
    this.musicVolume = this.readVolume('nnc_music_volume', 0.70);
  ...
  setMusicVolume(value) {
    ...
    if (this.musicGain && this.ctx) this.musicGain.gain.setTargetAtTime(0.95 * this.musicVolume, this.ctx.currentTime, 0.05);
  }
  ...
  unlock() {
    ...
    if (!this.ctx) {
      ...
      this.musicGain.gain.value = 0.95 * this.musicVolume;
  ...
  play(type) {
    ...
    switch (type) {
      case 'shot_shg':
        ...
        break;
      case 'shot_sek':
        // v2.0.93: SEEKER is 50% quieter and pitched higher; still dry/terminal, no bubble chirp.
  ...
  playDirgePhrase(context = {}) {
    ...
    // Bleak, slow minor/phrygian fragments. It is music, but it should feel broken, heavy and terminal-dirty.
    const root = boss ? 43.65 : (chill ? 49.00 : 55.00);
    const motifs = [
      [0, 3, 1, -2, 0],       // minor fall with a flat-second wound
      [0, -5, -2, 1, -7],     // descending machine hymn
      [0, 6, 3, 1, -2],       // tritone pressure, not heroic
      [0, 1, -2, -5, -7],     // static crawl
    ];
    const motif = motifs[this.music.motifIndex % motifs.length];
    this.music.motifIndex = (this.music.motifIndex + 1) % 4096;
    const semitone = n => root * Math.pow(2, n / 12);
    const step = boss ? 0.72 : (chill ? 0.92 : 0.64);
    const leadVol = chill ? 0.010 : 0.014 + intensity * 0.012;
    const lowVol = chill ? 0.020 : 0.028 + intensity * 0.014;
    const filter = staticLike ? 420 : (casino ? 560 : 500);

    // Low funeral body. Detuned pair makes it feel worn, not clean.
    this.musicNote(root * 0.5, step * 4.4, 'sawtooth', lowVol, 260, 0, 0.985, -9);
    this.musicNote(root * 0.5 * 1.005, step * 4.4, 'sawtooth', lowVol * 0.45, 210, 0.02, 1.01, 11);

    motif.forEach((m, i) => {
      const delay = i * step;
      const dur = step * (i === motif.length - 1 ? 2.3 : 1.15);
      const f = semitone(m + 24);
      const detune = (i % 2 ? -13 : 9) + (casino ? 7 : 0);
      this.musicNote(f, dur, i % 3 === 0 ? 'triangle' : 'square', leadVol * (i === 0 ? 0.85 : 1), filter + i * 110, delay, i === motif.length - 1 ? 0.985 : 1, detune);
      if (staticLike && i === 2) this.musicDust(0.06, 0.008 + intensity * 0.006, 5200, delay + 0.03);
      if (casino && i >= 2) this.musicNote(f * 2.01, 0.10, 'square', 0.004 + intensity * 0.003, 1550, delay + 0.025, 0.82, -18);
    });

    if (boss || intensity > 0.68) {
      this.musicNote(root * Math.pow(2, 6 / 12), step * 2.8, 'sawtooth', 0.012 + intensity * 0.010, 320, step * 1.5, 0.96, -16);
      this.musicDust(0.12, 0.012, 900, step * 1.52);
    }
  }
  ...
  ensureMusic() {
    ...
    this.music.layers.drone = this.makeToneLayer(54, 'sawtooth', 420);
    this.music.layers.sub = this.makeToneLayer(41, 'sine', 180);
    this.music.layers.pulse = this.makeToneLayer(82, 'square', 260);
  ...
  updateMusic(state, dt = 0.016) {
    ...
    const intensity = menu ? 0.22 : Math.max(0, Math.min(1, alivePressure * 0.40 + bulletPressure * 0.16 + lowHp * 0.22 + danger * 0.22 + (this.damageEnergy || 0) * 0.24 + (boss ? 0.30 : 0) + (staticLike ? 0.10 : 0)));
    ...
    if (this.musicStepT <= 0) {
      ...
      if (pulse) pulse.o.frequency.setTargetAtTime(menu ? 46 : (boss ? 74 : (combat ? 76 + intensity * 58 : 55)), now, 0.4);
      if (casinoL) casinoL.o.frequency.setTargetAtTime(menu ? 196 : (casino ? (chill ? 247 : 310 + intensity * 105) : 210), now, 0.6);
      if (dirgePad) {
        const base = menu ? 49.00 : (boss ? 65.41 : (chill ? 73.42 : 98.00));
        const wound = staticLike ? Math.pow(2, 1 / 12) : (casino ? Math.pow(2, 3 / 12) : 1);
    ...
    if ((room || menu) && this.music.phraseT <= 0) {
      this.playDirgePhrase({ boss, chill, casino, staticLike, intensity: menu ? 0.18 : intensity, menu });
      // Slow enough to live above the drone without becoming a cheerful loop, but denser under danger.
      this.music.phraseT = menu ? 8.4 : ((boss ? 4.4 : chill ? 7.0 : 5.7) - intensity * 1.85 + (portalOpen ? 0.8 : 0));
    }

    this.setMusicLayer('drone', inGame * (menu ? 0.030 : (chill ? 0.034 : 0.052 + intensity * 0.028)), 1.1);
    this.setMusicLayer('sub', menu ? 0.006 : (combat ? (0.012 + intensity * 0.052 + lowHp * 0.032) : (chill ? 0.010 : 0.004)), 0.75);
    this.setMusicLayer('pulse', menu ? 0.004 : (combat ? (0.010 + intensity * 0.070) : (portalOpen ? 0.014 : 0.003)), 0.45);
    this.setMusicLayer('hat', combat && intensity > 0.18 ? ((intensity - 0.14) * 0.060) : 0.0001, 0.35);
    this.setMusicLayer('casino', casino ? (chill ? 0.020 : 0.010 + intensity * 0.020) : 0.0001, 0.9);
    this.setMusicLayer('choir', menu ? 0.006 : (choir || boss ? (0.010 + intensity * 0.050) : 0.0001), 1.0);
    this.setMusicLayer('dirgePad', inGame * (menu ? 0.012 : (chill ? 0.010 : 0.013 + intensity * 0.020 + (lowHp * 0.010))), 1.8);
  }
