...
import { P, ENEMY_KINDS } from './state.v2-0-69.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-69.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, localText, esc as escHtml } from './i18n.v2-0-69.js';
...
${localText('Серия контрактов увеличивает payout, пока вы не провалите контракт.', 'Contract chain increases payout until a contract is failed.')}`;
}
...
export class Hud {
  ...
  handleFx(f, myId, state) {
    ...
    switch (f.t) {
      ...
      case 'room_invoice': {
        ...
        if (f.objective?.done) marks.push(`CONTRACT ${f.objective.label} +${f.objectiveBonusGld || 0}G +${f.objectiveBonusExp || 0}XP`);
        if (f.contractChain >= 2) marks.push(`CHAIN x${f.contractChain} +${f.contractBonusGld || 0}G +${f.contractBonusExp || 0}XP`);
  ...
  openWeaponChest(choices = []) {
    ...
    choices.forEach((opt, i) => {
      ...
      // Element tag is only for real elemental bullet upgrades. `WEAPON FIRE +14%` is fire-rate, not FIRE element.
      const upKey = String(opt.upgrade || opt.id || '');
      const tagClass = upKey === 'bullet_fire' ? 'fire' : upKey === 'bullet_freeze' ? 'freeze' : upKey === 'bullet_poison' ? 'poison' : upKey === 'drone_element_link' ? 'drone' : '';
