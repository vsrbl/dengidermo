...
import { P, ENEMY_KINDS } from './state.v2-0-49.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-49.js';
...
export class Hud {
  ...
  activeRoll(f) {
    ...
    if (f.phase === 'spin') {
      ...
      el.innerHTML = `<div class="roll-title">CASINO MUTATION ROLL</div>` +
        ...
        `<div class="roll-result">ROLLING...</div>`;
    ...
    spans.forEach((sp, i) => {
      ...
      this.activeRollSpin.timers.push(t);
    });
  }
  ...
  openWeaponChest(choices = []) {
    ...
    choices.forEach((opt, i) => {
      ...
      d.innerHTML = `<span class="key">[${i + 1}]</span>${esc(opt.label || opt.id)}${tagLabel}${locked}`;
