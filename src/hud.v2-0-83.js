...
import { P, ENEMY_KINDS } from './state.v2-0-83.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-83.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, priceText, localText, denyText, esc as escHtml } from './i18n.v2-0-83.js';
...
export class Hud {
  ...
  update(state, dt) {
    ...
    if (curRain > 0) {
      ...
      if (debtLayer > 0) {
        const body = localText(`Долговой двигатель не заменяет модификатор комнаты. Он добавляет отдельный постоянный слой: +Static Rain LVL ${debtLayer} только в этой комнате.`, `Debt Engine does not replace the room modifier. It adds a separate permanent layer: +Static Rain LVL ${debtLayer} this room.`);
    ...
    if (pendingNextRain > 0) {
      ...
      const body = nextStaticEligible(nx)
        ? localText('Следующая комната получит обычный Static Rain этого уровня. Это не слой Debt Engine.', 'The next room gets normal Static Rain at this level. This is not the Debt Engine layer.')
    ...
    if (debtEngineNextRain > 0) {
      const body = localText(`Долговой двигатель постоянный до конца забега, но показывается отдельно и не крадёт room modifier. Следующая eligible combat-комната получит +Static Rain LVL ${debtEngineNextRain}.`, `Debt Engine is permanent for this run, but it is shown separately and does not steal the room modifier. The next eligible combat room gets +Static Rain LVL ${debtEngineNextRain}.`);
