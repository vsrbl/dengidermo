...
import { Net, VERSION, GAME_SPEED } from './net.v2-0-36.js';
import { Input } from './input.v2-0-36.js';
import { GameState, P } from './state.v2-0-36.js';
import { Effects } from './effects.v2-0-36.js';
import { Renderer } from './render.v2-0-36.js';
import { Hud } from './hud.v2-0-36.js';
import { AudioBus } from './audio.v2-0-36.js';
import { SKIN_PRESETS, SKIN_RARITIES, DEFAULT_UNLOCKED_SKINS, ACTIVE_CORES, ACTIVE_MUTATIONS } from '../shared/data.v2-0-36.js';
...
function updateSkinPreview() {
  ...
  if (name) name.innerHTML = `${p.name} <span class="skin-rarity rarity-${p.rarity || 'basic'}">${rarity.label}</span>${unlocked ? '' : ' <span class="skin-lock">LOCKED</span>'}`;
  if (note) {
    note.textContent = selected ? 'SELECTED' : (unlocked ? 'UNLOCKED' : 'LOCKED · SKN CACHE / CASINO');
  }
}
