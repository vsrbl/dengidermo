...
import { P, ENEMY_KINDS, ENEMY_LABELS } from './state.v2-0-39.js';
...
export class Renderer {
  ...
  draw(state, effects, view, myPos, mouse, now) {
    ...
    for (const b of view.bullets) {
      const [, bx, by, vx, vy, size, fromP, rocket, kind, elem = ''] = b;
      ctx.save();
      ctx.translate(bx, by);
      ...
      if (rocket) {
        ...
        // SEK v2: smaller digital needle/chip, not a big packet/bubble.
        const tick = Math.floor(now * 24 + bx * 0.05 + by * 0.03) & 3;
        const s = Math.max(4, size);
        ctx.globalAlpha = 0.96;
        ctx.strokeStyle = COL.cyan; ctx.lineWidth = 1.5;
        ctx.strokeRect(Math.round(-s * 1.10), Math.round(-s * 0.38), Math.round(s * 1.85), Math.round(s * 0.76));
        ctx.fillStyle = COL.fg;
        ctx.fillRect(Math.round(s * 0.28), -1, Math.round(s * 0.76), 2);
        ctx.globalAlpha = 0.70; ctx.fillStyle = COL.cyan;
        ctx.fillRect(Math.round(-s * 0.54), Math.round(-s * 0.22), Math.round(s * 0.36), Math.round(s * 0.44));
        ctx.globalAlpha = 0.48; ctx.strokeStyle = COL.cyan; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.moveTo(Math.round(-s * 1.25), 0); ctx.lineTo(Math.round(-s * 2.5 - tick), 0); ctx.stroke();
        ctx.globalAlpha = 0.42; ctx.fillStyle = COL.cyan;
        ctx.fillRect(Math.round(-s * 2.8 - tick), -1, 3, 2);
        ctx.fillRect(Math.round(-s * 3.45 - tick * 1.5), 2, 2, 2);
      ...
      if (fromP && elem) {
        ...
        if (es.includes('fire')) {
          ctx.strokeStyle = COL.red; ctx.lineWidth = 1.4;
        ...
        if (es.includes('freeze')) {
          ctx.strokeStyle = COL.cyan; ctx.lineWidth = 1; ctx.setLineDash([2, 3]);
    ...
    for (const e of view.enemies) {
      const [eid, kindIdx, ex, ey, hp01, size, st, elite, dirX, dirY, shellPct = 0, shellLock = 0, linkId = '', shellType = '', exposed = 0, frozen = 0, burn = 0, poison = 0] = e;
