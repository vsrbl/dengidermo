...
import { P, ENEMY_KINDS } from './state.v2-0-97.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-97.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, priceText, localText, denyText, esc as escHtml } from './i18n.v2-0-97.js';
...
function locFail(v) {
  const s = String(v || '');
  const ru = { 'DAMAGE TAKEN':'ПОЛУЧЕН УРОН', 'TIME LOST':'ВРЕМЯ УШЛО', 'WIRE TOUCHED':'КАСАНИЕ ПРОВОДА', 'LANE HIT':'ПОПАДАНИЕ ЛИНИИ', 'TOO MUCH DMG':'СЛИШКОМ МНОГО УРОНА', 'SHELLS LEFT':'SHELL НЕ СЛОМАНЫ', 'CONDITION LOST':'УСЛОВИЕ СОРВАНО', 'ENEMIES LEFT':'ВРАГИ ОСТАЛИСЬ' };
...
function roomModHint(m, room = {}) {
  ...
  const hints = {
    ...
    static_rain: mode === 'paid'
      ? localText(`Статик-шторм LVL ${lvl}: накопленный заряд сейчас расходуется. На полу появляются квадратные метки; через короткую паузу туда бьёт разряд. После этой комнаты этот уровень исчезнет.`, `Static Storm LVL ${lvl}: a stored charge is being spent now. Square marks appear on the floor; after a short delay, lightning hits them. This level disappears after the room.`)
      : localText(`Статик-шторм LVL ${Math.max(1, lvl)}: комната периодически помечает опасные квадраты и бьёт по ним разрядом. Чем выше LVL, тем чаще и жёстче удары.`, `Static Storm LVL ${Math.max(1, lvl)}: the room marks dangerous squares and strikes them. Higher LVL means faster and harsher strikes.`),
    ...
    anchor_gravity: localText('Якоря гравитации: квадратные точки тянут игроков, врагов, подборы и пули. Планируй путь через тягу.', 'Gravity Anchors: square nodes pull players, enemies, pickups, and bullets. Plan movement around the pull.'),
