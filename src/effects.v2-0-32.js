  ...
  handleFx(f, info) {
    ...
    switch (f.t) {
      ...
      case 'active': {
        ...
        this.add({ kind: 'squareField', activeKind: String(f.label || '').toLowerCase().replace(/ .*/, ''), x: f.x, y: f.y, r: f.r || 160, ttl: 0.26, color: col });
      ...
      case 'active_line_tick':
        this.add({ kind: 'voidLine', tick: 1, x: f.x1, y: f.y1, x2: f.x2, y2: f.y2, w: f.width || 44, ttl: 0.14, color: '#b45cff' });
        break;
      case 'active_field':
        this.add({ kind: 'squareField', activeKind: String(f.kind || ''), x: f.x, y: f.y, r: f.r || 130, ttl: 0.30, color: f.tone === 'red' ? '#ff3048' : (f.tone === 'purple' ? '#b45cff' : '#66f6ff') });
        break;
      case 'active_tick':
