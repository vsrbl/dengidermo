...
import { P, ENEMY_KINDS } from './state.v2-0-73.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-73.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, localText, esc as escHtml } from './i18n.v2-0-73.js';
...
function objectiveExplain(obj = {}) {
  ...
  const map = {
    ...
    hunted_open: localText('Открой hunted exit portal. Контракт фиксируется сразу в момент открытия портала.', 'Open the hunted exit portal. The contract is locked the moment the portal opens.'),
...
function weaponReadability(opt = {}) {
  ...
  const m = {
    ...
    rkt_cluster: {
      ...
      changeRu: '+2 cluster blasts · больше AoE', changeEn: '+2 cluster blasts · more AoE'
    },
    rkt_mines: {
      role: 'CONTROL', tone: 'control',
      ru: 'ROCKETGUN оставляет отложенные квадратные мины.', en: 'ROCKETGUN leaves delayed square mines.',
      changeRu: 'area denial · ловушки после взрыва', changeEn: 'area denial · traps after blast'
    },
    wpn_dmg: {
      role: 'DPS', tone: 'dps',
