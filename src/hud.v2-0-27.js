...
import { P, ENEMY_KINDS } from './state.v2-0-27.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-27.js';
...
export class Hud {
  ...
  update(state, dt) {
    ...
    const skn = room.skinReward ? ` · <span class="term" data-explain-title="SKN CACHE" data-explain="В этой комнате спрятан закрытый скин. Название не раскрывается. Пройди комнату и выйди в портал, чтобы открыть его локально на этом устройстве.">SKN ${rarityText(room.skinReward)}</span>` : '';
    if (room.phase === 'install') obj.innerHTML = '<span class="done">INSTALL-ФАЗА</span>';
    else if (room.cat === 'boss') obj.innerHTML = room.portal[2] ? `<span class="done">ПОРТАЛ ОТКРЫТ — E</span>${skn}` : `УНИЧТОЖИТЬ БОССА${skn}`;
    else if (room.portal[2]) obj.innerHTML = `<span class="done">ПОРТАЛ ОТКРЫТ — E</span>${skn}`;
    else obj.innerHTML = `ЗАЧИСТКА ${room.kills} / ${room.quota}${skn}`;
  ...
  handleFx(f, myId, state) {
    ...
    switch (f.t) {
      case 'room': {
        ...
        this.banner(f.cat === 'boss' ? 'ЭТАЖ БОССА' : `${f.roomId}`, `LOOP ${f.loop} · DEPTH ${f.depth}${mods ? ' · ' + mods : ''}${f.director ? ' · ' + f.director : ''}${skn}`,
          f.cat === 'boss' ? 'red' : (f.skinRarity ? 'purple' : (mods ? 'purple' : '')));
        if (f.skinRarity) this.feed(`SKN CACHE: ${rarityText(f.skinRarity)} / после портала`, 'p');
      ...
      case 'director_room':
        this.feed(`ДИРЕКТОР: ${f.label || 'ROOM'}`, 'c');
        break;
      case 'director_wave':
        this.feed(`${f.label || 'WAVE'} · ${f.count || 0}`, f.intent === 'armor' ? 'p' : (f.intent === 'ranged' || f.intent === 'control' ? 'c' : 'r'));
        break;
      case 'skin_room': this.feed(`НЕИЗВЕСТНЫЙ SKN ${rarityText(f.skinRarity)} СПРЯТАН В КОМНАТЕ`, 'p'); break;
      case 'skin_room_ready': this.banner('SKN CACHE ГОТОВ', `E через портал · ${rarityText(f.skinRarity)}`, 'purple'); this.feed(`SKN CACHE ГОТОВ · ${rarityText(f.skinRarity)}`, 'p'); break;
      case 'portal_open': this.banner('ПОРТАЛ ОТКРЫТ', f.skinRarity ? `E — забрать SKN ${rarityText(f.skinRarity)}` : 'E — перейти дальше', f.skinRarity ? 'purple' : 'green'); this.feed(f.skinRarity ? `ПОРТАЛ ОТКРЫТ · SKN ${rarityText(f.skinRarity)}` : 'ПОРТАЛ ОТКРЫТ', f.skinRarity ? 'p' : 'g'); break;
  ...
  setTab(visible, state) {
    ...
    $('tab-run').innerHTML =
      ...
      `<span class="term" data-explain-title="ПРАВИЛА" data-explain="Активные модификаторы комнаты, которые меняют бой и награды.">ПРАВИЛА</span>: ${(room.mods || []).map(m => esc(MOD_LABELS[m] || m)).join(', ') || '—'}${room.skinReward ? '   SKN: ' + esc(rarityText(room.skinReward)) : ''}`;
    const t = $('tab-table');
    let html = '<tr>' +
  ...
  openAbilityChest(choices = []) {
    ...
    choices.forEach((opt, i) => {
      const d = document.createElement('div');
      const rarity = opt.rarity || opt.tone || (String(opt.actionLabel || '').includes('ЗАМЕНИТ') ? 'rare' : 'common');
      const tone = opt.disabled ? 'red' : (opt.tone || (rarity === 'cursed' ? 'red' : rarity === 'rare' ? 'purple' : rarity === 'core' ? 'cyan' : 'green'));
      d.className = 'choice ability-choice' + (opt.disabled ? ' disabled' : '') + ` tone-${tone} rarity-${rarity}`;
      const locked = opt.disabled ? `<span class="lock">${esc(opt.disabledReason || 'ЗАКРЫТО')}</span>` : '';
      const action = opt.actionLabel ? `<div class="choice-sub">${esc(opt.actionLabel)}</div>` : '';
      const tag = `<span class="rarity-tag">${esc(String(rarity).toUpperCase())}</span>`;
      d.innerHTML = `<span class="key">[${i + 1}]</span>${esc(opt.label || opt.id)}${tag}${locked}${action}`;
      const title = opt.disabled ? `${opt.label || opt.id} / НЕДОСТУПНО` : `${opt.label || opt.id} / ${String(rarity).toUpperCase()}`;
      const body = `${opt.actionLabel ? opt.actionLabel + ': ' : ''}${opt.preview ? opt.preview + ' ' : ''}${opt.desc || 'Награда сундука способностей.'}${opt.disabled ? ` Недоступно: ${opt.disabledReason || 'условие не выполнено'}.` : ' Можно выбрать сейчас.'}`;
  ...
  casinoResult(f, myId) {
    ...
    if (f.id !== myId) {
      const RES = { JCK: 'ДЖЕКПОТ', LOSE: 'проигрыш', STC: 'СТАТИКА', SKN: 'СКИН' };
