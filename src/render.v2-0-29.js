...
import { P, ENEMY_KINDS, ENEMY_LABELS } from './state.v2-0-29.js';
...
export class Renderer {
  ...
  draw(state, effects, view, myPos, mouse, now) {
    ...
    for (const e of view.enemies) {
      const [eid, kindIdx, ex, ey, hp01, size, st, elite, dirX, dirY, shellPct = 0, shellLock = 0, linkId = '', shellType = '', exposed = 0] = e;
    ...
    for (const p of view.players) {
      ...
      const skinBarrel = /^#[0-9a-fA-F]{6}$/.test(p[P.SKINBARREL] || '') ? p[P.SKINBARREL] : (isMe ? COL.green : '#555');
      ...
      const rarePulse = ['bone_static','black_lime','bad_tv'].includes(skinId) ? 1 + Math.sin(now * 8) * 0.035 : 1;
      const superJit = ['red_static','mirror_coin','terminal_ghost'].includes(skinId) ? Math.sign(Math.sin(now * 31)) * 1.4 : 0;
      const legPulse = ['jackpot_wound','dead_channel'].includes(skinId) ? 1 + Math.sin(now * 10) * 0.075 : 1;
      if (skinId === 'jackpot_wound') {
        this.square(px, py, 42 + Math.sin(now * 13) * 2, { stroke: '#ffd34d', lw: 1, alpha: 0.22, rotate: Math.PI / 4 + now * 0.8 });
        this.square(px, py, 36, { stroke: '#ff3048', lw: 1, alpha: 0.22, rotate: -Math.PI / 4 + now * 0.35 });
      } else if (skinId === 'dead_channel') {
        this.square(px + Math.sin(now * 17) * 1.5, py, 40, { stroke: '#f3f3f3', lw: 1, alpha: 0.18, rotate: Math.PI / 4 });
        this.square(px - Math.sin(now * 23) * 1.2, py, 34, { stroke: '#66f6ff', lw: 1, alpha: 0.16, rotate: -Math.PI / 4 });
      } else if (skinId === 'red_static' || skinId === 'mirror_coin' || skinId === 'terminal_ghost') {
        this.square(px + superJit, py - superJit, 34, { stroke: skinOutline, lw: 1, alpha: 0.24, rotate: Math.PI / 4 });
      }
      this.square(px + superJit, py - superJit, 28 * rarePulse * legPulse, { fill: skinFill, stroke: skinOutline, lw: 2 });
      // Authored preset markings: tiny signal cosmetics, not free color picking.
      ctx.strokeStyle = skinOutline; ctx.lineWidth = 1;
      ctx.globalAlpha *= 0.72;
      if (skinId === 'debt_red' || skinId === 'bruise_purple') {
        ctx.beginPath(); ctx.moveTo(px - 9, py - 7); ctx.lineTo(px + 8, py + 6); ctx.moveTo(px - 5, py + 9); ctx.lineTo(px + 10, py - 8); ctx.stroke();
        ...
        ctx.setLineDash([2,2]); ctx.strokeRect(px - 11 + Math.sin(now*19), py - 11, 22, 22); ctx.setLineDash([]);
        ...
        ctx.beginPath(); ctx.arc(px, py, 8 + Math.sin(now*5)*1.2, 0, Math.PI * 2); ctx.stroke();
      } else if (skinId === 'red_static') {
        ctx.strokeStyle = '#ff3048'; for (let yy = -9; yy <= 9; yy += 6) { ctx.beginPath(); ctx.moveTo(px - 11 + Math.sin(now*25+yy)*2, py + yy); ctx.lineTo(px + 11, py + yy); ctx.stroke(); }
        ...
        ctx.strokeStyle = '#66f6ff'; ctx.globalAlpha *= 0.65; ctx.strokeRect(px - 13 + Math.sin(now*10)*2, py - 13, 26, 26); ctx.strokeRect(px - 9, py - 9, 18, 18);
        ...
        ctx.strokeStyle = '#f3f3f3'; ctx.setLineDash([3, 3]); ctx.strokeRect(px - 12, py - 12, 24, 24); ctx.setLineDash([]); ctx.strokeStyle = '#66f6ff'; ctx.beginPath(); ctx.moveTo(px - 11, py + Math.sin(now*20)*6); ctx.lineTo(px + 11, py - Math.sin(now*20)*6); ctx.stroke();
      }
      ctx.restore();
      // aim tick
      const ax = p[P.AX], ay = p[P.AY];
      const al = Math.hypot(ax - px, ay - py) || 1;
      ctx.strokeStyle = skinBarrel; ctx.lineWidth = 2;
