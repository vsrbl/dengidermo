...
import { Net, VERSION, GAME_SPEED } from './net.v2-0-23.js';
import { Input } from './input.v2-0-23.js';
import { GameState, P } from './state.v2-0-23.js';
import { Effects } from './effects.v2-0-23.js';
import { Renderer } from './render.v2-0-23.js';
import { Hud } from './hud.v2-0-23.js';
import { AudioBus } from './audio.v2-0-23.js';
...
function readSkin() {
  return {
    fill: cleanColor($('skin-fill')?.value, '#f3f3f3'),
    outline: cleanColor($('skin-outline')?.value, '#00ff66'),
    barrel: cleanColor($('skin-barrel')?.value, '#00ff66')
  };
}
function saveSkin() {
  const skin = readSkin();
  localStorage.setItem('nnc_skin', JSON.stringify(skin));
  updateSkinPreview(skin);
  return skin;
}
function updateSkinPreview(skin = readSkin()) {
  const core = document.querySelector('#skin-preview .skin-core');
  const stick = document.querySelector('#skin-preview .skin-stick');
  if (core) { core.style.background = skin.fill; core.style.borderColor = skin.outline; core.style.boxShadow = `0 0 10px ${skin.outline}55`; }
  if (stick) { stick.style.background = skin.barrel; stick.style.boxShadow = `0 0 8px ${skin.barrel}88`; }
}
function loadSkin() {
  let saved = null;
  try { saved = JSON.parse(localStorage.getItem('nnc_skin') || 'null'); } catch {}
  const skin = {
    fill: cleanColor(saved?.fill, '#f3f3f3'),
    outline: cleanColor(saved?.outline, '#00ff66'),
    barrel: cleanColor(saved?.barrel, '#00ff66')
  };
  if ($('skin-fill')) $('skin-fill').value = skin.fill;
  if ($('skin-outline')) $('skin-outline').value = skin.outline;
  if ($('skin-barrel')) $('skin-barrel').value = skin.barrel;
  updateSkinPreview(skin);
  return skin;
}
...
loadSkin();
['skin-fill','skin-outline','skin-barrel'].forEach(id => $(id)?.addEventListener('input', saveSkin));
$('btn-skin-reset')?.addEventListener('click', () => {
  $('skin-fill').value = '#f3f3f3';
  $('skin-outline').value = '#00ff66';
  $('skin-barrel').value = '#00ff66';
  saveSkin();
});
