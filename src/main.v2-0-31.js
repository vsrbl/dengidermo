...
import { Net, VERSION, GAME_SPEED } from './net.v2-0-31.js';
import { Input } from './input.v2-0-31.js';
import { GameState, P } from './state.v2-0-31.js';
import { Effects } from './effects.v2-0-31.js';
import { Renderer } from './render.v2-0-31.js';
import { Hud } from './hud.v2-0-31.js';
import { AudioBus } from './audio.v2-0-31.js';
import { SKIN_PRESETS, SKIN_RARITIES, DEFAULT_UNLOCKED_SKINS, ACTIVE_CORES, ACTIVE_MUTATIONS } from '../shared/data.v2-0-31.js';
...
function updateSkinPreview() {
  ...
  if (editor) {
    editor.dataset.skin = unlocked ? p.id : 'locked';
    editor.dataset.rarity = p.rarity || 'basic';
    editor.classList.toggle('locked', !unlocked);
    editor.classList.toggle('selected', !!selected);
  }
  if (core) {
    core.style.background = unlocked ? p.fill : '#090909';
    core.style.borderColor = unlocked ? p.outline : '#2a2a2a';
    core.style.boxShadow = unlocked ? `0 0 ${p.rarity === 'legendary' ? 26 : p.rarity === 'superrare' ? 20 : 12}px ${p.outline}88` : 'none';
  }
  if (stick) {
    stick.style.background = unlocked ? p.barrel : '#202020';
    stick.style.boxShadow = unlocked ? `0 0 12px ${p.barrel}aa` : 'none';
  }
  if (name) name.innerHTML = unlocked
    ? `${p.name} <span class="skin-rarity rarity-${p.rarity || 'basic'}">${rarity.label}</span>`
    : `LOCKED SKN <span class="skin-rarity rarity-${p.rarity || 'basic'}">${rarity.label}</span>`;
  if (note) {
    if (!unlocked) note.textContent = `${rarity.label} // скрыт · открой через редкую комнату или SKN в казино`;
    else {
      const prefix = selected ? 'SELECTED' : 'UNLOCKED';
      note.textContent = `${prefix} // ${p.note}`;
    }
  }
}
...
function handleSkinUnlock(id, source = 'casino') {
  ...
  if (newly && skin) {
    ...
    try { hud.feed(`${prefix}: ${skin.name}`, skin.rarity === 'legendary' || skin.rarity === 'superrare' ? 'p' : 'g'); } catch {}
  } else if (skin) {
...
function ensureDevPanel() {
  ...
  devPanel.innerHTML = `
    ...
    <div class="dev-note">SOLO/HOST only · быстрые проверки Q / waves / skins</div>
    <div class="dev-row"><label>CORE</label><select id="dev-core">${coreOpts}</select></div>
    <div class="dev-row"><label>LVL</label><select id="dev-level"><option>1</option><option>2</option><option selected>3</option></select></div>
