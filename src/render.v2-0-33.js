...
import { P, ENEMY_KINDS, ENEMY_LABELS } from './state.v2-0-33.js';
  ...
  draw(state, effects, view, myPos, mouse, now) {
    ...
    for (const b of view.bullets) {
      ...
      if (rocket) {
        ...
        // SEK should read as a digital packet, not an organic bubble.
        const tick = Math.floor(now * 18 + bx * 0.03 + by * 0.02) & 3;
        ctx.strokeStyle = COL.cyan; ctx.lineWidth = 2;
        ctx.strokeRect(-size * 1.05, -size * 0.72, size * 2.1, size * 1.44);
        ctx.strokeStyle = COL.fg; ctx.lineWidth = 1;
        ctx.strokeRect(-size * 0.42, -size * 0.42, size * 0.84, size * 0.84);
        ctx.fillStyle = COL.cyan; ctx.globalAlpha = 0.68;
        ctx.fillRect(size * 0.25, -2, size * 1.05, 4);
        ctx.globalAlpha = 0.42; ctx.fillStyle = COL.fg;
        ctx.fillRect(-size * 1.35, -size * 0.35, 3 + tick, 2);
        ctx.fillRect(-size * 1.75, size * 0.22, 7 - tick, 2);
        ctx.globalAlpha = 0.50; ctx.strokeStyle = COL.cyan; ctx.setLineDash([2, 5]);
        ctx.beginPath(); ctx.moveTo(-size * 1.6, 0); ctx.lineTo(-size * 4.0, 0); ctx.stroke(); ctx.setLineDash([]);
        ctx.globalAlpha = 0.38; ctx.fillStyle = COL.cyan;
        ctx.fillRect(-size * 3.5 - tick * 2, -2, 7, 4);
