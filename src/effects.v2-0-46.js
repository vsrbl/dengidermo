  ...
  handleFx(f, info) {
    ...
    switch (f.t) {
      ...
      case 'active_line':
        ...
        else if (f.kind === 'void_laser') this.add({ kind: 'voidLaser', x: f.x1, y: f.y1, x2: f.x2, y2: f.y2, w: f.width || 2, hitW: f.hitWidth || 8, ttl: 0.10, color: '#b45cff' });
      ...
      case 'active_line_tick':
        if (f.kind === 'void_laser') this.add({ kind: 'voidLaser', tick: 1, x: f.x1, y: f.y1, x2: f.x2, y2: f.y2, w: f.width || 2, hitW: f.hitWidth || 8, ttl: 0.08, color: '#b45cff' });
  ...
  drawWorld(ctx) {
    for (const e of this.list) {
      ...
      if (e.kind === 'muzzle') {
        ...
        const nx = -uy, ny = ux;
        const fade = 1 - p;
        const coreW = Math.max(1.25, e.w || 2);
        const glowW = Math.max(3.5, (e.hitW || 8) * 0.42);
        const startSq = 8;
        const endSq = 6;
        ctx.setLineDash([]);
        ctx.globalAlpha = fade * (e.tick ? 0.20 : 0.15);
        ...
        ctx.globalAlpha = fade * (e.tick ? 0.98 : 0.90);
        ...
        ctx.fillStyle = e.color;
        ctx.fillRect(Math.round(e.x - startSq / 2), Math.round(e.y - startSq / 2), startSq, startSq);
        ctx.fillStyle = '#f4f1ff';
        ctx.fillRect(Math.round(e.x - 2), Math.round(e.y - 2), 4, 4);
        ctx.globalAlpha = fade * 0.62;
        ctx.fillStyle = e.color;
        ctx.fillRect(Math.round(e.x2 - endSq / 2), Math.round(e.y2 - endSq / 2), endSq, endSq);
        ctx.globalAlpha = fade * 0.36; ctx.fillStyle = e.color;
        for (let i = 0; i < 4; i++) {
          const q = (i + 1) / 5;
          const off = ((i % 2) ? 1 : -1) * 3.5;
          const bx = e.x + dx * q + nx * off;
          const by = e.y + dy * q + ny * off;
