...
import { Net, VERSION, BUILD_ID, PROTOCOL, GAME_SPEED } from './net.v2-0-93.js';
import { Input } from './input.v2-0-93.js';
import { GameState, P } from './state.v2-0-93.js';
import { Effects } from './effects.v2-0-93.js';
import { Renderer } from './render.v2-0-93.js';
import { Hud } from './hud.v2-0-93.js';
import { AudioBus } from './audio.v2-0-93.js';
import { SKIN_PRESETS, SKIN_RARITIES, DEFAULT_UNLOCKED_SKINS, ACTIVE_CORES, ACTIVE_MUTATIONS, ROOM_MODS, SPECIAL_ROOMS, ROOM_SEQUENCE } from '../shared/data.v2-0-93.js';
import { setupLanguageButtons, onLangChange, t, skinNote, labelStatus, getLang, locRole, locAction } from './i18n.v2-0-93.js';
...
function handleSkinUnlock(id, source = 'casino') {
  ...
  if (isSkinUnlocked(finalId)) {
    ...
    if (!alt) {
      try { hud.feed(`${getLang() !== 'en' ? 'ДУБЛЬ СКИНА' : 'SKN DUPLICATE'}: ${skin.rarity?.toUpperCase?.() || 'SKIN'}`, 'r'); } catch {}
      return;
    }
...
function syncAudioSliders() {
  ...
  if (mv) mv.value = Math.round(vals.music * 100);
  if (sv) sv.value = Math.round(vals.sfx * 100);
}
$('music-volume')?.addEventListener('input', e => audio.setMusicVolume?.(Number(e.currentTarget.value) / 100));
$('sfx-volume')?.addEventListener('input', e => audio.setSfxVolume?.(Number(e.currentTarget.value) / 100));
syncAudioSliders();
onLangChange(() => {
  updateSkinPreview();
