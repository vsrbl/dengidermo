...
const RU = {
  ui: {
    ...
    modsTitle: 'ПРАВИЛА КОМНАТЫ', modsBody: 'Особые правила комнаты: темнота, статический дождь, greed debt, casino virus, shifting walls, slow grid, blood tax, echo shots, anchor gravity и chill-room.',
...
const EN_UPGRADE = {
  ...
  droneproc: 'Drone bullets copy part of BLAST PROC.', orbreflect: 'Orbitals can erase enemy bullets.', debtengine: 'Permanent cursed engine for this run: big damage and luck, but every eligible combat room gets a separate layer: DEBT ENGINE +STATIC RAIN LVL 1 THIS ROOM per stack. It does not replace the room modifier.', overload: 'Much more damage, but maximum HP goes down.', gamble: 'More luck, but movement speed goes down.'
};
const RU_UPGRADE_CLEAN = {
  dmg: 'Весь исходящий урон растёт.', fire: 'Перезарядка оружия становится короче.', spd: 'Скорость движения растёт.', maxhp: 'Максимальное здоровье растёт.', magnet: 'Радиус притяжения pickups растёт.', dash: 'Добавляет один заряд рывка.', drone: 'Добавляет автостреляющего дрона.', orbital: 'Добавляет орбиталь с контактным уроном.', luck: 'Улучшает броски апгрейдов и казино.', proc: 'Попадания пуль могут создавать взрывы.', echo: 'Шанс выпустить дополнительные выстрелы.', leech: 'Лечение от нанесённого урона.', goldgun: 'Враги дают больше GLD при смерти.', dashcut: 'Рывок оглушает врагов рядом с траекторией. Стаки увеличивают радиус и длительность.', dashclone: 'Рывок оставляет echo-всплеск в точке старта.', droneproc: 'Пули дронов получают часть BLAST PROC.', orbreflect: 'Орбитали могут стирать вражеские пули.', debtengine: 'Постоянный cursed-двигатель до конца забега: большой урон и удача, но каждая eligible combat-комната получает отдельный слой: DEBT ENGINE +STATIC RAIN LVL 1 THIS ROOM за стак. Он не заменяет модификатор комнаты.', overload: 'Сильно повышает урон, но снижает максимум HP.', gamble: 'Даёт больше удачи, но снижает скорость движения.',
  ...
  sek_swarm: 'ПКМ выпускает рой самонаводящихся SEEKER-пуль сразу. Каждый стак даёт +5 пуль и чуть дольше cooldown.',
...
const RU_ENEMY = {
  grunt:'Базовый преследователь с контактным уроном.', runner:'Быстрый слабый враг, ломает позицию.', tank:'Медленный бронированный враг-стена.', shooter:'Дальний враг, стреляет красными снарядами.', charger:'Готовится, затем делает рывок по линии.', bomber:'Включает fuse и взрывается. Уходи из радиуса.', bouncer:'Pinball-враг: отскакивает и толкает.', glitch:'Blink attacker: телепортируется и бьёт.', echo:'Враг-копия: стреляет типом оружия выбранного игрока, но медленнее и держит дистанцию стрелка.', orbiter:'Подвижный guard с фронтальным shield.', anchor:'Control core: тянет и замедляет.', splitter:'После смерти делится на мелких врагов.', prism:'Стреляет split/prism lanes.', pulse:'Давит square-wave атакой.', leech:'Лечит раненых врагов. Приоритетная цель.', herald:'Призывает swarm за линией давления.', warden:'Связывает shell armor у врагов.', damper:'Медленное подвижное поле, тормозит пули и прикрывает ближайших врагов.', boss:'Босс: burst fire и adds.'
};
...
export function cleanPlayerText(text) {
  ...
  if (langIsEn() && /[А-Яа-яЁё]/.test(s)) {
    const exact = {
      ...
      'Перезарядка оружия становится короче.': 'Weapon cooldown becomes shorter.',
