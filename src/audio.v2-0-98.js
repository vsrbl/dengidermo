...
export class AudioBus {
  ...
  play(type) {
    ...
    switch (type) {
      case 'shot_shg':
        ...
        break;
      case 'shot_sek':
        // v2.0.98: SEEKER is 50% quieter and pitched higher; still dry/terminal, no bubble chirp.
  ...
  musicRootFor(context = {}) {
    // v2.0.98: one tonal anchor for the whole procedural score.
    // Bass is only a shadow under this root; melodic layers carry the music.
    return 49.00;
  }

  musicScaleFor(context = {}) {
    if (context.staticLike) return [0, -1, -5, -6, -10, -12, -13, -17, -18, -22];
    if (context.casino) return [0, -1, -6, -7, -11, -12, -13, -18, -19, -23];
    if (context.boss) return [0, -2, -5, -6, -8, -11, -12, -17, -18, -24];
    if (context.chill || context.menu) return [0, -2, -5, -7, -10, -12, -14, -17, -19];
    return [0, -1, -5, -7, -8, -11, -12, -17, -19, -24];
  }

  musicMotifsFor(context = {}) {
    // Downward, suspended phrases. No bright cadences, no happy arpeggio loop.
    const normal = [
      [0, 2, 4, 6, 7],
      [1, 3, 2, 5, 8],
      [0, 4, 6, 5, 8, 9],
    ...
    if (context.boss) return bossSet.concat(normal);
    if (context.staticLike) return staticSet.concat(normal);
    if (context.casino) return casinoSet.concat(normal);
    return normal;
  }
  ...
  playDirgePhrase(context = {}) {
    ...
    const scale = this.musicScaleFor(context);
    const motifs = this.musicMotifsFor(context);
    const semitone = n => root * Math.pow(2, n / 12);

    const phraseSeed = this.music.motifIndex++;
    const phase = (phraseSeed + (boss ? 5 : 0) + (casino ? 7 : 0) + (staticLike ? 11 : 0) + Math.floor(crowd * 6) + Math.floor(damage * 8)) % motifs.length;
    const motif = motifs[phase].map(i => scale[i % scale.length]);
    const pressure = Math.max(intensity, crowd * 0.8, damage * 0.9);
    const phraseMode = (phraseSeed + (boss ? 2 : 0) + (staticLike ? 3 : 0) + (casino ? 4 : 0) + Math.floor(pressure * 5)) % 5;

    const baseStep = menu ? 1.55 : chill ? 1.75 : boss ? 0.92 : Math.max(0.82, 1.34 - pressure * 0.34);
    const swing = staticLike ? 0.18 : casino ? 0.11 : 0.07;
    const leadVol = menu ? 0.010 : chill ? 0.010 : 0.017 + pressure * 0.018 + damage * 0.008;
    const padNoteVol = menu ? 0.0045 : 0.006 + pressure * 0.006;
    const filterBase = staticLike ? 260 : casino ? 330 : boss ? 300 : 290;
    const registerBase = menu ? 12 : chill ? 10 : (pressure > 0.78 ? 17 : 12);

    // Bass accent is deliberately almost subliminal. It no longer leads the track.
    if (phraseMode === 3 || boss || damage > 0.72) {
      this.musicNote(root * 0.5, baseStep * 2.8, 'sine', 0.0009 + pressure * 0.0019, 74 + pressure * 18, 0, 0.999, 0);
    }

    // Low mournful body, but not a foreground bass loop.
    const bodyInterval = scale[(phraseSeed + 3) % scale.length] - 12;
    this.musicNote(semitone(bodyInterval), baseStep * (boss ? 4.8 : 5.7), 'triangle', padNoteVol * 0.36, 185 + pressure * 55, baseStep * 0.18, 0.999, -6);

    motif.forEach((m, i) => {
      // More variation through gaps and route changes. Horror needs air.
      if (!boss && !menu && pressure < 0.50 && ((phraseSeed + i) % 5 === 2)) return;
      if (chill && i > 3 && ((phraseSeed + i) % 2)) return;
      const odd = i % 2;
      const delay = i * baseStep * (1 + (odd ? swing : -swing * 0.45));
      const durMul = phraseMode === 1 ? (1.65 + (i % 2) * 0.55) : phraseMode === 2 ? 0.92 : phraseMode === 4 ? 2.15 : 1.25;
      const dur = baseStep * (i === motif.length - 1 ? durMul + 1.2 : durMul);
      const register = registerBase + (phraseMode === 2 && i % 3 === 0 ? 7 : 0) + (phraseMode === 4 ? -5 : 0);
      const f = semitone(m + register);
      const type = phraseMode === 2 ? 'square' : (i % 3 === 0 ? 'triangle' : 'sawtooth');
      const detune = (odd ? -8 : 5) + (staticLike ? -5 : 0) + (casino ? 4 : 0) + (boss ? -3 : 0);
      const vol = leadVol * (phraseMode === 4 ? 0.78 : 1) * (i === 0 ? 0.72 : 1);
      this.musicNote(f, dur, type, vol, filterBase + i * 34 + pressure * 80, delay, phraseMode === 2 ? 0.995 : 0.999, detune);

      // Randomized ambience-bed events, not busy percussion.
      if ((staticLike || damage > 0.45 || phraseMode === 2) && i % 2 === 1) this.musicDust(0.05 + pressure * 0.04, 0.003 + pressure * 0.006, 3600 + i * 330, delay + 0.035);
      if (casino && i === motif.length - 2 && pressure > 0.28) this.musicNote(f * 1.5, 0.10, 'square', 0.0025 + pressure * 0.0025, 980, delay + 0.05, 1, -11);
    });

    // Rare high lament / alarm line for danger, instead of louder bass.
    if ((crowd > 0.48 || damage > 0.38 || boss) && phraseMode !== 4) {
      const a = scale[(phraseSeed + 5) % scale.length] + (boss ? 19 : 17);
      const b = scale[(phraseSeed + 7) % scale.length] + (boss ? 17 : 15);
      const when = baseStep * (1.2 + ((phraseSeed % 3) * 0.25));
      this.musicNote(semitone(a), baseStep * 1.9, 'triangle', 0.008 + pressure * 0.011, 520 + pressure * 160, when, 0.997, -9);
      if (pressure > 0.63) this.musicNote(semitone(b), baseStep * 2.2, 'sawtooth', 0.004 + pressure * 0.006, 360, when + baseStep * 0.55, 0.998, 7);
    }
  }
  ...
  ensureMusic() {
    ...
    this.music.layers.sub = this.makeToneLayer(24.5, 'sine', 95);
    this.music.layers.pulse = this.makeToneLayer(98, 'square', 210);
    this.music.layers.hat = this.makeNoiseLayer(4800, 10);
    this.music.layers.casino = this.makeToneLayer(294, 'square', 620);
    this.music.layers.choir = this.makeToneLayer(147, 'sawtooth', 500);
    this.music.layers.dirgePad = this.makeToneLayer(98, 'sawtooth', 340);
    this.music.layers.scrape = this.makeNoiseLayer(760, 3.8);
    this.music.layers.glass = this.makeToneLayer(392, 'triangle', 1200);
    return true;
  }
  ...
  updateMusic(state, dt = 0.016) {
    ...
    if (this.musicStepT <= 0) {
      this.musicStepT = menu ? 2.9 : Math.max(0.95, 1.95 - intensity * 0.55);
      const now = this.ctx.currentTime;
      ...
      if (pulse) pulse.o.frequency.setTargetAtTime(root * (combat && intensity > 0.62 ? 3 : 2), now, 0.9);
      ...
      if (dirgePad) {
        ...
        dirgePad.f.frequency.setTargetAtTime(staticLike ? 250 : (boss ? 235 : 325), now, 1.2);
      }
      if (choirL) choirL.f.frequency.setTargetAtTime((boss || choir) ? 430 + intensity * 180 : 310 + intensity * 90, now, 1.4);
    ...
    if ((room || menu) && this.music.phraseT <= 0) {
      this.playDirgePhrase({ boss, chill, casino, staticLike, intensity: menu ? 0.14 : intensity, menu, damage, crowd: alivePressure });
      const roomShift = (staticLike ? -0.75 : 0) + (casino ? -0.55 : 0) + (lowHp > 0.45 ? -0.85 : 0) + (damage > 0.45 ? -0.70 : 0);
      this.music.phraseT = menu ? Math.max(4.5, 7.4 - (this.music.motifIndex % 3) * 0.9) : Math.max(1.85, ((boss ? 3.7 : chill ? 7.8 : 5.1) - intensity * 2.25 - alivePressure * 0.70 + roomShift + (portalOpen ? 1.1 : 0)));
    }

    this.setMusicLayer('drone', inGame * (menu ? 0.016 : (chill ? 0.018 : 0.021 + intensity * 0.006)), 1.5);
    this.setMusicLayer('sub', menu ? 0.00015 : (combat ? (0.00025 + Math.max(0, intensity - 0.82) * 0.0035 + lowHp * 0.0012 + (boss ? 0.0018 : 0)) : (chill ? 0.00022 : 0.0001)), 1.45);
    this.setMusicLayer('pulse', menu ? 0.00015 : (combat ? (0.00035 + Math.max(0, intensity - 0.58) * 0.0042) : (portalOpen ? 0.00055 : 0.00012)), 1.1);
    this.setMusicLayer('hat', combat && intensity > 0.38 ? ((intensity - 0.32) * 0.017) : 0.0001, 0.5);
    this.setMusicLayer('casino', casino ? (chill ? 0.0035 : 0.0018 + intensity * 0.0038) : 0.0001, 1.1);
    this.setMusicLayer('choir', menu ? 0.014 : (choir || boss ? (0.024 + intensity * 0.066) : (0.010 + intensity * 0.018)), 1.25);
    this.setMusicLayer('dirgePad', inGame * (menu ? 0.026 : (chill ? 0.020 : 0.034 + intensity * 0.035 + (lowHp * 0.014))), 2.1);
    this.setMusicLayer('scrape', combat && (staticLike || damage > 0.25 || alivePressure > 0.45) ? (0.002 + intensity * 0.008) : 0.0001, 1.0);
    this.setMusicLayer('glass', menu ? 0.004 : (combat && intensity > 0.46 ? 0.003 + intensity * 0.008 : 0.0001), 1.8);
  }
