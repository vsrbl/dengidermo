...
import { P, ENEMY_KINDS } from './state.v2-0-92.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-92.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, locLabel, locReward, disabledReason, objectStateText, priceText, localText, denyText, esc as escHtml } from './i18n.v2-0-92.js';
  ...
  update(state, dt) {
    ...
    if (pendingNextRain > 0) {
      ...
      const body = nextStaticEligible(nx)
        ? localText('Следующая комната получит статик-шторм этого уровня.', 'The next room gets Static Rain at this level.')
