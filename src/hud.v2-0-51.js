...
import { P, ENEMY_KINDS } from './state.v2-0-51.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-51.js';
import { t, onLangChange, cleanPlayerText, activeNoneLabel, activeNoneDesc, activeShort as locActiveShort, activeDescFrom, chestDesc, pickupDesc, enemyDesc, weaponDesc, optionDesc, locAction, locRole, disabledReason, objectStateText, localText, esc as escHtml } from './i18n.v2-0-51.js';
...
export class Hud {
  ...
  setWorldHover(state, input, renderer) {
    ...
    if (!found) for (const o of state.latest.objects || []) {
      ...
      if (type === 'bet') found = { title: t('betTitle'), body: t('betInspect'), tone: 'red' };
  ...
  openWeaponChest(choices = []) {
    ...
    choices.forEach((opt, i) => {
      ...
      const title = opt.disabled ? `${opt.label || opt.id} / ${t('unavailable').toUpperCase()}` : (opt.label || opt.id);
  ...
  openAbilityChest(choices = []) {
    ...
    choices.forEach((opt, i) => {
      ...
      const title = opt.disabled ? `${opt.label || opt.id} / ${t('unavailable').toUpperCase()}` : `${opt.label || opt.id} / ${String(group).toUpperCase()}`;
