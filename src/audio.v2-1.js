...
export class AudioBus {
  constructor() {
    ...
    this.cooldowns = {
      ...
      dash: 0.07, dash_uncommon: 0.08, dash_rare: 0.09, dash_superrare: 0.11, dash_jackpot: 0.16, dash_dead_channel: 0.16, jump_takeoff: 0.12, jump_wall: 0.10, jump_rebound_combo: 0.055, jump_land: 0.14, skin_legendary: 0.55, gld: 0.055, exp: 0.055, hea: 0.09, pickup: 0.05,
  ...
  playJumpReboundCombo(mult = 2) {
    if (!this.can('jump_rebound_combo')) return;
    // Each new x2/x3/x4... step has its own ascending pitch. The climb is
    // capped in the safe high-mid range so long pinball chains never squeal.
    const step = Math.max(0, Math.min(24, (Math.max(2, Number(mult) || 2) - 2) * 2));
    const root = 330 * Math.pow(2, step / 12);
    this.tone(root, 0.060, 'square', 0.040, 1.08);
    this.tone(root * 1.5, 0.040, 'triangle', 0.023, 1.03, 0.010);
    this.noise(0.018, 0.012, 4100 + Math.min(2600, step * 90), 11, 0.004);
  }
  ...
  play(type) {
    ...
    switch (type) {
      ...
      case 'ui_click':
        ...
        this.noise(0.012, 0.010, 4200, 10, 0.002);
        break;
      case 'wager_accept_click':
        this.noise(0.018, 0.014, 3600, 9, 0.000);
        this.tone(392, 0.045, 'square', 0.030, 1.12, 0.000);
        this.tone(659.25, 0.055, 'triangle', 0.024, 0.82, 0.032);
        break;
      case 'wager_skip_click':
        this.noise(0.014, 0.012, 2400, 7, 0.000);
        this.tone(330, 0.040, 'square', 0.025, 0.82, 0.000);
        this.tone(220, 0.045, 'triangle', 0.018, 0.68, 0.028);
        break;
      case 'wager_complete':
        this.noise(0.025, 0.014, 5200, 12, 0.000);
        this.tone(392.00, 0.075, 'square', 0.042, 1.08, 0.000);
        this.tone(587.33, 0.090, 'triangle', 0.034, 0.92, 0.055);
        this.tone(783.99, 0.115, 'triangle', 0.026, 0.84, 0.118);
        break;
      case 'boss_silence_enter':
        this.noise(0.160, 0.075, 740, 2.2, 0.000);
        this.tone(116.54, 0.240, 'sawtooth', 0.080, 0.48, 0.000);
        this.tone(77.78, 0.300, 'square', 0.052, 0.42, 0.070);
        break;
      case 'boss_silence_exit':
        this.noise(0.050, 0.020, 4300, 11, 0.000);
        this.tone(233.08, 0.085, 'square', 0.038, 1.16, 0.000);
        this.tone(466.16, 0.120, 'triangle', 0.032, 0.92, 0.055);
        break;
      case 'trinode_fire':
      ...
      case 'trinode_section_break':
        ...
        break;
      case 'anchor_phase_field':
        this.noise(0.105, 0.050, 760, 2.8, 0.000);
        this.tone(82.41, 0.220, 'sawtooth', 0.075, 1.80, 0.000);
        this.tone(164.81, 0.180, 'square', 0.040, 1.35, 0.055);
        this.tone(329.63, 0.120, 'triangle', 0.025, 1.06, 0.145);
        break;
      case 'anchor_phase_shots':
        this.noise(0.075, 0.040, 3600, 9.0, 0.000);
        this.tone(659.25, 0.085, 'square', 0.042, 0.72, 0.000);
        this.tone(329.63, 0.105, 'square', 0.036, 0.68, 0.045);
        this.tone(164.81, 0.135, 'triangle', 0.028, 0.60, 0.105);
        break;
      case 'oct_laser':
        this.noise(0.045, 0.026, 5200, 13, 0.000);
        this.tone(740, 0.070, 'square', 0.025, 0.78, 0.000);
        this.tone(1480, 0.040, 'triangle', 0.014, 0.64, 0.018);
        break;
      case 'oct_phase':
        this.noise(0.070, 0.035, 3100, 9, 0.000);
        this.tone(196, 0.085, 'square', 0.045, 1.15, 0.000);
        this.tone(392, 0.100, 'square', 0.034, 1.12, 0.055);
        this.tone(784, 0.125, 'triangle', 0.025, 1.08, 0.125);
        break;
      case 'oct_crash':
        this.noise(0.190, 0.120, 480, 1.8, 0.000);
        this.tone(156, 0.190, 'sawtooth', 0.105, 0.42, 0.000);
        this.tone(78, 0.300, 'square', 0.075, 0.34, 0.085);
        this.noise(0.120, 0.045, 4200, 10, 0.045);
        break;
      case 'jackpot':
  ...
  handleFx(f, info = {}) {
    ...
    switch (f.t) {
      ...
      case 'player_jump_wall':
        if (mine) {
          this.play('jump_wall');
          if ((f.mul || 1) > 1) this.playJumpReboundCombo(f.mul);
        }
        break;
      ...
      case 'impact_wall_push': if (mine) this.play('impact_wall_push'); break;
      ...
      case 'anchor_phase': this.play(f.phase === 'field' ? 'anchor_phase_field' : 'anchor_phase_shots'); break;
      case 'oct_phase': this.play('oct_phase'); if (f.red) this.play('boss_silence_enter'); break;
      case 'oct_laser_pulse': this.play('oct_laser'); break;
      case 'oct_crash': this.play('oct_crash'); break;
      ...
      case 'room_wager_complete': if (mine) this.play('wager_complete'); break;
      case 'room_wager_reward_active': if (mine) this.play('casino_win'); break;
      case 'room_wager_paid': if (mine && !f.silent) this.play('wager_complete'); break;
      case 'room_wager_lost': if (mine) { this.play('denied'); this.play('debt'); } break;
      case 'boss_q_silence': if (!f.id || mine) this.play('boss_silence_enter'); break;
      case 'boss_q_silence_end': if (mine) this.play('boss_silence_exit'); break;
