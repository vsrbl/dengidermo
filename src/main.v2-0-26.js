...
import { Net, VERSION, GAME_SPEED } from './net.v2-0-26.js';
import { Input } from './input.v2-0-26.js';
import { GameState, P } from './state.v2-0-26.js';
import { Effects } from './effects.v2-0-26.js';
import { Renderer } from './render.v2-0-26.js';
import { Hud } from './hud.v2-0-26.js';
import { AudioBus } from './audio.v2-0-26.js';
import { SKIN_PRESETS, SKIN_RARITIES, DEFAULT_UNLOCKED_SKINS } from '../shared/data.v2-0-26.js';
...
function updateSkinPreview() {
  ...
  if (core) { core.style.background = p.fill; core.style.borderColor = p.outline; core.style.boxShadow = unlocked ? `0 0 ${p.rarity === 'legendary' ? 26 : p.rarity === 'superrare' ? 20 : 12}px ${p.outline}88` : 'none'; }
  if (stick) { stick.style.background = p.barrel; stick.style.boxShadow = unlocked ? `0 0 12px ${p.barrel}aa` : 'none'; }
  if (name) name.innerHTML = `${unlocked ? '' : 'LOCKED // '}${p.name} <span class="skin-rarity rarity-${p.rarity || 'basic'}">${rarity.label}</span>`;
  if (note) {
    const prefix = selected ? 'SELECTED' : unlocked ? 'UNLOCKED' : 'CASINO DROP';
...
function handleCasinoSkinReward(pl = {}) {
  ...
  const newly = unlockSkin(pl.skinId);
  const skin = SKIN_BY_ID[pl.skinId];
  if (newly && skin) {
    selectedSkinId = skin.id;
    const idx = presetById(skin.id); if (idx >= 0) skinIndex = idx;
    localStorage.setItem(skinSaveKey, selectedSkinId);
    updateSkinPreview();
    try { hud.feed(`SKN UNLOCK: ${skin.name}`, skin.rarity === 'legendary' || skin.rarity === 'superrare' ? 'p' : 'g'); } catch {}
  } else if (skin) {
    try { hud.feed(`SKN DUPLICATE: ${skin.name}`, 'r'); } catch {}
  }
}
