...
export class Hud {
  ...
  bindContextHelp() {
    // v2.0.9: stable delegated hover help.
    // The previous version re-ran showHelp on every pointermove and anchored to DOM nodes
    // that TAB redraws every frame. This version stores a stable panel/rect and only
    // changes content when the hovered help id changes.
    const findHelp = (clientX, clientY, fallback = null) => {
      const direct = fallback?.closest?.('[data-help]');
      if (direct && !direct.closest?.('#context-help')) return direct;
      ...
      for (const el of stack) {
        const h = el?.closest?.('[data-help]');
        if (h && !h.closest?.('#context-help')) return h;
      }
      return null;
    };
    this._findHelp = findHelp;

    document.addEventListener('pointermove', e => {
      this.helpPointer = { x: e.clientX, y: e.clientY };
      const el = findHelp(e.clientX, e.clientY, e.target);
      if (el?.dataset?.help) this.showHelp(el.dataset.help, el);
      else this.hideHelpSoon(110);
    }, { passive: true });
    ...
    document.addEventListener('pointerleave', () => this.hideHelp(), { passive: true });
    document.addEventListener('focusin', e => {
      const el = e.target?.closest?.('[data-help]');
      if (el) this.showHelp(el.dataset.help, el);
    });
    document.addEventListener('focusout', () => this.hideHelpSoon(90));
  ...
  helpPanelFor(anchor = null) {
    ...
    return anchor.closest?.('.panel') || anchor.closest?.('#hud-prompt') || anchor.closest?.('#hud-bottom') || anchor.closest?.('#hud-top') || anchor;
  }
  ...
  showHelp(id, anchor = null) {
    ...
    const panelRect = panel?.getBoundingClientRect?.() || anchor?.getBoundingClientRect?.() || null;
    this.helpAnchor = anchor;
    this.helpPanel = panel;
    this.helpAnchorRect = anchor?.getBoundingClientRect?.() || null;
    this.helpPanelRect = panelRect;

    const changed = this.helpVisibleId !== id || box.classList.contains('hidden');
    ...
    this.positionHelp();
    requestAnimationFrame(() => this.positionHelp());
  }
  ...
  positionHelp() {
    ...
    box.style.maxWidth = Math.min(320, Math.max(220, window.innerWidth - margin * 2)) + 'px';

    const panelRect = this.helpPanel?.isConnected ? this.helpPanel.getBoundingClientRect() : this.helpPanelRect;
    const anchorRect = this.helpAnchor?.isConnected ? this.helpAnchor.getBoundingClientRect() : this.helpAnchorRect;
    const r = panelRect || anchorRect || { left: this.helpPointer.x, right: this.helpPointer.x, top: this.helpPointer.y - 60, bottom: this.helpPointer.y + 60, height: 120 };
    const bw = box.offsetWidth || 300;
    const bh = box.offsetHeight || 130;

    // Default: right next to the menu/panel that owns the hovered term.
    ...
    // If both sides are tight, pin beside the hovered word instead of window edge.
    if (left < margin && anchorRect) {
      left = Math.min(window.innerWidth - bw - margin, Math.max(margin, anchorRect.right + gap));
      top = anchorRect.top - 10;
      box.classList.remove('left-side');
    }
    if (left + bw + margin > window.innerWidth) left = window.innerWidth - bw - margin;
  ...
  ensureHelpStillValid() {
    const box = $('context-help');
    if (!box || box.classList.contains('hidden')) return;
    // Hide only when the whole owning menu/panel disappeared. Do not hide just because
    // TAB redrew the specific span under the cursor.
    if (this.helpPanel && !this.helpPanel.isConnected) { this.hideHelp(); return; }
    if (this.helpPanel?.closest?.('.overlay.hidden')) { this.hideHelp(); return; }
    if (this.helpAnchor && !this.helpAnchor.isConnected) {
      const p = this.helpPointer || { x: -1, y: -1 };
      const fresh = this._findHelp?.(p.x, p.y, null);
      if (fresh?.dataset?.help) { this.showHelp(fresh.dataset.help, fresh); return; }
      // Keep the last stable panel placement for a few frames instead of flickering.
    }
    this.positionHelp();
  }
