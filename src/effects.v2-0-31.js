    ...
    switch (f.t) {
      ...
      case 'dash': {
        ...
        this.add({
          ...
          ttl: legendary ? 0.34 : superRare ? 0.29 : rare ? 0.25 : 0.21,
        ...
        if (mine) { this.zoomKick = legendary ? 0.55 : 0.42; this.kick(legendary ? 5 : superRare ? 4 : 3); }
        break;
      }
      case 'levelup':
      ...
      case 'active_line':
        this.add({ kind: 'voidLine', x: f.x1, y: f.y1, x2: f.x2, y2: f.y2, w: f.width || 44, ttl: 0.20, color: f.tone === 'red' ? '#ff3048' : (f.tone === 'purple' ? '#b45cff' : '#66f6ff') });
        break;
      case 'active_line_tick':
        this.add({ kind: 'voidLinePulse', x: f.x1, y: f.y1, x2: f.x2, y2: f.y2, w: f.width || 44, ttl: 0.18, color: '#b45cff' });
        break;
      case 'active_field':
        this.add({ kind: 'squareField', x: f.x, y: f.y, r: f.r || 130, ttl: 0.22, color: f.tone === 'red' ? '#ff3048' : (f.tone === 'purple' ? '#b45cff' : '#66f6ff') });
        break;
      case 'active_tick':
        this.add({ kind: 'ring', x: f.x, y: f.y, r: f.r || 130, ttl: 0.16, color: f.tone === 'red' ? '#ff3048' : (f.tone === 'purple' ? '#b45cff' : '#66f6ff') });
        break;
  ...
  drawWorld(ctx) {
    for (const e of this.list) {
      ...
      if (e.kind === 'muzzle') {
        ...
        const power = rarity === 'legendary' ? 4 : rarity === 'superrare' ? 3 : rarity === 'rare' ? 2 : rarity === 'uncommon' ? 1 : 0;
        ctx.globalAlpha = (1 - p) * 0.94;
        ctx.strokeStyle = e.color; ctx.lineWidth = 2 + Math.min(1.5, power * 0.35);
        ctx.setLineDash(style === 'dead_channel' ? [5, 4, 1, 4] : style === 'jackpot' ? [13, 5, 3, 5] : [9, 6]);
        for (let i = 0; i < 7 + power; i++) {
          if (((i + step) % 4) === 0) continue;
          const a = (i / (7 + power)) * len;
          const b = Math.min(len, a + len * (0.10 + power * 0.012));
          const off = ((i % 2) ? 8 : -8) * (1 - p) + (style === 'mirror' ? ((i % 2) ? 9 : -9) : 0);
          ctx.beginPath();
        ...
        ctx.setLineDash([]);
        // Skin identity lives in the dash, not in animated player bodies: hard digital blocks along the cut.
        const blocks = 3 + power * 2;
        for (let i = 0; i < blocks; i++) {
          const q = (i + 1) / (blocks + 1);
          const skip = ((i + step) % 5) === 1 && power < 4;
          if (skip) continue;
          const bx = e.x + dx * q + nx * (((i % 3) - 1) * (5 + power * 2));
          const by = e.y + dy * q + ny * (((i % 3) - 1) * (5 + power * 2));
          const sz = 6 + power * 2 + ((i + step) % 2) * 3;
          ctx.globalAlpha = (1 - p) * (0.28 + power * 0.08);
          ctx.fillStyle = (i % 2 && e.alt) ? e.alt : e.color;
          ctx.fillRect(Math.round(bx - sz / 2), Math.round(by - sz / 2), sz, sz);
        }
        if (power >= 1) {
          ctx.globalAlpha = (1 - p) * 0.55;
          ctx.strokeStyle = e.alt || e.color; ctx.lineWidth = 1.5;
          ctx.setLineDash([3, 5]);
          ctx.beginPath();
          ctx.moveTo(Math.round(e.x + nx * (12 + power * 3)), Math.round(e.y + ny * (12 + power * 3)));
          ctx.lineTo(Math.round(e.x2 + nx * (12 + power * 3)), Math.round(e.y2 + ny * (12 + power * 3)));
        ...
        if (style === 'jackpot') {
          ...
          for (let i = 0; i < 5; i++) {
            const q = (i + 0.5) / 5;
            const s = 12 + (i % 2) * 5;
            ctx.strokeStyle = i % 2 ? (e.alt || '#ff3048') : (e.color || '#ffd34d');
            ctx.strokeRect(Math.round(e.x + dx * q - s / 2), Math.round(e.y + dy * q - s / 2), s, s);
          }
        } else if (style === 'dead_channel') {
          ctx.globalAlpha = (1 - p) * 0.50;
          ctx.strokeStyle = e.alt || '#66f6ff'; ctx.lineWidth = 1;
          for (let i = 0; i < 4; i++) {
            const off = (i - 1.5) * 7;
            ctx.beginPath();
            ctx.moveTo(Math.round(e.x + nx * off + (i % 2 ? 2 : -2)), Math.round(e.y + ny * off));
            ctx.lineTo(Math.round(e.x2 + nx * off - (i % 2 ? 2 : -2)), Math.round(e.y2 + ny * off));
            ctx.stroke();
          }
        } else if (style === 'tv' || style === 'static' || style === 'red_static') {
          ctx.globalAlpha = (1 - p) * 0.38;
          ctx.fillStyle = e.alt || e.color;
          for (let i = 0; i < 7; i++) {
            const q = (i + 1) / 8;
            const w = 18 + (i % 3) * 7;
            ctx.fillRect(Math.round(e.x + dx * q - w / 2), Math.round(e.y + dy * q + nx * 0 - 1), w, 2);
          }
        }
        ctx.fillStyle = e.color; ctx.globalAlpha = (1 - p) * 0.62;
        ctx.fillRect(Math.round(e.x2 - 9 - power), Math.round(e.y2 - 9 - power), 18 + power * 2, 18 + power * 2);
        ctx.globalAlpha = (1 - p) * 0.28;
        ctx.fillRect(Math.round(e.x - 13 - power), Math.round(e.y - 13 - power), 26 + power * 2, 26 + power * 2);
      } else if (e.kind === 'voidLine' || e.kind === 'voidLinePulse') {
        const dx = e.x2 - e.x, dy = e.y2 - e.y;
        const len = Math.hypot(dx, dy) || 1;
        const ux = dx / len, uy = dy / len;
        const nx = -uy, ny = ux;
        const baseW = Math.max(16, e.w || 44);
        const pulse = e.kind === 'voidLinePulse';
        const step = Math.floor((e.t || 0) * 38);
        ctx.globalAlpha = (1 - p) * (pulse ? 0.76 : 0.48);
        ctx.strokeStyle = e.color; ctx.lineWidth = pulse ? 3 : 2;
        ctx.setLineDash(pulse ? [5, 3, 1, 3] : [12, 7, 2, 7]);
        for (let i = -2; i <= 2; i++) {
          const off = i * baseW * 0.22 + ((i + step) % 2 ? 2 : -2);
          ctx.beginPath();
          ctx.moveTo(Math.round(e.x + nx * off), Math.round(e.y + ny * off));
        ...
        ctx.setLineDash([]);
        ctx.globalAlpha = (1 - p) * (pulse ? 0.24 : 0.16);
        ctx.fillStyle = e.color;
        for (let i = 0; i < 9; i++) {
          if (((i + step) % 4) === 0 && !pulse) continue;
          const q = (i + 0.5) / 9;
          const sz = pulse ? 9 : 6 + (i % 2) * 3;
          const bx = e.x + dx * q + nx * (((i % 3) - 1) * baseW * 0.28);
          const by = e.y + dy * q + ny * (((i % 3) - 1) * baseW * 0.28);
          ctx.fillRect(Math.round(bx - sz / 2), Math.round(by - sz / 2), sz, sz);
        }
