...
export class AudioBus {
  ...
  play(type) {
    ...
    switch (type) {
      case 'shot_shg':
        ...
        break;
      case 'shot_sek':
        // v2.0.100: SEEKER is 50% quieter and pitched higher; still dry/terminal, no bubble chirp.
  ...
  musicRootFor(context = {}) {
    // v2.0.100: one dark tonal home. Bass is only a distant floor;
  ...
  musicMotifsFor(context = {}) {
    // v2.0.100: explicit melody families. Same dark scale grammar, different moods.
  ...
  updateMusic(state, dt = 0.016) {
    ...
    // v2.0.100: the drones are floor texture only. Clear melodic phrases carry the music.
