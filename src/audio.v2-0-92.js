...
function inGameMusicAmount(room) { return room ? 1 : 0; }
...
export class AudioBus {
  ...
  play(type) {
    ...
    switch (type) {
      case 'shot_shg':
        ...
        break;
      case 'shot_sek':
        // v2.0.92: SEEKER is 50% quieter and pitched higher; still dry/terminal, no bubble chirp.
  ...
  ensureMusic() {
    ...
    this.music = { master, layers: {}, phraseT: 0.5, motifIndex: 0, lastRoomTone: '' };
  ...
  updateMusic(state, dt = 0.016) {
    ...
    const combat = room?.phase === 'play' && !room?.portal?.[2] && room?.cat !== 'chill';
    const boss = room?.cat === 'boss';
    const chill = room?.cat === 'chill' || room?.special === 'chill_room';
    ...
    const intensity = Math.max(0, Math.min(1, (enemies / 22) * 0.55 + (bullets / 60) * 0.18 + lowHp * 0.35 + (boss ? 0.35 : 0) + (staticLike ? 0.10 : 0)));
    const portalOpen = !!room?.portal?.[2];

    this.musicStepT = Math.max(0, (this.musicStepT || 0) - dt);
    if (this.musicStepT <= 0) {
      this.musicStepT = 1.35;
      const now = this.ctx.currentTime;
      const pulse = this.music.layers.pulse;
      const casinoL = this.music.layers.casino;
      const dirgePad = this.music.layers.dirgePad;
      if (pulse) pulse.o.frequency.setTargetAtTime(boss ? 74 : (combat ? 82 + intensity * 42 : 58), now, 0.4);
      if (casinoL) casinoL.o.frequency.setTargetAtTime(casino ? (chill ? 247 : 330 + intensity * 80) : 220, now, 0.6);
      if (dirgePad) {
        const base = boss ? 65.41 : (chill ? 73.42 : 98.00);
    ...
    const inGame = inGameMusicAmount(room);
    this.music.phraseT = Math.max(0, (this.music.phraseT || 0) - dt);
    if (room && this.music.phraseT <= 0) {
      this.playDirgePhrase({ boss, chill, casino, staticLike, intensity });
      // Slow enough to live above the drone without becoming a cheerful loop.
      this.music.phraseT = (boss ? 4.7 : chill ? 7.2 : 5.6) - intensity * 1.35 + (portalOpen ? 1.0 : 0);
    }

    this.setMusicLayer('drone', inGame * (chill ? 0.034 : 0.052 + intensity * 0.022), 1.1);
    this.setMusicLayer('sub', combat ? (0.012 + intensity * 0.043 + lowHp * 0.032) : (chill ? 0.010 : 0.004), 0.75);
    this.setMusicLayer('pulse', combat ? (0.012 + intensity * 0.056) : (portalOpen ? 0.014 : 0.003), 0.45);
    this.setMusicLayer('hat', combat && intensity > 0.22 ? ((intensity - 0.18) * 0.050) : 0.0001, 0.35);
    this.setMusicLayer('casino', casino ? (chill ? 0.020 : 0.010 + intensity * 0.020) : 0.0001, 0.9);
    this.setMusicLayer('choir', choir || boss ? (0.010 + intensity * 0.040) : 0.0001, 1.0);
    this.setMusicLayer('dirgePad', inGame * (chill ? 0.010 : 0.013 + intensity * 0.014 + (lowHp * 0.010)), 1.8);
  }
