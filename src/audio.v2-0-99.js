...
export class AudioBus {
  ...
  play(type) {
    ...
    switch (type) {
      case 'shot_shg':
        ...
        break;
      case 'shot_sek':
        // v2.0.99: SEEKER is 50% quieter and pitched higher; still dry/terminal, no bubble chirp.
  ...
  musicRootFor(context = {}) {
    // v2.0.99: one dark tonal home. Bass is only a distant floor;
  ...
  musicMotifsFor(context = {}) {
    // Direct semitone melodies, not indexes. They share the same dark grammar,
    // but rooms pick different phrase shapes so the music is clearly varied.
    const menu = [
      [12, 10, 7, 5, 3, 0],
      [15, 12, 10, 7, 8, 5],
      [19, 17, 15, 12, 10, 7],
    ...
    const bossSet = [
      [24, 23, 19, 17, 12, 11, 7],
      [19, 16, 15, 12, 8, 7, 4, 0],
      [27, 24, 23, 19, 16, 15, 12],
      [17, 19, 16, 12, 11, 7, 4]
    ];
    if (context.menu || context.chill) return menu.concat(normal);
  ...
  playDirgePhrase(context = {}) {
    ...
    const pressure = Math.max(intensity, crowd * 0.85, damage * 0.95);
    const phase = (phraseSeed + (boss ? 5 : 0) + (casino ? 7 : 0) + (staticLike ? 11 : 0) + Math.floor(crowd * 7) + Math.floor(damage * 9)) % motifs.length;
    const motif = motifs[phase];
    const phraseMode = (phraseSeed + (boss ? 2 : 0) + (staticLike ? 3 : 0) + (casino ? 4 : 0) + Math.floor(pressure * 6)) % 6;

    const step = menu ? 0.92 : chill ? 1.04 : boss ? 0.58 : Math.max(0.56, 0.86 - pressure * 0.18);
    const swing = staticLike ? 0.17 : casino ? 0.10 : 0.06;
    const leadVol = menu ? 0.030 : chill ? 0.028 : 0.040 + pressure * 0.026 + damage * 0.014;
    const answerVol = menu ? 0.014 : 0.017 + pressure * 0.013;
    const highVol = boss || damage > 0.45 || crowd > 0.52 ? 0.016 + pressure * 0.018 : 0.007;
    const filterBase = menu ? 840 : staticLike ? 720 : casino ? 920 : boss ? 820 : 880;
    const leadType = phraseMode === 1 ? 'sine' : (phraseMode === 3 || casino ? 'triangle' : 'sawtooth');

    // Rare bass shadow only. This should be felt, not heard as the main music.
    if (boss || damage > 0.78 || (phraseMode === 4 && pressure > 0.58)) {
      this.musicNote(root * 0.5, step * 3.2, 'sine', 0.00045 + pressure * 0.0011, 58 + pressure * 12, 0, 0.999, 0);
    }

    // A very low body note sits behind the melody, heavily filtered.
    const body = motif[Math.min(3, motif.length - 1)] - 24;
    this.musicNote(semitone(body), step * (boss ? 5.7 : 6.6), 'triangle', 0.0018 + pressure * 0.0022, 145 + pressure * 30, step * 0.25, 0.999, -7);

    // Main melody: now intentionally audible and varied.
    motif.forEach((m, i) => {
      if (!boss && !menu && pressure < 0.32 && i > 4 && ((phraseSeed + i) % 3 === 1)) return;
      const delay = i * step * (1 + (i % 2 ? swing : -swing * 0.35));
      const hold = (i === motif.length - 1 ? 1.95 : (phraseMode === 2 ? 0.92 : phraseMode === 5 ? 1.45 : 1.18));
      const dur = step * hold;
      const octave = phraseMode === 2 && i % 3 === 0 ? 12 : 0;
      const f = semitone(m + octave);
      const detune = (i % 2 ? -6 : 4) + (staticLike ? -4 : 0) + (casino ? 5 : 0) + (boss ? -3 : 0);
      const vol = leadVol * (i === 0 ? 0.86 : 1) * (phraseMode === 5 && i % 2 ? 0.78 : 1);
      this.musicNote(f, dur, leadType, vol, filterBase + i * 44 + pressure * 120, delay, phraseMode === 3 ? 0.997 : 0.999, detune);

      // Counter-line every other note, lower and quieter, so it feels composed rather than random.
      if (i % 2 === 1 && i < motif.length - 1) {
        const lower = motif[Math.max(0, i - 1)] - 12;
        this.musicNote(semitone(lower), step * 1.6, 'triangle', answerVol * (boss ? 1.25 : 1), 520 + pressure * 110, delay + step * 0.42, 0.999, -10);
      }

      // Air/noise details only support the melody; they do not replace it.
      if ((staticLike || damage > 0.42) && i % 3 === 2) this.musicDust(0.045 + pressure * 0.035, 0.002 + pressure * 0.004, 3900 + i * 260, delay + 0.035);
    });

    // High lament: a recognizable top-line when the room gets tense.
    if (crowd > 0.40 || damage > 0.34 || boss || phraseMode === 2) {
      const a = motif[(phraseSeed + 2) % motif.length] + (boss ? 12 : 7);
      const b = motif[(phraseSeed + 4) % motif.length] + (boss ? 10 : 5);
      const when = step * (1.35 + ((phraseSeed % 3) * 0.20));
      this.musicNote(semitone(a), step * 2.2, 'sine', highVol, 1080 + pressure * 220, when, 0.998, -11);
      if (pressure > 0.60) this.musicNote(semitone(b), step * 2.4, 'triangle', highVol * 0.72, 760 + pressure * 160, when + step * 0.62, 0.998, 7);
    }
  }
  ...
  updateMusic(state, dt = 0.016) {
    ...
    if ((room || menu) && this.music.phraseT <= 0) {
      ...
      this.music.phraseT = menu ? Math.max(3.4, 5.8 - (this.music.motifIndex % 4) * 0.55) : Math.max(1.55, ((boss ? 2.8 : chill ? 5.9 : 3.7) - intensity * 1.45 - alivePressure * 0.42 + roomShift + (portalOpen ? 0.9 : 0)));
    }

    // v2.0.99: ambience stays behind the phrases. Melody is the foreground.
    this.setMusicLayer('drone', inGame * (menu ? 0.008 : (chill ? 0.010 : 0.011 + intensity * 0.003)), 1.8);
    this.setMusicLayer('sub', menu ? 0.00005 : (combat ? (0.00006 + Math.max(0, intensity - 0.88) * 0.0011 + lowHp * 0.00035 + (boss ? 0.00045 : 0)) : 0.00005), 1.7);
    this.setMusicLayer('pulse', menu ? 0.00005 : (combat ? (0.00008 + Math.max(0, intensity - 0.72) * 0.0013) : (portalOpen ? 0.00012 : 0.00005)), 1.4);
    this.setMusicLayer('hat', combat && intensity > 0.62 ? ((intensity - 0.58) * 0.006) : 0.0001, 0.7);
    this.setMusicLayer('casino', casino ? (chill ? 0.0012 : 0.0009 + intensity * 0.0018) : 0.0001, 1.3);
    this.setMusicLayer('choir', menu ? 0.008 : (choir || boss ? (0.010 + intensity * 0.026) : (0.006 + intensity * 0.010)), 1.6);
    this.setMusicLayer('dirgePad', inGame * (menu ? 0.011 : (chill ? 0.010 : 0.012 + intensity * 0.010 + (lowHp * 0.004))), 2.4);
    this.setMusicLayer('scrape', combat && (staticLike || damage > 0.25 || alivePressure > 0.58) ? (0.001 + intensity * 0.003) : 0.0001, 1.2);
    this.setMusicLayer('glass', menu ? 0.002 : (combat && intensity > 0.46 ? 0.002 + intensity * 0.004 : 0.0001), 2.0);
  }
