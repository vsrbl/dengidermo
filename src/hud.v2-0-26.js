...
import { P, ENEMY_KINDS } from './state.v2-0-26.js';
import { UPGRADES, WEAPONS, CHESTS, ROOM_MODS, BET_STAKES, ENEMIES } from '../shared/data.v2-0-26.js';
  ...
  handleFx(f, myId, state) {
    ...
    switch (f.t) {
      case 'room': {
        ...
        this.banner(f.cat === 'boss' ? 'ЭТАЖ БОССА' : `${f.roomId}`, `LOOP ${f.loop} · DEPTH ${f.depth}${mods ? ' · ' + mods : ''}${f.director ? ' · ' + f.director : ''}`,
